...
/*************VARS VARS VARS*****************/
int addtime = 200;
//change max speed from 1 sec to 2sec
int MINDIST = 4000;
//change pref distance from 2K to 4K
int MAXTARSPEED = 75;
int MINTARSPEED = 0;
...
double fsin(double d) {
	int id = (int)d;
	int tid = (d > id?1: - 1) + id;
	double ds = abs(sin(tid) - sin(id));
	return sin(id) + ds * (d - id);
}
double fcos(double d) {
	int id = (int)d;
	int tid = (d > id?1: - 1) + id;
	double ds = abs(cos(tid) - cos(id));
	return cos(id) + ds * (d - id);
}
double ftan(double d) {
	int id = (int)d;
	int tid = (d > id?1: - 1) + id;
	double ds = abs(tan(tid) - tan(id));
	return tan(id) + ds * (d - id);
}
...
class Point {
	...
	int getAngleTo(Point p) {
		return atan2(p.y - y, p.x - x);
	}
	int getDistanceTo(Point p) {
		return dist(p.y, p.x, y, x);
	}
...
class ScanPoint {
	...
	public:
	int stime, dir, width, range;
	//known at scan time
	Point me;
	Point pos;
	//dirived from each scan
	double accuracy;
	ScanPoint() {
	}
...
class Memory {
	...
	//total average position//recent average position// variance//notice stationary (average position//notice min max area covered//notice charging/retreating
...
int speedturntime(int oldspeed, int newspeed, int olddir, int newdir) {
	...
	if(newspeed != oldspeed)at = acceltime(newspeed, oldspeed);
	return(max(at, tt) + ft);
}
...
void domove() {
	...
	if( !crash(me, myspeed1, mydir1, addtime)) {
		...
		setspeed = myspeed1;
		settime = time1;
		if(setspeed < 0)setdir = me.getAngleTo(CENTERPOINT) + 180;
		else setdir = me.getAngleTo(CENTERPOINT);
	}
...
int main() {
	...
	while(1) {
		memory.getState();
		if(memory.getTime() > nextmovetime)//change donot remove at 0 speed may extra move
domove();
