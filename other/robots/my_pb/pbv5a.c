...
void domove(){
  ...
  int comp=0;
  if (dist>MAXDIST) {//too far
    comp=min(5*((dist-MAXDIST)/300),20);
  }
  if (dist<MINDIST) {//too close
    comp=min(5*((MINDIST-dist)/300),20);
  }
