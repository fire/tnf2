...
int getDirectAngleTo(Point p) {return
(int)atan2(p.y-loc_y(),p.x-loc_x());}
int getDirectDistanceTo(Point p) {return dist(p.y,p.x,loc_y(),loc_x());}
...
//report();
  ...
  int shottime=realran/MAXSHOOTSPEED*SCANTIME+SHOTTIME;
  if (shotran<=7400) {
...
//(int)atan2(pingloc[PINGTOP].y-loc_y(),pingloc[PINGTOP].x-loc_x());
  if (!found) {
    scan_wid=10;
    lost++;clockwise++;
    ...
    if (tracking) {
      lost=0;
      int minscan=(int)(200.0/dd*180/MYPI/2);//approx methodII
//int minscan=(int)(200.0/dd*180/MYPI+1);//methodI
//int minscan=(int)(200.0/dd*180/MYPI+1)-(90-atan(dd/100));//methodII
      scan_wid=max(scan_wid/2,minscan);
//      if (scan_wid!=minscan)
//        scandir+=scan_wid/2*((clockwise+tracking)%2 ? 1 : -1);
    }
    else {
      lost++;clockwise++;
