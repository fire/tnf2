...
int main ()
{
  ...
  int sFine[5][2] = { {0, 1}, {3, 3}, {-3, 3}, {6, 6}, {-6, 6} };
  int pFine = 0;
  ...
  while (1) {
	...
	pWide = (++pWide) % 20;
      ...
      //Fine search & track
      dir = ldir + sFine[pFine][0];
      if (pFine == 0 && shot1 > mt + 100)
	dir -= 1;
      rng = scan (dir, sFine[pFine][1]);
      if (rng > 0) {
	fx = (int)(mx + cos (dir + sFine[pFine][1]) * rng);
	fy = (int)(my + sin (dir + sFine[pFine][1]) * rng);
	Ht[Hptr] = mt;
	Hx[Hptr] = fx;
	Hy[Hptr] = fy;
	Hptr = (++Hptr) % 6;
	...
	fx = mx + cos (dir - sFine[pFine][1]) * rng;
	fy = my + sin (dir - sFine[pFine][1]) * rng;
	Ht[Hptr] = mt;
	Hx[Hptr] = fx;
	Hy[Hptr] = fy;
	Hptr = (++Hptr) % 6;
	...
	py = 0;
      ...
      pr = sqrt (px * px + py * py);
      pd = atan2 (py, px);
      printlog ("Log x%5d y%5d a%3d d%5d", px + mx, py + my, pd, pr);
