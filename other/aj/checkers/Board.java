...
public class Board {
	 ...
	 * Description of the Field
	 */
	public static char REDPAWN = 'r', REDKING = 'R', WHITEPAWN = 'w',
			WHITEKING = 'W', BLOCK = '#', EMPTY = ' ';
	 ...
	 *            Description of Parameter
	 */
	public void findJump(int level, String child, int pos, boolean rmove,
			boolean wmove, boolean imRed) {
		...
		if (rmove) {
			...
			if (jr != -1) {
				...
				if (level == 0) {
					...
					findJump(level + 1, child + "-" + jr, jr, rmove, wmove,
							imRed);
				}
				setMap(i, c);
				setMap(pos, m);
			...
			if (jl != -1) {
				...
				if (level == 0) {
					...
					findJump(level + 1, child + "-" + jl, jl, rmove, wmove,
							imRed);
				}
				setMap(i, c);
				setMap(pos, m);
	...
	public int jumpRight(int red, int pos, boolean imRed) {
		...
		boolean vt = ((imRed && (t == WHITEPAWN || t == WHITEKING)) || (!imRed && (t == REDPAWN || t == REDKING)));
		char t2 = getMap(e2);
		if (t2 == EMPTY && vt) {
			return e2;
		} else {
	...
	public String toString() {
		...
		for (int a = 1; a < 33; a++) {
			...
			if (a == 32) {
				s += BLOCK + " " + getScore(nextMove)
						+ (nextMove == RED ? "RED" : "WHITE") + "\n";
			}
		}
		return s;
	}
...
/*
 * red player1 (1-12) ie (12 peices each) white player2 (21-32) king crown ends
