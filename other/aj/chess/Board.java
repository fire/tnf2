...
public class Board {
	...
	public static char BLACKPAWN = 'p', BLACKKING = 'k', BLACKKNIGHT = 'h',
			BLACKBISHOP = 'b', BLACKROOK = 'r', BLACKQUEEN = 'q';

	public static char WHITEPAWN = 'P', WHITEKING = 'K', WHITEKNIGHT = 'H',
			WHITEBISHOP = 'B', WHITEROOK = 'R', WHITEQUEEN = 'Q';
		...
		for (int a = 0; a < 8; a++) {
			for (int b = 0; b < 8; b++) {
				...
				if (c.toUpperCase().equals("P"))
					tot += (isWhite(c) ? -1 : 1) * 10;
				if (c.toUpperCase().equals("B"))
					tot += (isWhite(c) ? -1 : 1) * 100;
				if (c.toUpperCase().equals("H"))
					tot += (isWhite(c) ? -1 : 1) * 120;
				if (c.toUpperCase().equals("R"))
					tot += (isWhite(c) ? -1 : 1) * 150;
				if (c.toUpperCase().equals("Q"))
					tot += (isWhite(c) ? -1 : 1) * 650;
				if (c.toUpperCase().equals("K"))
					tot += (isWhite(c) ? -1 : 1) * 1000;
			}
		}
		return tot;
	}
	...
	public Board() {
		...
		nextToMove = WHITE;
		if (nextToMove == WHITE) {
			friend = blackfriend.toUpperCase();
			targ = blackfriend;
		}
		if (nextToMove == BLACK) {
			friend = blackfriend;
			targ = blackfriend.toUpperCase();
		}
	}
		...
		for (int a = 0; a < allMoves.size(); a++) {
			...
			for (int c = 0; c < b.allMoves.size(); c++) {
				...
				if (m2.toUpperCase().endsWith("K")) {
					// System.out.println("move "+m+" creates invalid state
					// "+m2+" removing");
	...
	public Vector getAllMoves(int pos, String c) {
		...
		if (c.toUpperCase().equals("R")) {
			...
			int count = 1;
			while (!done) {
				if (pos / 8 > count - 1 && isOpenAt(pos - 8 * count))
					v.addElement(pos + ":" + (pos - 8 * count));
				if (pos / 8 > count - 1 && isTargetAt(pos - 8 * count)) {
					v.addElement(pos + ":" + (pos - 8 * count) + ":"
							+ (pos - 8 * count));
					done = true;
				}
				if (pos / 8 > count - 1 && isFriendlyAt(pos - 8 * count))
					done = true;
				if (pos - 8 * count < 0 || pos - 8 * count > 64)
					done = true;
				count++;
			}
			count = 1;
			...
			while (!done) {
				if (pos % 8 > count - 1 && isOpenAt(pos - 1 * count))
					v.addElement(pos + ":" + (pos - 1 * count));
				if (pos % 8 > count - 1 && isTargetAt(pos - 1 * count)) {
					v.addElement(pos + ":" + (pos - 1 * count) + ":"
							+ getMap(pos - 1 * count));
					done = true;
				}
				if (pos % 8 > count - 1 && isFriendlyAt(pos - 1 * count))
					done = true;
				if (pos - 1 * count < 0 || pos - 1 * count > 64)
					done = true;
				count++;
			}
			count = 1;
			...
			while (!done) {
				if (pos / 8 < 8 - count && isOpenAt(pos + 8 * count))
					v.addElement(pos + ":" + (pos + 8 * count));
				...
				if (pos / 8 < 8 - count && isFriendlyAt(pos + 8 * count))
					done = true;
				if (pos + 8 * count < 0 || pos + 8 * count > 64)
					done = true;
				count++;
			...
			while (!done) {
				if (pos % 8 < 8 - count && isOpenAt(pos + 1 * count))
					v.addElement(pos + ":" + (pos + 1 * count));
				...
				if (pos % 8 < 8 - count && isFriendlyAt(pos + 1 * count))
					done = true;
				if (pos + 1 * count < 0 || pos + 1 * count > 64)
					done = true;
				count++;
		...
		if (c.toUpperCase().equals("B")) {
			...
			int count = 1;
			while (!done) {
				if (pos / 8 > count - 1 && pos % 8 > count - 1
						&& isOpenAt(pos - 8 * count - 1 * count))
					v.addElement(pos + ":" + (pos - 8 * count - 1 * count));
				if (pos / 8 > count - 1 && pos % 8 > count - 1
						&& isTargetAt(pos - 8 * count - 1 * count)) {
					v.addElement(pos + ":" + (pos - 8 * count - 1 * count)
							+ ":" + getMap(pos - 8 * count - 1 * count));
					done = true;
				}
				if (pos / 8 > count - 1 && pos % 8 > count - 1
						&& isFriendlyAt(pos - 8 * count - 1 * count))
					done = true;
				if (pos - 8 * count - 1 * count < 0
						|| pos - 8 * count - 1 * count > 64)
					done = true;
				count++;
			}
			count = 1;
			...
			while (!done) {
				if (pos / 8 < 8 - count && pos % 8 > count - 1
						&& isOpenAt(pos + 8 * count - 1 * count))
					v.addElement(pos + ":" + (pos + 8 * count - 1 * count));
				if (pos / 8 < 8 - count && pos % 8 > count - 1
						&& isTargetAt(pos + 8 * count - 1 * count)) {
			...
			while (!done) {
				if (pos / 8 < 8 - count && pos % 8 < 8 - count
						&& isOpenAt(pos + 8 * count + 1 * count))
					v.addElement(pos + ":" + (pos + 8 * count + 1 * count));
				if (pos / 8 < 8 - count && pos % 8 < 8 - count
						&& isTargetAt(pos + 8 * count + 1 * count)) {
			...
			while (!done) {
				if (pos / 8 > count - 1 && pos % 8 < 8 - count
						&& isOpenAt(pos - 8 * count + 1 * count))
					v.addElement(pos + ":" + (pos - 8 * count + 1 * count));
				if (pos / 8 > count - 1 && pos % 8 < 8 - count
						&& isTargetAt(pos - 8 * count + 1 * count)) {
		...
		if (c.toUpperCase().equals("K")) {
			if (pos / 8 > 0 && isOpenAt(pos - 8))
				v.addElement(pos + ":" + (pos - 8));
			if (pos / 8 > 0 && pos % 8 > 0 && isOpenAt(pos - 8 - 1))
				v.addElement(pos + ":" + (pos - 8 - 1));
			if (pos % 8 > 0 && isOpenAt(pos - 1))
				v.addElement(pos + ":" + (pos - 1));
			if (pos / 8 < 7 && pos % 8 > 0 && isOpenAt(pos + 8 - 1))
				v.addElement(pos + ":" + (pos + 8 - 1));
			if (pos / 8 < 7 && isOpenAt(pos + 8))
				v.addElement(pos + ":" + (pos + 8));
			if (pos / 8 < 7 && pos % 8 < 7 && isOpenAt(pos + 8 + 1))
				v.addElement(pos + ":" + (pos + 8 + 1));
			if (pos % 8 < 7 && isOpenAt(pos + 1))
				v.addElement(pos + ":" + (pos + 1));
			if (pos / 8 > 0 && pos % 8 < 7 && isOpenAt(pos - 8 + 1))
				v.addElement(pos + ":" + (pos - 8 + 1));

			if (pos / 8 > 0 && isTargetAt(pos - 8))
				v.addElement(pos + ":" + (pos - 8) + ":" + getMap(pos - 8));
			if (pos / 8 > 0 && pos % 8 > 0 && isTargetAt(pos - 8 - 1))
				v.addElement(pos + ":" + (pos - 8 - 1) + ":"
						+ getMap(pos - 8 - 1));
			if (pos % 8 > 0 && isTargetAt(pos - 1))
				v.addElement(pos + ":" + (pos - 1) + ":" + getMap(pos - 1));
			if (pos / 8 < 7 && pos % 8 > 0 && isTargetAt(pos + 8 - 1))
				v.addElement(pos + ":" + (pos + 8 - 1) + ":"
						+ getMap(pos + 8 - 1));
			if (pos / 8 < 7 && isTargetAt(pos + 8))
				v.addElement(pos + ":" + (pos + 8) + ":" + getMap(pos + 8));
			if (pos / 8 < 7 && pos % 8 < 7 && isTargetAt(pos + 8 + 1))
				v.addElement(pos + ":" + (pos + 8 + 1) + ":"
						+ getMap(pos + 8 + 1));
			if (pos % 8 < 7 && isTargetAt(pos + 1))
				v.addElement(pos + ":" + (pos + 1) + ":" + getMap(pos + 1));
			if (pos / 8 > 0 && pos % 8 < 7 && isTargetAt(pos - 8 + 1))
				v.addElement(pos + ":" + (pos - 8 + 1) + ":"
						+ getMap(pos - 8 + 1));
		}
		if (c.toUpperCase().equals("H")) {
			if (pos / 8 > 0) {
				if (pos % 8 > 1 && isOpenAt(pos - 8 - 2))
					v.addElement(pos + ":" + (pos - 8 - 2));
				if (pos % 8 > 1 && isTargetAt(pos - 8 - 2))
					v.addElement(pos + ":" + (pos - 8 - 2) + ":"
							+ getMap(pos - 8 - 2));
				if (pos % 8 < 6 && isOpenAt(pos - 8 + 2))
					v.addElement(pos + ":" + (pos - 8 + 2));
				if (pos % 8 < 6 && isTargetAt(pos - 8 + 2))
					v.addElement(pos + ":" + (pos - 8 + 2) + ":"
							+ getMap(pos - 8 + 2));
			}
			if (pos / 8 > 1) {
				if (pos % 8 > 0 && isOpenAt(pos - 16 - 1))
					v.addElement(pos + ":" + (pos - 16 - 1));
				if (pos % 8 > 0 && isTargetAt(pos - 16 - 1))
					v.addElement(pos + ":" + (pos - 16 - 1) + ":"
							+ getMap(pos - 16 - 1));
				if (pos % 8 < 7 && isOpenAt(pos - 16 + 1))
					v.addElement(pos + ":" + (pos - 16 + 1));
				if (pos % 8 < 7 && isTargetAt(pos - 16 + 1))
					v.addElement(pos + ":" + (pos - 16 + 1) + ":"
							+ getMap(pos - 16 + 1));
			}
			if (pos / 8 < 7) {
		...
		if (c.toUpperCase().equals("P")) {
			if (pos / 8 > 0 && isWhite(c)) {
				if (pos % 8 > 0 && isTargetAt(pos - 9))
					v.addElement(pos + ":" + (pos - 9) + ":" + getMap(pos - 9));
				if (pos % 8 < 7 && isTargetAt(pos - 7))
					v.addElement(pos + ":" + (pos - 7) + ":" + getMap(pos - 7));
				if (isOpenAt(pos - 8))
					v.addElement(pos + ":" + (pos - 8));
				if (isOpenAt(pos - 16) && isOpenAt(pos - 8) && pos / 8 == 6)
					v.addElement(pos + ":" + (pos - 16));
			} else if (pos / 8 < 7 && !isWhite(c)) {
				if (pos % 8 > 0 && isTargetAt(pos + 7))
					v.addElement(pos + ":" + (pos + 7) + ":" + getMap(pos + 7));
				if (pos % 8 < 7 && isTargetAt(pos + 9))
					v.addElement(pos + ":" + (pos + 9) + ":" + getMap(pos + 9));
				if (isOpenAt(pos + 8))
					v.addElement(pos + ":" + (pos + 8));
				if (isOpenAt(pos + 16) && isOpenAt(pos + 8) && pos / 8 == 1)
					v.addElement(pos + ":" + (pos + 16));
			}
		}
		return v;
	...
	public static Board newBoard() {
		char m[][] = {
				...
				{ WHITEROOK, WHITEKNIGHT, WHITEBISHOP, WHITEKING, WHITEQUEEN,
