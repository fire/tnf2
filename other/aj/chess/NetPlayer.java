...
public class NetPlayer implements ActionListener {
	...
	public NetPlayer(String host, int port, String gname, String brain)
	...
	public synchronized void actionPerformed(String s) {
		if (s.startsWith("__")) {
			...
			if (b.getAllMoves().contains(s)) {
				...
				System.out
						.println("MyError: invalid move detected. restart connection");
				send("START");
				begin();
			}
		}
	...
	public void myApply(String s) {
		...
		// System.out.println("next to move
		// ="+(b.getNextMove()==b.WHITE?"WHITE":"BLACK"));
		if (s == null) {
		...
		b = b.applyMove(s);
		b.getAllMoves();
		// System.out.println("new next to move
		// ="+(b.getNextMove()==b.WHITE?"WHITE":"BLACK"));
	}
	...
	public void makeMove() {
		...
		if (human != null) {
			...
			String m = null;
			if (v.size() != 0)
				m = (String) (v.elementAt((int) (Math.random() * v.size())));
