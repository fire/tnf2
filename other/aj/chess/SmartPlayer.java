...
public class SmartPlayer {
	...
	public SmartPlayer(String host, int port, String gname)
	...
	public void makeMove() {
		...
		while (!checkTimeOut(timeout)) {
			...
			if (!n.equals(m)) {
				m = n;
				System.out
						.println("best move changed " + m + " level=" + level);
			}
		}
		b = b.applyMove(m);
		System.out.println("t=" + (System.currentTimeMillis() - timeout)
				+ " lv=" + level + " move=" + moves + " " + m);
		send(m);
	}
	...
	public static void main(String s[]) {
		if (s.length == 0) {
			System.out
					.println("FORMAT: java SmartPlayer <host> <port> <gamename> [-vw verbose wait]");
			System.exit(0);
		}
		// if (s.length==0) {
		// char m[][]={ {' ',' ',' ','r'},
