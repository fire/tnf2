...
class Mage implements ListSelectionListener, ActionListener {
	...
	JTextArea castingDetails = new JTextArea(12, 33),
			spellDetails = new JTextArea(12, 33);
	...
	public String getOrder(boolean right) {
		...
		while (o.indexOf("TARGET " + ref) >= 0) {
			...
			o = o.substring(0, o.indexOf(ch))
					+ o.substring(o.indexOf(ch) + ch.length()).trim();
		}
		...
		while (o.indexOf(";;") >= 0) {
			o = o.substring(0, o.indexOf(";;"))
					+ o.substring(o.indexOf(";;") + ";;".length()).trim();
		}
		while (o.indexOf(ref) >= 0) {
			h = o.substring(o.indexOf(ref));
			if (h.indexOf("\n") >= 0)
				h = h.substring(0, h.indexOf("\n")).trim();
			o = o.substring(0, o.indexOf(h))
					+ o.substring(o.indexOf(h) + h.length()).trim();
		}
		return h;
	}

	public void replaceOrder(boolean right, String next, String choose,
			String comment) {
		String o = orders.getText();
		...
		o = o.substring(0, o.indexOf(old)) + rep
				+ o.substring(o.indexOf(old) + old.length());
		orders.setText(o);
		...
		if (comment != null && old != null && !old.toUpperCase().equals(old)
				&& old.indexOf(next) < 0) {
	...
	public void addOrders(boolean right, Spell2 sp) {
		...
		if (sp.getStepsFrom(curr) != 1
				&& sp.includes(curr, sp.getRemainingGest(curr)).size() > 0) {
			...
			for (int a = 0; a < v.size(); a++) {
				...
				if (sp2.getStepsFrom(curr) == 1) {
					...
					if (comment.indexOf("NOW") < 0)
						comment += "NOW ";
					comment += " " + sp2.getCleanName() + " ";
					if (comment.indexOf("LATER") < 0)
						comment += " LATER " + sp.getCleanName() + ";";
				} else if (sp2.getStepsFrom(curr) > 1) {
					choiceName = null;
					if (comment.indexOf("FIRST") < 0)
						comment += "FIRST ";
					comment += sp2.getCleanName() + " ";
					if (comment.indexOf("LATER") < 0)
						comment += " LATER " + sp.getCleanName() + ";";
	...
	/*
	 * public void addOrders(boolean right,Spell2 sp) { String
	 * o=orders.getText(); if (o.indexOf("\nEND")>=0) {
	 * o=o.substring(0,o.indexOf("\nEND")); } if (o.indexOf("\n\n")>=0) {
	 * o=o.substring(0,o.indexOf("\n\n"))+o.substring(o.indexOf("\n\n")+1); }
	 * String h="",ch="";
	 * 
	 * String next=sp.gest.substring(0,1); if (right) { while (o.indexOf("CHOICE
	 * RH ")>=0) { ch=o.substring(o.indexOf("CHOICE RH ")); if
	 * (ch.indexOf("\n")>=0) ch=ch.substring(0,ch.indexOf("\n")).trim();
	 * o=o.substring(0,o.indexOf(ch))+o.substring(o.indexOf(ch)+ch.length()).trim(); }
	 * while (o.indexOf("RH ")>=0) { h=o.substring(o.indexOf("RH ")); if
	 * (h.indexOf("\n")>=0) h=h.substring(0,h.indexOf("\n")).trim();
	 * o=o.substring(0,o.indexOf(h))+o.substring(o.indexOf(h)+h.length()).trim(); }
	 * String curr=currRH.getText(); for (int a=0;a<sp.gest.length()-1;a++) {
	 * String t=curr+sp.gest.substring(sp.gest.length()-a); if
	 * (sp.getStepsFrom(t)==0) {
	 * next=sp.gest.substring(sp.gest.length()-a).substring(0,1); } } o+="\nRH
	 * "+next; orders.setText(o+"\nEND\n"); if (sp.getStepsFrom(curr)==1) { if
	 * (sp.oneHand() || o.indexOf(sp.getCleanName())<0) o+="\nCHOICE RH
	 * "+sp.getCleanName()+""; } if (o.indexOf("RH P")>=0 && o.indexOf("LH
	 * P")>=0) o+=" WARNING *p* "; orders.setText(o+"\nEND\n"); if
	 * (!next.toUpperCase().equals(next) && o.indexOf((!right?"R":"L")+"H
	 * "+next)<0) addOrders(!right,sp); } else { while (o.indexOf("CHOICE LH
	 * ")>=0) { ch=o.substring(o.indexOf("CHOICE LH ")); if
	 * (ch.indexOf("\n")>=0) ch=ch.substring(0,ch.indexOf("\n")).trim();
	 * o=o.substring(0,o.indexOf(ch))+o.substring(o.indexOf(ch)+ch.length()).trim(); }
	 * while (o.indexOf("LH ")>=0) { h=o.substring(o.indexOf("LH ")); if
	 * (h.indexOf("\n")>=0) h=h.substring(0,h.indexOf("\n")).trim();
	 * o=o.substring(0,o.indexOf(h))+o.substring(o.indexOf(h)+h.length()).trim(); }
	 * String curr=currLH.getText(); for (int a=0;a<sp.gest.length()-1;a++) {
	 * String t=curr+sp.gest.substring(sp.gest.length()-a); if
	 * (sp.getStepsFrom(t)==0)
	 * next=sp.gest.substring(sp.gest.length()-a).substring(0,1); } o+="\nLH
	 * "+next; orders.setText(o+"\nEND\n"); if (sp.getStepsFrom(curr)==1) { if
