...
public class Spell2 {
	...
	public static Spell2 parse(GmlPair g) {
		...
		if (di != null)
			sp.setDiscription(di.getString());
		return sp;
	}
	...
	public Vector choice(String curr) {
		...
		for (int a = 0; a < Tool2.allSpells.size(); a++) {
			Spell2 sp = (Spell2) Tool2.allSpells.elementAt(a);
			if (sp.gest.equals("p"))
				continue;// skip surrender as choice
			if (sp.getStepsFrom(curr + gest) == 0 && !res.contains(sp)) {
				// if (t.lastIndexOf(sp.gest)>curr.length() &&
				// t.endsWith(sp.gest) && !res.contains(sp)) {
				res.addElement(sp);
			}
		}
		return res;
	...
	public Vector includes(String curr, String rem) {
		...
		for (int a = 0; a < Tool2.allSpells.size(); a++) {
			Spell2 sp = (Spell2) Tool2.allSpells.elementAt(a);
			if (sp == this || sp.gest.toUpperCase().equals("P"))
				continue;// skip surrender as inlcude
			// if (gest.toUpperCase().indexOf(sp.gest.toUpperCase())>=0 &&
			// gest.toUpperCase().indexOf(sp.gest.toUpperCase())<gest.length()-1
			// && !res.contains(sp)) {
			// res.addElement(sp);
			// }
			if (t.lastIndexOf(sp.gest.toUpperCase()) + sp.gest.length() > Math
					.max(curr.length(), 0)
					&& !t.endsWith(sp.gest.toUpperCase()) && !res.contains(sp)) {
				res.addElement(sp);
			}
		}
		return res;
	...
	public String summaryString(String curr) {
		...
		String res = next + " " + getStepsFrom(curr) + "," + getStepsFrom("")
				+ ":" + toString() + " i="
				+ includes(curr, getRemainingGest(curr)).size() + " hs=";
 ...
 * 
 * Left: BSFWPPC Right: BDSFFFC
 * 
 * Peltor (Flandar) HP: 14 State: thinking (orders) Sick(5)
 * 
 * Left: BPSF>CF Right: B>>>CCW
 * 
 * ------ Game # Turn #
 * 
 * Dayron (Cmt) LH:BSFWPPC RH:BDSFFFC
 * 
 * Peltor (Flandar) LH:BPSF>CF RH:B>>>CCW
 * 
 * --- mage <name list> mage <name list> option RH option RH *save if mymage
 * option LH option LH *save if mymage
 * 
 * ---- spell - <#steps>name gesture --- gesture color at choices P - black SP -
 * P red for can choose (SP or P) SPD - P blue for free (P)
 * 
 * 
 * myspells - sort alphebetically theyspells - sort by speed
 * 
 * alphebetically sort by gest = branches visible fastest sort by number of
 * turns = next incomming/outgoing sort by relation = relations = list of all
 * next spells that are started 3,2 and 1 steps
 * 
 * RH - spell (name) (turns) (gest) (relations) LH - spell (name) (turns) (gest)
 * (relations)
 * 
 * 
 * Desiese - 7 - DFPSWc - cWWS
 * 
 * color (gest) for free spells relatiosn show (spell info) for all next spells
 * --- send auto turn number auto check for PP auto CHOOSE auto TARGET auto
 * PARALIZE auto DIRECT?
 * 
 * 
 * -------------------------------- |GM TU | | | |mage rh spell_N_T_R | | lh | | | | |
 * --------------------------------
 * 
 * spell name steps to complete gesture head start relations include relations
 * 
 * 
 * 
 * CurrentGest [ ]
