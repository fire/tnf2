			...
			for (int a=0;a<allSpells.size();a++) {
				...
				if (sp.getStepsFrom(hand)==b) {
					...
					for (int c=0;c<l.size();c++) {
						res+="\tinc="+((Spell)l.elementAt(c)).summaryString(hand,allSpells)+"\n";
					}
					l=sp.choice(hand,allSpells);
					for (int c=0;c<l.size();c++) {
						res+="\tchoice="+((Spell)l.elementAt(c)).summaryString(hand,allSpells)+"\n";
					}
					...
					for (int d=0;d<10;d++) {
						for (int c=0;c<l.size();c++) {
							...
							if (hs.getStepsFrom(hand+sp.gest)==d) {
								res+="\ths="+((Spell)l.elementAt(c)).summaryString(sp.gest,allSpells)+"\n";
							}
						}
					}
				}
...
//caps check?
	...
	public String summaryString(String curr,Vector allSpells) {
		String res=getStepsFrom(curr)+","+getStepsFrom("")+":"+toString()+" i="+includes(curr,allSpells).size()+" hs=";
