...
public class test implements ActionListener{
	...
	public void guiSetup() {
		...
		for (int i = 0; i < 6; i++) { 
			...
			if (i == 1) { column.setPreferredWidth(10);} 
			else if (i == 2) { column.setPreferredWidth(10);} 
			else if (i == 3) { column.setPreferredWidth(10);} 
	...
	public static Vector getOptionsFor(String hand) {
		...
		for (int b=1;b<10;b++) {
			for (int a=0;a<Tool2.allSpells.size();a++) {
				Spell2 sp=(Spell2)Tool2.allSpells.elementAt(a);
				if (sp.getStepsFrom(hand)==b && b!=sp.gest.length() && !temp.contains(sp)) {
					res.addElement(sp);
					temp.addElement(sp);
				}
			}
