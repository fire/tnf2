...
//Standard Duel Status for Peltor (game 3723, turn 7):
		...
		if (all.indexOf(rawHeader)>=0) {
			...
			if (all.indexOf(head)>=0 && all.indexOf(tail)>=all.indexOf(head)) {
				...
				if (title.indexOf("turn ")>=0) {
					...
					if (turn.indexOf(")")>=0) turn=turn.substring(turn.indexOf(")")).trim();
				}
			}
		}
		...
		while (true) {
			...
			if (all.indexOf(right)>=0) {
				String h=all.substring(0,all.indexOf(right)+right.length());
	...
	public void readSpells() {
		try {
			...
			GmlPair all=GmlPair.parse(this.getClass().getResource("spells.gml"));
			...
			for (int a=0;a<gmlSpells.length;a++) {
				...
				if (sp.name.startsWith("descision")) continue;
	...
	public static Spell2 getSpellBySummary(String val){
		...
		for (int a=0;a<allSpells.size();a++) {
			...
			if (val.indexOf(sp.name)>=0 && val.indexOf(sp.gest)>=0) return sp;
		}
		return null;
	}
		...
		for (int d=1;d<10;d++) {
			for (int c=0;c<l.size();c++) {
				...
				if (hs.getStepsFrom(hand+sp.gest)==d) {
					res+="\ths="+((Spell2)l.elementAt(c)).summaryString(sp.gest)+"\n";
				}
			}
		}
		return res;
	...
	public static Vector getOptionsFor(String hand) {
		...
		for (int b=1;b<10;b++) {
			for (int a=0;a<allSpells.size();a++) {
				Spell2 sp=(Spell2)allSpells.elementAt(a);
				//if ((sp.getStepsFrom(hand)==b || sp.getGest().length()==b )&& !tmp.contains(sp)) {
