...
public class LoanCalc implements ActionListener {
	...
	public static double getRemain(double loan, double rate, double num,
			double pay) {
		for (int a = 0; a < num; a++) {
	...
	public void result(double l, double r, int n, double p) {
		...
		double totalint = 0;
		if (log)
			System.out.println("loan = " + aj.misc.Stuff.money(l, 2) + " int ="
					+ aj.misc.Stuff.trunc(r, 4) + " payment = "
					+ aj.misc.Stuff.money(p, 2) + " term =" + n);
		for (int a = 0; a < n; a++) {
			totalprin += p - (l * r / 12);
			totalint += l * r / 12;
			l = l + l * r / 12 - p;
			if (log)
				System.out.println("payment #" + (a + 1) + " loan ballance"
						+ aj.misc.Stuff.money(l, 2) + " principal "
						+ aj.misc.Stuff.money(p - (l * r / 12), 2) + "("
						+ aj.misc.Stuff.money(totalprin, 2) + ") int="
						+ aj.misc.Stuff.money(l * r / 12, 2) + "("
						+ aj.misc.Stuff.money(totalint, 2) + ")");
		}
	}
	/*
	 * principal &interest access loan morgage insurance 76 home owners
	 * associations 100
	 */
}
