...
public class Node implements DisplayItem {
	...
	public void setColor(GmlPair g) {
		...
		color=g;
		if (g.isString() && g.getString().equalsIgnoreCase("RED")) cc=Color.red;
		else if (g.isString() && g.getString().equalsIgnoreCase("BLUE")) cc=Color.blue;
		else if (g.isString() && g.getString().equalsIgnoreCase("Green")) cc=Color.green;
		else if (g.isString() && g.getString().equalsIgnoreCase("Yellow")) cc=Color.yellow;
		else if (g.isString() && g.getString().equalsIgnoreCase("Cyan")) cc=Color.cyan;
		else if (g.isString() && g.getString().equalsIgnoreCase("Magenta")) cc=Color.magenta;
		else if (g.isString() && g.getString().equalsIgnoreCase("gray")) cc=Color.gray;
		else if (g.isString() && g.getString().equalsIgnoreCase("lightgray")) cc=Color.lightGray;
		else if (g.isString() && g.getString().equalsIgnoreCase("Darkgray")) cc=Color.darkGray;
		else if (g.isList()) {
			...
			if (r==null || gg==null || b==null) {
				...
				cc=new Color((int)(r.getDouble()),(int)(gg.getDouble()),(int)(b.getDouble()));
			}
		}
		else {
			cc=Color.black;
	...
	public GmlPair toGmlPair() {
		...
		if (color.isList() || !color.getString().equalsIgnoreCase("Black")) v.addElement (color);
	...
	public static Node parse(GmlPair g) {
		if (!g.getName().equalsIgnoreCase("NODE")) return null;
		String na=null;
		int id=-1;
		...
		if (id==-1) node=new Node(na);
		else node=new Node(na,id);
		n=g.getOneByName("X");
		if (n!=null) node.setX(n.getDouble());
		n=g.getOneByName("Y");
		if (n!=null) node.setY(n.getDouble());
		n=g.getOneByName("color");
		if (n!=null) node.setColor(n);
		if (n==null) node.setColor(null);
