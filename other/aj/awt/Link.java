...
public class Link implements DisplayItem {
	...
	public void display(Graphics g, double s) {
		...
		if (from.size() == 0 || to.size() == 0) {
			System.out.println("MyError: bad link nodes not found.  from:"
					+ from + " to:" + to);
		}
		...
		for (int a = 0; a < from.size(); a++) {
			for (int b = 0; b < to.size(); b++) {
				...
				if (f == t)
					continue;
				g.drawLine((int) f.getX(s), (int) f.getY(s), (int) t.getX(s),
						(int) t.getY(s));
				Arrow A = new Arrow((int) f.getX(s), (int) f.getY(s), (int) t
						.getX(s), (int) t.getY(s));
				A.display(g, 1);
			}
		}
	...
	public static Link parse(GmlPair g, Vector nodes) {
		if (!g.getName().equalsIgnoreCase("LINK"))
			return null;
		...
		if (cgml != null && cgml.isList()) {
			...
			if (r == null || gr == null || b == null)
				color = Color.black;
			else {
				color = new Color((int) (r.getDouble()) % 256, (int) (gr
						.getDouble()) % 256, (int) (b.getDouble()) % 256);
			}
		} else if (cgml != null) {
			String cna = cgml.getString();
			if (cna.equalsIgnoreCase("BLACK"))
				color = Color.black;
			else if (cna.equalsIgnoreCase("WHITE"))
				color = Color.white;
			else if (cna.equalsIgnoreCase("red"))
				color = Color.red;
			else if (cna.equalsIgnoreCase("blue"))
				color = Color.blue;
			else if (cna.equalsIgnoreCase("green"))
				color = Color.green;
			else if (cna.equalsIgnoreCase("yellow"))
				color = Color.yellow;
			else if (cna.equalsIgnoreCase("cyan"))
				color = Color.cyan;
			else
				color = Color.black;
		}
		for (int a = 0; a < fngml.length; a++) {
			...
			for (int c = 0; c < nlist.length; c++) {
				for (int b = 0; b < nodes.size(); b++) {
					...
					if (b + 1 == nodes.size())
						System.out
								.println("MyError: No fromnode found with id "
										+ nlist[c] + ".  line =" + fngml[a]);
				}
			}
		}
		for (int a = 0; a < tngml.length; a++) {
			...
			for (int c = 0; c < nlist.length; c++) {
				for (int b = 0; b < nodes.size(); b++) {
					...
					if (b + 1 == nodes.size())
						System.out.println("MyError: No tonode found with id "
								+ nlist[c] + ".  line =" + tngml[a]);
				}
			}
		}
	...
	public static void main(String s[]) {
		...
		try {
			...
			System.out.println(g + " = " + l);
			g = GmlPair
					.parse("Link [fromnode \"test,test2\" tonode \"test3,test2\" note test]");
			l = Link.parse(g, nodes);
			System.out.println(g + " = " + l);
