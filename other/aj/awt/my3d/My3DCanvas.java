...
public class My3DCanvas extends Canvas {
	...
	public void paint(Graphics g) {
		...
		// unit(v3,cross(v1,v2));
		Matrix t = new Matrix(v1.val[0], v1.val[1], v1.val[2], v2.val[0],
				v2.val[1], v2.val[2], v3.val[0], v3.val[1], v3.val[2]);
		m = Matrix.mul(t, m);
		...
		d = Math
				.max(Math.abs(vrp.val[2] - vp.val[2]), Math.max(Math
						.abs(vrp.val[1] - vp.val[1]), Math.abs(vrp.val[0]
						- vp.val[0])));

		v1 = new MyVector(1.0 * size / d / 2, 1.0 * size / d / 2, 1.0 * size
				/ d / 2);
	...
	public void drawPoint(Graphics g, Point3d p, Color c) {
		// check z buffer
		// get lighting effect
		g.drawLine((int) (p.point.val[0]), (int) (p.point.val[1]),
				(int) (p.point.val[0]), (int) (p.point.val[1]));
	}
		...
		try {
			...
			for (int a = 0; a < n.length; a++) {
				...
				for (int b = 0; b < p.length; b++) {
					...
					points.addElement(new Point3d(new MyVector(x, y, z),
							new MyVector(nx, ny, nz)));
				}
				Color c = Color.gray;
