 ...
 */
public class Mand extends Canvas implements MouseListener, MouseMotionListener,
		ActionListener {
	/**
	 * 
	 */
	...
	int defaultSize = 200, defaultFocus = 1, defaultDepth = 500,
			defaultColors = 256;

	int size = defaultSize, height = size, width = size,
			iterationLimit = defaultDepth, currentFocusLimit = defaultFocus;

	double currentRealMin = -2.0, currentRealMax = 0.50,
			currentImagMin = -1.25, currentImagMax = 1.25;
	...
	TextField tfFocus = new TextField(4), tfDepth = new TextField(4),
			tfSize = new TextField(5), tfColors = new TextField(4);

	TextField tfRMin = new TextField(12), tfRMax = new TextField(12),
			tfIMin = new TextField(12), tfIMax = new TextField(12);
	...
	public String getGml() {
		return "mand [ " + " minx \"" + currentRealMin + "\"" + " maxx \""
				+ currentRealMax + "\"" + " miny \"" + currentImagMin + "\""
				+ " maxy \"" + currentImagMax + "\"" + " depth "
				+ iterationLimit + " colors " + maxColors + " centerx \""
				+ (currentRealMin + currentRealMax) / 2 + "\"" + " centery \""
				+ (currentImagMin + currentImagMax) / 2 + "\"" + " widthx \""
				+ Math.abs(currentRealMin - currentRealMax) + "\""
				+ " widthy \"" + Math.abs(currentImagMin - currentImagMax)
				+ "\"" + " ]";
	}
	...
	public void loadFile(String s) {
		try {
			...
			if (minx != null)
				currentRealMin = minx.getDouble();
			if (maxx != null)
				currentRealMax = maxx.getDouble();
			if (miny != null)
				currentImagMin = miny.getDouble();
			if (maxy != null)
				currentImagMax = maxy.getDouble();
			if (fcolors != null)
				maxColors = (int) (fcolors.getDouble());
			if (fdepth != null)
				iterationLimit = (int) (fdepth.getDouble());
	...
	public void setDefaultPosition() {
		setAll(size, currentFocusLimit, iterationLimit, maxColors, -2.0, 0.50,
				-1.25, 1.25);
	}

	public void setAll(int i, int j, int k, int col, double minx, double maxx,
	...
	public void mouseReleased(MouseEvent mouseEvent) {
		...
		currentRealMax = currentRealMax - (double) (width - Math.max(Sx, Ex))
				* d1;
		currentRealMin = currentRealMin + (double) Math.min(Sx, Ex) * d1;
		currentImagMax = currentImagMax - (double) (height - Math.max(Sy, Ey))
				* d2;
	...
	public void paint(Graphics g) {
		if (mustRedraw) {
			...
			g.drawImage(I, 0, 0, width, height, Color.red, this);
	}
	...
	public void recalc() {
		...
		updateThread = new Thread() {
			...
			public void run() {
				...
				double realInc = (currentRealMax - currentRealMin) * 1.0
						/ width;
				double imagInc = (currentImagMax - currentImagMin) * 1.0
						/ height;
				int x;
				int y;
				double Nx;
				...
				for (localFocus = 128; localFocus > localFocusLimit; localFocus /= 2) {
					...
					for (x = 0, Nx = currentRealMin; x < width; x += localFocus, Nx += xstep) {
						...
						int amtdon = (int) ((currwork * currval + lastval)
								/ doneval * 100);
					...
					TXstatus.setText("100% Done "
							+ aj.misc.Stuff.trunc(elapstime / 1000.0, 2)
							+ " secs");
				}
			}
		};
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		if (actionEvent.getSource() == tfRMin
				|| actionEvent.getSource() == tfRMax
				|| actionEvent.getSource() == tfIMin
				|| actionEvent.getSource() == tfIMax) {
			try {
