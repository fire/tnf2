...
public class Graph extends ScrollZoomDisplayCanvas implements ActionListener {
	...
	public Graph(String s[]) {
		try {
			...
			for (int a = 0; a < n.length; a++) {
				...
				/*
				 * GmlPair nn[] = n[a].getAllByName ("name"); GmlPair ii[] =
				 * n[a].getAllByName ("id"); Node nnn = null; String name = "";
				 * if (nn.length > 0) { name = nn[0].getString(); } if
				 * (ii.length > 0) { nnn = new Node (name,
				 * (int)ii[0].getDouble()); } else { nnn = new Node (name); } nn =
				 * n[a].getAllByName ("x"); if (nn.length > 0) { nnn.setX ((int)
				 * (nn[0].getDouble())); } nn = n[a].getAllByName ("y"); if
				 * (nn.length > 0) { nnn.setY ((int) (nn[0].getDouble())); }
				 * nodes.addElement (nnn); add (nnn);
				 */
			}
			...
			for (int a = 0; a < l.length; a++) {
				...
				/*
				 * Link lll = makeLink (l[a]); if (lll==null) continue;
				 * links.addElement (lll); add (lll);
				 */
			}
	...
	public Link makeLink(GmlPair l) {
		...
		if (cc.length > 0) {
			if (cc[0].isList()) {
				...
				C = new Color((int) (r.getDouble()), (int) (g.getDouble()),
						(int) (bl.getDouble()));
			} else {
		...
		while (n.size() > 0) {
			for (int a = 0; a < l.size(); a++) {
				...
				for (int b = 0; b < child.size(); b++) {
					if (T.contains(child.elementAt(b)))
						continue;
	...
	public void layoutCircle() {
		...
		while (inner.size() > 0 && steps < 6) {
			...
			for (int a = 0; a < inner.size(); a++) {
				Node n = (Node) inner.elementAt(a);
				n.x = (int) (Math.cos(2 * Math.PI * a / inner.size() + ra) * radious)
						+ CENTER;
				n.y = (int) (Math.sin(2 * Math.PI * a / inner.size() + ra) * radious)
						+ CENTER;
			}
			inner = outer;
			outer = new Vector();
	...
	public static void main(String s[]) {
		if (s.length == 0) {
			...
			System.out
					.println("  -BYID save links with node ID instead of name");
			System.exit(0);
		}
		for (int a = 0; a < s.length; a++) {
			if (s[a].equalsIgnoreCase("-BYID"))
				Link.BYID = true;
		}
		mb.add(f);
		f.add(save);
