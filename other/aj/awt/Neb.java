...
public class Neb extends Canvas implements MouseListener {
	...
	public Neb (String s[]) {
		...
		try {
			if (s.length == 0) {
				System.out.println ("FORMAT: java aj.awt.Neb <size> <step> <ops> <1=v 2=h 3=vh 0=non> <seed>");
			}
			if (s.length > 0)screenSize = Integer.parseInt (s[0]);
			if (s.length > 1)step = Math.max (Integer.parseInt (s[1]), 2);
			if (s.length > 2)ops = Integer.parseInt (s[2]);
			if (s.length > 3) {
				symV = (Integer.parseInt (s[3]) & 1) == 1;
				symH = (Integer.parseInt (s[3]) & 2) == 2;
			}
			if (s.length > 4)R = new Random (Integer.parseInt (s[4]));
			if (s[4].equals ("0")) {
	...
	public static void build (Graphics G, Color c, int ss, int step, int ops, boolean symV, boolean symH, Random R) {
		int x = ss / 2, y = ss / 2;
		for (int a = 0; a < ops; a++) {
			G.setColor (c);
			G.drawLine (x, y, x, y);
			...
			double n1 = (R != null?R.nextDouble():Math.random()), n2 = (R != null?R.nextDouble():Math.random());
			x = x + (int) ((n1 * 2 - 1) * step);
			y = y + (int) ((n2 * 2 - 1) * step);
			if (x > ss)x = ss - x;
			if (y > ss)y = ss - y;
