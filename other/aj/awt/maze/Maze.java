...
public class Maze extends Canvas {
	...
	public Maze(String s[]) {
		if (s.length != 2) {
			System.out
					.println("FORMAT: java aj.awt.maze.Maze <width> <height> ");
			System.exit(0);
		}
		try {
	...
	public static char[][] makeMaze(int w, int h, boolean connect8) {
		...
		if (Math.random() * 2 > 1) {
			...
			if (Math.random() * 2 > 1) {
				...
				// bottem
				start = new Point((int) (Math.random() * (w / 2)) * 2 + 1,
						h - 2);
		...
		while (active.size() > 0) {
			// Point
			// p=(Point)active.elementAt((int)(Math.random()*active.size()));//chose
			// random active
			// Point
			// p=(Point)active.elementAt(0);//(int)(Math.random()*active.size()));//chose
			// random active
