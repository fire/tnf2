...
public class Dungeon {
	...
	public static void main(String[] args) {
		...
		double randomness = .5;
		Dungeon d = Dungeon.createDungeon(seed, new char[height][width],
				sparceness, deadendchance, randomness, numrooms, doorchange, 6,
				12, 4, 8);
	...
	private static int DEFAULT_MINROOMWIDTH = 7, DEFAULT_MAXROOMWIDTH = 13,
			DEFAULT_MINROOMHEIGHT = 5, DEFAULT_MAXROOMHEIGHT = 9,
			DEFAULT_SPARCENESS = 12;
	 ...
	 *            the random seed
	 * @param c
	 *            grid of chars to use Must be all SOLID or EMPTY
	 * @param sparceNess
	 *            number of steps to backup from deadends
	 * @param deadEndChance
	 *            chance to reconnect dead ends (0=leave all, 1=connect all)
	 * @param numRooms
	 *            number of rooms to add
	 * @param doorChance
	 *            chance to make door into room (0=no doors, 1=all doors)
	 * @param minroomwidth
	 * @param maxroomwidth
	 * @param minroomheight
	 * @param maxroomheight
	 * @return
	 */
	public static Dungeon createDungeon(int seed, char c[][], int sparceNess,
			double deadEndChance, double randomness, int numRooms,
			double doorChance, int minroomwidth, int maxroomwidth,
		...
		d.unwind(sparceNess);
		d.addRooms(numRooms, minroomwidth, maxroomwidth, minroomheight,
				maxroomheight);
	...
	private void validateGrid() {
		for (int a = 0; a < grid.length; a++) {
			for (int b = 0; b < grid[a].length; b++) {
				if (grid[a][b] != SOLID && grid[a][b] != EMPTY)
					grid[a][b] = SOLID;
			}
		}
	}
	...
	/**
	 * follow each visited square until all visited squares are complete and
	 * removed
	 * 
	 */
	...
	public Dungeon(int x, int y) {
		...
		Point v = new Point((int) (random.nextDouble() * x), (int) (random
				.nextDouble() * y));
		...
		removeDeadEnds(1);
		addRooms(10, DEFAULT_MINROOMWIDTH, DEFAULT_MAXROOMWIDTH,
				DEFAULT_MINROOMHEIGHT, DEFAULT_MAXROOMHEIGHT);
		addDoors(.9);
	 ...
	 * 
	 * @param d
	 *            chance to add door on wall not already connected
	 */
	private void addDoors(double d) {
		for (int a = 1; a < grid.length; a += 2) {
			for (int b = 1; b < grid[a].length; b += 2) {

				if (b > 0 && b < grid[a].length - 2 && grid[a][b] == EMPTY
						&& grid[a][b + 1] == EMPTY && grid[a][b + 2] == ROOM) {
					grid[a][b + 1] = DOOR;
				}
				if (b > 1 && b < grid[a].length - 1 && grid[a][b] == EMPTY
						&& grid[a][b - 1] == EMPTY && grid[a][b - 2] == ROOM) {
					grid[a][b - 1] = DOOR;
				}
				if (a > 0 && a < grid.length - 2 && grid[a][b] == EMPTY
						&& grid[a + 1][b] == EMPTY && grid[a + 2][b] == ROOM) {
					grid[a + 1][b] = DOOR;
				}
				if (a > 1 && a < grid.length - 1 && grid[a][b] == EMPTY
						&& grid[a - 1][b] == EMPTY && grid[a - 2][b] == ROOM) {
					grid[a - 1][b] = DOOR;
				}
				if (b > 0 && b < grid[a].length - 2 && grid[a][b] == ROOM
						&& grid[a][b + 1] == EMPTY && grid[a][b + 2] == ROOM) {
					grid[a][b + 1] = DOOR;
				}
				if (b > 1 && b < grid[a].length - 1 && grid[a][b] == ROOM
						&& grid[a][b - 1] == EMPTY && grid[a][b - 2] == ROOM) {
					grid[a][b - 1] = DOOR;
				}
				if (a > 0 && a < grid.length - 2 && grid[a][b] == ROOM
						&& grid[a + 1][b] == EMPTY && grid[a + 2][b] == ROOM) {
					grid[a + 1][b] = DOOR;
				}
				if (a > 1 && a < grid.length - 1 && grid[a][b] == ROOM
						&& grid[a - 1][b] == EMPTY && grid[a - 2][b] == ROOM) {
					grid[a - 1][b] = DOOR;
				}
			}
	...
	/**
	 * Returns score 0 being no connect, 999999 being out of bounds, lower
	 * number is better
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Finds all deadends and converts %percentDeadEndConnect back to other
	 * halls
	 * 
	...
	private void follow(Point p, double randomness) {
		if (p.y < 0 || p.x < 0 || p.y > grid.length - 1
				|| p.x > grid[p.y].length - 1)
			return;
		grid[p.y][p.x] = EMPTY;
		int count = 0;
		int dir = (int) (random.nextDouble() * 4);
