...
public class Maze3d extends Canvas {
	...
	public Maze3d(String s[]) {
		if (s.length != 3) {
			System.out
					.println("FORMAT: java aj.awt.maze.Maze3d <depth> <width> <height> ");
			System.exit(0);
		}
		try {
			maze = makeMaze(Integer.parseInt(s[0]), Integer.parseInt(s[1]),
					Integer.parseInt(s[2]), false);
			view(maze);
	...
	public static void view(char maz[][][]) {
		...
		for (int a = 0; a < maz.length; a++) {
			for (int b = 0; b < maz[a].length; b++) {
				for (int c = 0; c < maz[a][b].length; c++) {
					if (maz[a][b][c] == ' ' && a < maz.length - 1
							&& maz[a + 1][b][c] == ' ')
						System.out.println("Transform { translation " + c * 2.5
								+ " " + a * -2.5 + " " + b * 2.5
								+ " children [ USE MyOpenSlab ]},");
					else if (maz[a][b][c] == ' ')
						// if (maz[a][b][c]==' ')
						System.out.println("Transform { translation " + c * 2.5
								+ " " + a * -2.5 + " " + b * 2.5
	...
	/*
	 * starb x y z stare wallb x y z walle
	 * 
	 * xxxx xxxx = wall xxxx xxxx
	 * 
	 * xx xx xxxx = star xxxx
	 * 
	 * x x x
	 */
	...
	public static char[][][] makeMaze(int d, int w, int h, boolean connect8) {
		...
		while (active.size() > 0) {
			// Point3d
			// p=(Point3d)active.elementAt((int)(Math.random()*active.size()));//chose
			// random active
			// Point3d
			// p=(Point3d)active.elementAt(0);//(int)(Math.random()*active.size()));//chose
			// random active
			Point3d p = (Point3d) active.elementAt(active.size() - 1);
			// chose random active
			// System.out.println(active.size()+"to do. Working from "+p.x+"
			// "+p.y+ " "+p.z);
			...
			if (canMove(maz, p)) {
				...
				while (true) {
					if (dir == 0
							&& p.x > 0
							&& adjacent(maz, new Point3d(p.x - 1, p.y, p.z)) < 2
							&& maz[p.z][p.y][p.x - 1] != ' ') {
						maz[p.z][p.y][p.x - 1] = ' ';
					...
					if (dir == 2
							&& p.y > 0
							&& adjacent(maz, new Point3d(p.x, p.y - 1, p.z)) < 2
							&& maz[p.z][p.y - 1][p.x] != ' ') {
						maz[p.z][p.y - 1][p.x] = ' ';
					...
					if (dir == 4
							&& p.z > 0
							&& adjacent(maz, new Point3d(p.x, p.y, p.z - 1)) < 2
							&& maz[p.z - 1][p.y][p.x] != ' ') {
						maz[p.z - 1][p.y][p.x] = ' ';
				...
				// System.out.println("do move from "+p.x+" "+p.y+" "+p.z+" to
				// "+p3.x+" "+p3.y+" "+p3.z+" dir="+dir);
				// view (maz);
			} else {
	...
	public static boolean canMove(char[][][] maz, Point3d p) {
		...
		if (p.x > 0 && adjacent(maz, new Point3d(p.x - 1, p.y, p.z)) < 2
				&& maz[p.z][p.y][p.x - 1] != ' ') {
			return true;
		}
		if (p.x < maz[p.z][p.y].length - 1
				&& adjacent(maz, new Point3d(p.x + 1, p.y, p.z)) < 2
				&& maz[p.z][p.y][p.x + 1] != ' ') {
			return true;
		}
		if (p.y > 0 && adjacent(maz, new Point3d(p.x, p.y - 1, p.z)) < 2
				&& maz[p.z][p.y - 1][p.x] != ' ') {
			return true;
		}
		if (p.y < maz[p.z].length - 1
				&& adjacent(maz, new Point3d(p.x, p.y + 1, p.z)) < 2
				&& maz[p.z][p.y + 1][p.x] != ' ') {
			return true;
		}
		if (p.z > 0 && adjacent(maz, new Point3d(p.x, p.y, p.z - 1)) < 2
				&& maz[p.z - 1][p.y][p.x] != ' ') {
			return true;
		}
		if (p.z < maz.length - 1
...
/*
 * static char horz='-',vert='I',wall='H'; static stst1='>',st2='<',st3='/',st4='\\',st5='^',st6='V';
 * static char st[]={'H','I','-','>',']','<','[','V','v','^','`','/','\\'};
 * static char[][] follow={{'H^`Vv>]<['},{'I/\\>]<['},{'-\\/>]<['}, {'>]<IH'},{']H>[I'},{'<H>[I'},{'[<]HI'},
 * {'VvH-^'},{'vVH-V'},{'^`VH-'},{'`^vH-'}, {'/\\I-'},{'\\/I-'}};
 * 
 * 6 wall types (w or w/o door or window) 6 stair types (w or w/o rail)
 * 
