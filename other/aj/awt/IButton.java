...
/**
 * An Image button. Load image from file (gif or jpg only). Image button remains
 * porportional by default, and can have a fixed size.
 * 
 * @author judda
 * @created April 12, 2000
 */
public class IButton extends Component implements MouseListener,
		MouseMotionListener {
	/**
	 * 
	 */
	 ...
	 * 
	 * @param s
	 *            the location of the image or the text of the button if no such
	 ...
	 * 
	 * @param s
	 *            The new Image value. Either the location s or the text s if
	...
	public void paint(Graphics g) {
		...
		if (i != null) {
			g.drawImage(i, (pressed ? 1 : 0), (pressed ? 1 : 0), w, h,
					getBackground(), this);
			// use below line to disable animated gifs and jpgs
			// g.drawImage(i,(pressed ? 1 : 0),(pressed ? 1 :
			// 0),w,h,getBackground(),null);
		}
		g.setColor((!pressed ? getForeground() : getBackground()));
		g.drawLine(0, 0, w, 0);
		g.drawLine(0, 0, 0, h);
		g.setColor((!pressed ? getForeground().brighter().brighter().brighter()
	...
	public void mouseDragged(MouseEvent ME) {
		boolean inside = (ME.getX() - 0 > 0 && ME.getX() - 0 < width
				&& ME.getY() - 0 > 0 && ME.getY() - 0 < height);
	...
	public void mousePressed(MouseEvent ME) {
		if (ME.getX() - 0 > 0 && ME.getX() - 0 < width && ME.getY() - 0 > 0
				&& ME.getY() - 0 < height) {
			started = true;
			setPressed(true);
		}
