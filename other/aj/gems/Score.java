...
//		v.add("other");
	...
	public static String getRank(String gameName, String name, int score) {
		...
		for (int a=0;a<v.size();a++) {
			...
			found=a;
			if (i.intValue()>score) continue;
			break;
		}
		found+=1;
		String place=((found-1%10)==0?"st":"")+((found-2%10)==0?"nd":"")+((found-3%10)==0?"rd":"");
		if (place.equals("")) place="th";
		res+=found+place+" ";
		...
		found+=1;
		place=((found-1%10)==0?"st":"")+((found-2%10)==0?"nd":"")+((found-3%10)==0?"rd":"");
		if (place.equals("")) place="th";
		res+=found+place;
		return res;
	}
...
//			m.add(s.other);
			if (s.user.equalsIgnoreCase(name) && s.gameType.equals(gametype)) v.add(m);
		}
		v=sort(v,2);
		return v;
	}
...
//			m.add(s.gameType);
			...
			m.add(new Integer(s.cells));
//			m.add(s.other);
			if (s.gameType.equals(gametype)) v.add(m);
		}
		v=sort(v,2);
		return v;
	}
...
//	}
	...
	public static Vector getKnownUsers() {
		if (all==null) readFile();
		Vector v=new Vector();
		for (int a=0;a<all.size();a++) {
			Score s=(Score)all.elementAt(a);
			if (!v.contains(s.user)) v.addElement(s.user);
		}
		return v;
	}
	...
	public static void create(String user, long date, String gameType, int score, int time, int colors, int cells, String other) {
		if (all==null) readFile();
		Score s=new Score();
		s.user=user;
		s.date=date;
