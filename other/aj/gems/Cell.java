...
public class Cell {
	...
	public void paint(Graphics g) {
		...
		for (int a = 0; a < allColor.length; a++) {
			if (c == allColor[a]) {
				...
				switch (allShape[a]) {
				case 0:
					Generate.drawHex(g, c, s1, s1);
					break;
				// case 1: Generate.drawHex(g,c,s2,s1);break;
				case 1:
					Generate.drawHex(g, c, s1, s2);
					break;
				case 2:
					Generate.drawDiamond(g, c, s1, s1);
					break;
				// case 4: Generate.drawDiamond(g,c,s2,s1);break;
				case 3:
					Generate.drawDiamond(g, c, s1, s2);
					break;
				case 4:
					Generate.drawSquare(g, c, s1, s1);
					break;
				case 5:
					Generate.drawSquare(g, c, s2, s1);
					break;
				case 6:
					Generate.drawSquare(g, c, s1, s2);
					break;
				case 7:
					Generate.drawNavet(g, c, s1, s1);
					break;
				case 8:
					Generate.drawNavet(g, c, s2, s1);
					break;
				default:
					Generate.drawNavet(g, c, s1, s2);
				}
			}
		}
		// if (c==allColor[0])
		...
		if (sparkle > 0) {
			...
			g.drawLine((int) (drawx - SIZE / 2 + sparklex - sparkle % BLINK),
					(int) (drawy - SIZE / 2 + sparkley), (int) (drawx - SIZE
							/ 2 + sparklex + sparkle % BLINK), (int) (drawy
							- SIZE / 2 + sparkley));
			g.drawLine((int) (drawx - SIZE / 2 + sparklex), (int) (drawy - SIZE
					/ 2 + sparkley - sparkle % BLINK),
					(int) (drawx - SIZE / 2 + sparklex), (int) (drawy - SIZE
							/ 2 + sparkley + sparkle % BLINK));
		}
		// if (sparkle>-1) {
		// }
	...
	public void setMove(int dx, int dy, long dt) {
		if (startx != destx || starty != desty) {// !moving) {
			...
			destTime = System.currentTimeMillis() + dt
					+ (int) (400 * Math.random() - 200);
		}
		destx = dx * SIZE;
		desty = dy * SIZE;
	}
