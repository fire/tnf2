...
public class MathDrill {
  ...
  public static void main (String s[]) throws IOException {
    if (s.length==0) {
      ...
      System.out.println("OPERATION (+,-,*,/)  = --,-+,-*,-/                (ie -*  default -+)");
      System.out.println("FIXED NUMBER         = -fixtop#,-fixbot#,-fixany# (ie -fixany2)");
      System.out.println("REQUIRE POSITIVE     = -positive ");
      System.out.println("WHOLE NUMBER ANSWERS = -positive ");
      System.out.println("NUMBER OF PROBLEMS   = -prob#       (ie -prob45 ) default -prob120");
      System.out.println("LARGEST NUMBER       = -max#        (ie -max9   ) default -max9   ");
      System.out.println("SMALLEST NUMBER      = -min#        (ie -min0   ) default -min0   ");
//      <-max#> <-min#> <-{+,-,*,/}> <-fixtop/fixbot/fixany#> <-prob#> <-POSITIVE>");
      System.exit(0);
    }
...
//   <-max#> <-min#> <-{+,-,*,/}> <-fixtop/fixbot#> <-maxans#>");
  ...
  public void start() {
    ...
    for (a=0;a<pnum;a++) {
      ...
      if (fixany) {
        if (Math.random()*2>1) {fixtop=true;fixbot=false;}
        else {fixtop=false;fixbot=true;}
      }
      if (fixtop) {T=constnum;}
      else if (fixbot) {B=constnum;}
      if (sign.equals("+")) {}
      else if (sign.equals("-")) {
        if (T-B<0 && pos==true) {T=B+T;}
      }
      else if (sign.equals("*")) {}
      else if (sign.equals("/")) {
        if (pos==true) {T=B*T;}
