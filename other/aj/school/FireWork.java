...
public class FireWork {
	...
	long lastmovetime;
	Color c;
	Color clist[]={Color.red,Color.blue,Color.green,Color.cyan,Color.MAGENTA,Color.pink,Color.orange,Color.white,Color.gray};
	...
	public void setRandomStart(int maxx,int maxy,double dxmax,double dymax,int maxRad) {
		hasExploded=false;
		decay=-1;
		this.maxy=maxy;this.maxx=maxx;
		this.dxmax=dxmax;this.dymax=dymax;
		this.maxRad=maxRad;
		x=Math.random()*maxx;
		y=maxy;
		...
		if (x<maxx/2 && dx<0) dx=-dx;
		if (x>maxx/2 && dx>0) dx=-dx;
	}
	
	public void draw(Graphics g) {
		if (hasExploded()) return;
		if (decay>2000) return;
		double d=(2000.0-decay)/2000;
		g.setColor(new Color((int)(c.getRed()*d),(int)(c.getBlue()*d),(int)(c.getGreen()*d)));
		double rad=this.rad;
		if (decay==-1) rad=Math.min(rad,2);
		else rad=rad*d+1;
		g.fillOval((int)(x-rad/2),(int)(y-rad/2),(int)rad,(int)rad);
	}
...
//		int style=(int)(Math.random()*2);
		...
		newExSpeed=Math.random()*(1.0*startdy/4)*(Math.random()*1+2)/4;
		if (Math.random()*2>1)
	...
	private void setCenter(FireWork fw) {
		...
		this.dxmax=fw.dxmax;this.dymax=fw.dymax;
