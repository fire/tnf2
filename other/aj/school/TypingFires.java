...
/*
 * progress #1 sign in by name #2 begin at level 1 #3 advance after X (about 10)
 * tries. if too much time or errors, return later, after 3 advance return
 * required auto return rewards (every 10 minutes = 10 minutes fun) replay show
 * earned save show (best time) free shoot - 100 (any left, any right move,
 * space shoot?) mouse shoot - 100 dodge bombs - 100
 */
// TODO menu - sign in
...
public class TypingFires extends JPanel {
	...
	private void resetLevel() {
		...
		String gml = "gmlfile ["
				...
				+ "homekey \"<left1>\"" + "homekey \"<right1>\""
				+ "left1 \"s\"" + "left1 \"d\"" + "left1 \"f\""
				+ "right1 \"j\"" + "right1 \"k\"" + "right1 \";\""
				+ "right1 \"l\"" +
	...
	private void doMatch() {
		...
		for (int a = 0; a < match.length && a < look.length; a++) {
			if (look[a].equals(match[a])) {
				...
				for (int b = 0; b < look[a].length(); b++) {
					if (match[a].length() > b
							&& look[a].charAt(b) != match[a].charAt(b)) {
						// System.out.println("letter "+b+"
						// nomatch!("+look[a].charAt(b)+")");
						res += "<strike><font color=red>" + look[a].charAt(b)
								+ "</font></strike>";
		...
		if (match.length > look.length)
			doNotResetLevel = false;
		if (match.length == look.length
				&& match[match.length - 1].length() >= look[match.length - 1]
						.length())
			doNotResetLevel = false;
		res += "</body></html>";
		follow.setText(res);
	...
	private void addFireWork() {
		...
		fw.setRandomStart(size, size * 3 / 2, launchspeed / 4, launchspeed,
				MAXRAD);
