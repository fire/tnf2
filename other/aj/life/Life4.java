...
public class Life4 extends JApplet implements ActionListener, MouseListener, MouseMotionListener{
...
//file - O.lif
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource()==stop) {
			ld.STOP=!ld.STOP;
			if (ld.STOP) stop.setText("Start");
			if (!ld.STOP) stop.setText("Stop");
		}
		...
		for (int a=0;a<maxx;a++) {
			for (int b=0;b<maxy;b++) {
				...
				if (map[a][b]==Color.white && c==3) {
					...
					if (a<=MINBOUND) lessx=true;
					else if (a>=maxx-MINBOUND) morex=true;
					if (b<MINBOUND) lessy=true;
					else if (b>=maxy-MINBOUND) morey=true;
				}
				else if (map[a][b]!=Color.white && (c==2 || c==3)) {
					next[a][b]=avColor(a,b);
					if (a<MAXBOUND ) cutx2=false;
					else if (a>maxx-MAXBOUND ) cutx=false;
					if (b<MAXBOUND ) cuty2=false;
					else if (b>maxy-MAXBOUND ) cuty=false;
				}
			}
		}
		if (maxx<=startMAX) cutx=cutx2=false;
		if (maxy<=startMAX) cuty=cuty2=false;
		if (lessy || morey) cuty=cuty2=false;
		if (lessx || morex) cutx=cutx2=false;
		map=next;
		if (morey || lessy || morex || lessx || cutx || cuty || cutx2 || cuty2) {
			//System.out.println("resizing from "+maxx+" "+maxy);
//System.out.println(""+cutx+" "+cutx2+" "+cuty+" "+cuty2+" "+morey+" "+lessy+" "+morex+" "+lessx);
...
//System.out.println("change x="+(cutx?1:0)+" y="+(cuty?1:0));
			for (int a=(cutx?1:0);a<ox && a<maxx;a++) {
				for (int b=(cuty?1:0);b<oy && b<maxy;b++) {
					next[a+(lessx?1:0)][b+(lessy?1:0)]=map[a+(cutx2?1:0)][b+(cuty2?1:0)];
	...
	public int count(int x,int y) {
		...
		if (x-1>=0 && y-1>=0 && map[x-1][y-1]!=Color.white) total++;
		if (x-1>=0 && y+1<maxy && map[x-1][y+1]!=Color.white) total++;
		if (x+1<maxx && y-1>=0 && map[x+1][y-1]!=Color.white) total++;
		if (x+1<maxx && y+1<maxy && map[x+1][y+1]!=Color.white) total++;
		if (x-1>=0 && map[x-1][y]!=Color.white) total++;
		if (x+1<maxx && map[x+1][y]!=Color.white) total++;
		if (y+1<maxy && map[x][y+1]!=Color.white) total++;
		if (y-1>=0 && map[x][y-1]!=Color.white) total++;
		return total;
	}
	public void doRandomSetup() {
		for (int a=0;a<COLONIES;a++) {
			...
			for (int b=0;b<num;b++) {
				int lx=x+(int)(Math.random()*Math.pow(COLSIZE,.9)-(Math.pow(COLSIZE,.9)/2));
				int ly=y+(int)(Math.random()*Math.pow(COLSIZE,.9)-(Math.pow(COLSIZE,.9)/2));
				if (lx<0) lx=0;
				if (ly<0) ly=0;
				if (lx>=startMAX) lx=startMAX-1;
				if (ly>=startMAX) ly=startMAX-1;
				map[ly][lx]=ct;
			}
		}
	}
	...
	public void paint(Graphics g) {
		...
		if (i==null) {
			...
			if (getSize().height<1 || getSize().width<1) return;
		...
		for (int a=0;a<maxx;a++) {
			for (int b=0;b<maxy;b++) {
				if (map[a][b]==Color.white) continue;
				G.setColor(map[a][b]);
				G.fillRect((int)(dx*a),(int)(dy*b),(int)(dx),(int)(dy));
				G.drawRect((int)(dx*a),(int)(dy*b),(int)(dx),(int)(dy));
			}
		}
