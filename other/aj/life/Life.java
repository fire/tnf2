...
class LifeDisplay extends Canvas implements Runnable {
	...
	public int count(int y, int x, int type) {
		...
		if (x - 1 >= 0 && y - 1 >= 0 && map[y - 1][x - 1] == type)
			total++;
		if (x - 1 >= 0 && y + 1 < MAX && map[y + 1][x - 1] == type)
			total++;

		if (x + 1 < MAX && y - 1 >= 0 && map[y - 1][x + 1] == type)
			total++;
		if (x + 1 < MAX && y + 1 < MAX && map[y + 1][x + 1] == type)
			total++;

		if (x - 1 >= 0 && map[y][x - 1] == type)
			total++;
		if (x + 1 < MAX && map[y][x + 1] == type)
			total++;

		if (y + 1 < MAX && map[y + 1][x] == type)
			total++;
		if (y - 1 >= 0 && map[y - 1][x] == type)
			total++;
	...
	public void doRandomSetup() {
		for (int a = 0; a < COLONIES; a++) {
			...
			for (int b = 0; b < num; b++) {
				int lx = x
						+ (int) (Math.random() * Math.pow(COLSIZE, .5) - (Math
								.pow(COLSIZE, .5) / 2));
				int ly = y
						+ (int) (Math.random() * Math.pow(COLSIZE, .5) - (Math
								.pow(COLSIZE, .5) / 2));
