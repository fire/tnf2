 ...
 * start stop capable
 * */
public class Life2 extends Applet implements ActionListener, MouseListener, MouseMotionListener{
...
class Life2Display extends Canvas implements Runnable {
	...
	public int count(int y,int x) {
		...
		if (x-1>=0 && y-1>=0 && map[y-1][x-1]!=Color.white) total++;
		if (x-1>=0 && y+1<MAX && map[y+1][x-1]!=Color.white) total++;
		if (x+1<MAX && y-1>=0 && map[y-1][x+1]!=Color.white) total++;
		if (x+1<MAX && y+1<MAX && map[y+1][x+1]!=Color.white) total++;
		if (x-1>=0 && map[y][x-1]!=Color.white) total++;
		if (x+1<MAX && map[y][x+1]!=Color.white) total++;
		if (y+1<MAX && map[y+1][x]!=Color.white) total++;
		if (y-1>=0 && map[y-1][x]!=Color.white) total++;
		return total;
	}
