 ...
 *
 * */
public class Life3 extends Applet implements ActionListener, MouseListener,
		MouseMotionListener {
...
class Life3Display extends Canvas implements Runnable {
	...
	public int count(int x, int y) {
		int total = 0;
		if (lm.get(x - 1, y - 1) != null)
			total++;
		if (lm.get(x, y - 1) != null)
			total++;
		if (lm.get(x + 1, y - 1) != null)
			total++;
		if (lm.get(x + 1, y) != null)
			total++;
		if (lm.get(x + 1, y + 1) != null)
			total++;
		if (lm.get(x, y + 1) != null)
	...
	public void paint(Graphics g) {
		...
		// System.out.println("paint called "+lm.maxx+" "+lm.minx+" "+lm.maxy+"
		// "+lm.miny);
		for (int a = lm.minx; a <= lm.maxx; a++) {
			for (int b = lm.miny; b < lm.maxy; b++) {
				...
				if (lc == null)
					continue;
				G.setColor(lc.color);
				G.fillRect((int) (dx * (a - lm.minx)),
						(int) (dy * (b - lm.miny)), (int) (dx), (int) (dy));
			}
		}
...
class Life3Map {
	...
	public void doRandomSetup() {
		for (int a = 0; a < startCellGroups; a++) {
			...
			for (int b = 0; b < num; b++) {
				int lx = x
						+ (int) (Math.random() * Math.pow(numCellsPerGroup, .9) - (Math
								.pow(numCellsPerGroup, .9) / 2));
				int ly = y
						+ (int) (Math.random() * Math.pow(numCellsPerGroup, .9) - (Math
								.pow(numCellsPerGroup, .9) / 2));
				if (lx < 0)
				...
				if (ly < 0)
					ly = 0;
				if (lx >= defaultStartSize)
					lx = defaultStartSize - 1;
				if (ly >= defaultStartSize)
					ly = defaultStartSize - 1;
