...
public class Gladator {
	 ...
	 * Description of the Field
	 */
	public static int HEAD = 0, BODY = 1, LEFTARM = 2, RIGHTARM = 3,
	 ...
	 *            The new Armor value
	 */
	public void setArmor(Armor h, Armor b, Armor ll, Armor rl, Armor la,
			Armor ra) {
		headArmor = h;
		bodyArmor = b;
		lLegArmor = ll;
	 ...
	 */
	public void finish() {
		// determin last performance/ best overall performnce (max of school &&
		// last)
		...
		if (attempt.handtohand != 0) {
			lastPerformance.handtohand = 100 * pass.handtohand
					/ attempt.handtohand;
		}
		if (attempt.parry != 0) {
	...
	public String toString() {
		...
		s += "HEALTH: head:" + HHP + " body:" + BHP + " rarm:" + RAHP
				+ " larm:" + LAHP + " rleg:" + RLHP + " lleg:" + LLHP + "\n";
		s += "REAL SKILL\n";
		s += real + "\n";
	...
	private boolean betterWeapon(Item i) {
		return false;
		// if (i.getCost() > mainWeapon.getCost() || i.getCost() >
		// altWeapon.getCost()) return true;
	}
...
/*
 * leg 0 or arm 0 = -33% #attacks if (hit in area 0) then body blow. 1 arm 0 =
 * -33% to hit parry block 1 leg 0 = -33% to dodge arm==0 system shock leg==0
 * system shock 2 arms legs = passout body=0 passout head=0 passout total
 * hitpoint < surrender = evacuated
 */

/*
 * imporve skill rules school standings only change from school testing last
 * performance = pass/attempts if not attempted then last score passes forward
