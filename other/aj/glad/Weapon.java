...
public class Weapon extends Item {
	...
	public int getDamage() {
		...
		try {
			...
			if (hitdice.indexOf("d") > 0) {
				num = Integer.parseInt(hitdice.substring(0, hitdice
						.indexOf("d")));
				if (hitdice.indexOf("+") > 0) {
					dice = Integer.parseInt(hitdice.substring(
							hitdice.indexOf("d") + 1, hitdice.indexOf("+"))
							.trim());
					plus = Integer.parseInt(hitdice.substring(
							hitdice.indexOf("+") + 1).trim());
				} else {
					dice = Integer.parseInt(hitdice.substring(
							hitdice.indexOf("d") + 1).trim());
				}
			} else {
	...
	public String toString() {
		String s = name + " lbs" + getWeight() + " gp" + getCost() + " hp"
				+ getMaxHitPoints();
		s += " WEAPON ";
		if (getType() == CLUB) {
			s += "CLUB";
