...
public class Robot extends ArenaItem {
	...
	public void display(Graphics g) {
		...
		//draw robot circle
		g.drawOval(x - ROBOTDIAMETER / 2, y - ROBOTDIAMETER / 2, ROBOTDIAMETER, ROBOTDIAMETER);
		//draw speed
		if (currspeed!=0)  {
			Arrow AAA=new Arrow(new Point(x,y), new Point(x + (int) (currspeed / 2 * Math.cos(Stuff.dtr(currheading))), 
				y + (int) (currspeed / 2 * Math.sin(Stuff.dtr(currheading)))),4,6);
			AAA.setFill(true);
			AAA.display(g,1);
		}
		//draw damage
		g.fillRect((int) (x + ROBOTDIAMETER), (int) (y-100/4), DAMAGEWIDTH - 2, (int) (damage/2));
		//draw full damage line
		g.drawRect((int) (x + ROBOTDIAMETER), (int) (y-100/4), DAMAGEWIDTH - 2, (int) (100/2));
		//draw heading 
		Arrow AA=new Arrow(new Point(x,y),new Point(x + (int) (ROBOTDIAMETER *2* Math.cos(Stuff.dtr(currheading))),
				y + (int) (ROBOTDIAMETER*2 * Math.sin(Stuff.dtr(currheading)))),4,6);
		AA.setFill(true);
		AA.display(g,1);
		...
		if (c==null) return;
		g.setColor(c);
		//g.drawLine(x, y, x + (int) (ROBOTDIAMETER * Math.cos(Stuff.dtr(currheading))), 
