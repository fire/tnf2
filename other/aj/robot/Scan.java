...
public class Scan extends ArenaItem{
	...
	public void display(Graphics g) {
		...
		g.setColor(c);
		if (ran > 0) {
			g.drawArc(x - ran,y - ran,ran* 2,ran*2, (360 - dir) - width, width * 2);
			g.setColor(Color.black);
			g.drawString(""+width,(int)(x + Math.cos(Stuff.dtr(dir-width))*ran),(int)(y + Math.sin(Stuff.dtr(dir-width))*ran));
			g.setColor(c);
		}
		//360-dir for inverted y down canvas
		g.drawLine(x, y, x + (int) (10000*arena.getScale() * Math.cos(Stuff.dtr(dir - width))), 
				y + (int) (10000*arena.getScale() * Math.sin(Stuff.dtr(dir - width))));
		g.drawLine(x, y, x + (int) (10000*arena.getScale() * Math.cos(Stuff.dtr(dir + width))), 
				y + (int) (10000 *arena.getScale()* Math.sin(Stuff.dtr(dir + width))));
	}
	...
	public String toString() {
		int x = (int) (getX(arena.getScale()));
		int y = (int) (getY(arena.getScale()));
		return "Scan at " + x + " " + y + " ran=" + ran + " dir=" + dir + " wid=" + width;
	}
