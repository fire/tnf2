...
public class Plan implements ActionListener {
	...
	public void updateGui() {
		...
		for (int a = 0; a < activeFields; a++) {
			if (nameT[a] == null)
				nameT[a] = new JTextField();
			nameT[a].setText("Field" + (char) ('a' + a));
			if (bidQT[a] == null)
				bidQT[a] = new JTextField();
			if (bidT[a] == null)
				bidT[a] = new JTextField();
			if (valueT[a] == null) {
			...
			if (askT[a] == null)
				askT[a] = new JTextField();
			if (askQT[a] == null)
				askQT[a] = new JTextField();
	...
	public void setupGui() {
		...
		String all[] = { "2-MSFT_Price", "3-FedPolicyB", "4-Comp_Ret", "5-",
				"6-", "7-", "8-" };
	...
	public void actionPerformed(ActionEvent ae) {
		...
		double sumv = 0;
		for (int a = 0; a < activeFields; a++) {
			v[a] = Math.min(1, Math.max(0, Double.parseDouble(Stuff.superTrim(
					valueT[a].getText(), ",$ \t\n"))));
			sumv += v[a];
		}
		...
		for (int a = 0; a < activeFields; a++) {
			// System.out.println("bid "+nam1.getText()+" ~"+Stuff.trunc(ab,3)+"
			// ~"+aaplQBid.getText()+" 1");
			// System.out.println("ask "+nam1.getText()+" ~"+Stuff.trunc(aa,3)+"
			// ~"+aaplQAsk.getText()+" 1");
