...
public class Orders {
	...
	public Orders(IEMTool toolRef) {
		...
		for (int a = 0; a < commands.size(); a++) {
			...
			try {
				...
				report += "MyError> bad number in netheld> "
						+ lookUpNetHeld(name) + "\n";
				continue;
			}
			int del = 0;
			try {
				...
				report += "MyError> bad number in deltaheld> "
						+ lookUpDeltaHeld(name) + "\n";
				continue;
			}
			...
			try {
				if (quant.startsWith("~")) {
					...
					if (isAsk) {
						// Sys.out.println("request="+quant+" have
						// "+lookUpDeltaHeld(name));
						quan = del - quan;
						if (quan < 1) {
							report += " SKIPPING inventory beyond threshold ASK>"
									+ quan + "\n";
							continue;
						}
					}
					if (isBid) {
						...
						quan = quan - hav;
						if (quan < 1) {
							report += " SKIPPING inventory beyond threshold BID>"
									+ quan + "\n";
							continue;
						}
					}
			...
			try {
				...
				if (d > 2) {
					d = d / 1000.0;
					report += "WARNING: val >1 converting to decimal (orig="
							+ val + " new=" + d + ")\n";
				}
				val = Stuff.trunc(d, 3) + "";
			} catch (NumberFormatException nfe) {
				if (val.startsWith("~")) {
					...
					String task = curA;
					double ask = Double
							.parseDouble((task.indexOf("*") >= 0 ? task
									.substring(0, task.indexOf("*"))
									+ task.substring(task.indexOf("*") + 1)
									: task)) / 1000.0;
					String tbid = lookUpBid(name);
					double bid = Double
							.parseDouble((tbid.indexOf("*") >= 0 ? tbid
									.substring(0, tbid.indexOf("*"))
									+ tbid.substring(tbid.indexOf("*") + 1)
									: tbid)) / 1000.0;
					...
					if (isAsk) {
						...
						if (val.endsWith("*")) {
							report += " SKIPPING Current low asker with "
									+ curA + "\n";
							continue;
						}
						try {
							...
							if (d < max) {
								report += " SKIPPPING Target ASK " + max
										+ " higher than current ask " + d
										+ "\n";
								val = "" + Stuff.trunc(max, 3);
								continue;
							} else {
								// Sys.out.println("TARGET ASK *BELOW* current
								// of "+curA+" new ask="+d);
								val = "" + Stuff.trunc(d, 3);
							}
					...
					if (isBid) {
						val = curB;
						if (val.endsWith("*")) {
							report += " SKIPPING Currenty high bidder with bid of "
									+ curB + "\n";
							continue;
						}
						try {
							double d = Double.parseDouble(val) / 1000.0 + .002;
							if (d > max) {
								report += " SKIPPING TARGET BID " + max
										+ " below current bid " + d + "\n";
								val = "" + Stuff.trunc(max, 3);
								continue;
							} else {
								// Sys.out.println("TARGET BID *ABOVE* current
								// of "+curB+" new bid="+d);
								val = "" + Stuff.trunc(d, 3);
							}
						} catch (NumberFormatException nfe2) {
	...
	private String lookUp(String name) {
		...
		for (int a = 0; a < assets.size(); a++) {
			...
			if (bund == null
					&& s.toUpperCase().indexOf(name.toUpperCase() + " ") >= 0)
				return s;
			if (bund != null && s.toUpperCase().equals(bund.toUpperCase())
					&& s.toUpperCase().indexOf(name.toUpperCase() + " ") >= 0)
				return s;
		}
