...
//add proxy config
  ...
  boolean winner=false;
	...
	public static void help() {
		System.out.println("Format: java aj.iem.Risk Sid Mid <options> [contracts...]");
...
//add winning payout of winning and losing payout of losing for each contract
		if (s.length>=3) {
			String sid=s[0];
			...
			for (int a=2;a<s.length;a++) {
				if (s[a].indexOf("-")>=0 || s[a].indexOf("?")>=0) {
					if (s[a].toUpperCase().indexOf("-H")>=0 || s[a].indexOf("?")>=0) {
						help();
					} 
					if (s[a].toUpperCase().indexOf("-Q")>=0) {System.out.println("Quick download detected");quick=true;} 
					if (s[a].toUpperCase().indexOf("-L")>=0) {System.out.println("Loggin turned on");log=true;} 
					if (s[a].toUpperCase().indexOf("-S")>=0) {System.out.println("Socket Loggin turned on");logSocket=true;} 
					if (s[a].toUpperCase().startsWith("-W")) {
						...
						if (IEMTool.proxyHost.indexOf(":")>=0) {
							try {
								...
								System.out.println("MyError proxy config bad number");
								System.exit(0);
							}
						}
						System.out.println("Proxy config found "+IEMTool.proxyHost+" and port="+IEMTool.proxyPort);
					}
					continue;
				}
				...
				try {
					...
					if (t-50<midt) {
						System.out.println("MyError: bad number in params AID="+aid+" cannot be within 50 of MID");
						System.exit(0);						
					}
				} catch (NumberFormatException nfe){
					System.out.println("MyError: bad number in params AID="+aid);
					System.exit(0);
				}
...
mtw=-mtw+mtl+marketinv;
				//System.out.println(" assets="+r.numheld+" profit if win="+Stuff.trunc(tw,3)+" marketonlyprofit="+Stuff.trunc(mtw,3));
				System.out.println(" Profit if win="+Stuff.trunc(tw,3)+" marketonlyprofit="+Stuff.trunc(mtw,3)+" " +(r.winner?"WINNER":""));
			}
		}
		else {
			help();
...
if (logSocket) System.out.println("preML --begin--");
if (logSocket) System.out.println(temp);
if (logSocket) System.out.println("preML --end--");
...
while (td.indexOf("/")>=0) {td=td.substring(0,td.indexOf("/"))+"%2F"+td.substring(td.indexOf("/")+1);}
while (td.indexOf(" ")>=0) {td=td.substring(0,td.indexOf(" "))+"+"+td.substring(td.indexOf(" ")+1);}
while (td.indexOf(":")>=0) {td=td.substring(0,td.indexOf(":"))+"%3A"+td.substring(td.indexOf(":")+1);}
//System.out.println("TopDate fixed "+td);
			...
			String request=
				...
				"\r\n";
			String header=
				"POST /webex/WebEx.dll?TradeHistoryHandler  HTTP/1.0\r\n"+
...
//System.out.println("request ---\n"+request);
			System.out.print("+");
			temp=IEMTool.readAllSocket(header,request,"iemweb.biz.uiowa.edu",80);
...
//if (log) System.out.println("READALL --end--");
		...
		System.out.print("*");
		String contract="";
		if (all.indexOf(" PM")>=0) contract=all.substring(all.indexOf("PM")+2,all.indexOf("PM")+102).trim();
		else if (all.indexOf(" AM")>=0) contract=all.substring(all.indexOf("AM")+2,all.indexOf("AM")+102).trim();
		if (all.indexOf("Liquidation")>=0) {
			...
			if (t[2].startsWith("1.")) winner=true;
  ///1/19/04  13:18:13    Liquidation  51  0.00000  
			System.out.println("winnwer="+winner);
		}
		if (contract.indexOf("Trade")>=0) contract=contract.substring(0,contract.indexOf("Trade"));
		contName=contract;
...
//System.out.println("all.length()="+all.length());
		String holdings=all.substring(all.indexOf("Holdings:")+"Holdings:".length(),all.indexOf("Holdings:")+"Holdings:".length()+102);
//System.out.println("holdings.length()="+holdings.length());
		holdings=Stuff.superTrim(holdings);
		if (holdings.indexOf("*")>=0) holdings=holdings.substring(0,holdings.indexOf("*"));
...
//System.out.println(all);
		...
		while (all.indexOf(" Buy",lastbuy)>=lastbuy) {
			if (log) System.out.print("b");
			String t;
			int nextbuy=all.indexOf(" Buy",lastbuy);
			int uptobuy=all.indexOf(" Buy",nextbuy+1);
			int uptosell=all.indexOf(" Sell",nextbuy);
//System.out.println("lastbuy="+lastbuy+" nextbuy="+nextbuy+" uptobuy="+uptobuy);
			if (uptosell>nextbuy && uptobuy>uptosell) uptobuy=uptosell;
			if (uptobuy<lastbuy) uptobuy=all.length();
			t=all.substring(nextbuy,uptobuy);
			lastbuy=nextbuy+1;

			//String t=all.substring(all.indexOf("Buy",lastbuy),lastbuy+all.substring(all.indexOf("Buy",lastbuy))
			//if (false) {
			...
			//}
			if (t.indexOf("Fixed Price Bundle")>=0) continue;
			if (t.indexOf("Market Prices Bundle")>=0)  
				t=t.substring(0,t.indexOf("Market Prices Bundle"))+t.substring(t.indexOf("Market Prices Bundle")+"Market Prices Bundle".length())+"M";
			buys.addElement(t.trim());
			if (buys.size()%100==0) System.out.print("-");
		}
...
//System.out.println("lastbuy="+lastbuy+" nextbuy="+nextbuy+" uptobuy="+uptobuy);
			...
			if (sells.size()%100==0) System.out.print("-");
		}
		if (log) System.out.println(".");
		...
		for (int a=0;a<buys.size();a++) {
			...
			if (market) {
				...
				//mlose-=v;
				minv+=amt;
				if (log) System.out.println("MARKET BUYFOUND "+contract+" "+t[1]+"@"+t[2]+" market inventor="+minv);
			}
			else {
				if (log) System.out.println("BUYFOUND "+contract+" "+t[1]+"@"+t[2]+" = "+v+" pricePaid to buy="+cost+" netinv="+netinv+" "+(market?"M":""));
				win+=amt-v;
				lose-=v;
				netinv+=amt;
				cost+=v;
		...
		for (int a=0;a<sells.size();a++) {
			...
			if (market) {
				...
				mlose+=v;
				minv-=amt;
				if (log) System.out.println("MARKET SELLFOUND "+contract+" "+t[1]+"@"+t[2]+" market inventor="+minv);
			}
			else {
				win-=amt-v;
				lose+=v;
				netinv-=amt;
				earn+=v;
				if (log) System.out.println("SELLFOUND "+contract+" "+t[1]+"@"+t[2]+" = "+v+" cash received from sale="+earn+" netinv="+netinv+" ");
			}
		}
		//System.out.println("REPORT Total for "+netinv+" of "+contract+" pricePaid to buy = "+Stuff.trunc(cost,3)+" cash Received from sale="+Stuff.trunc(earn,3));
		//System.out.println("REPORT Total if "+contract+" win="+Stuff.trunc(win,3)+" lose="+Stuff.trunc(lose,3));

		totalWin=win;
		totalLose=lose;
	}

	public static String readAll(String userId,String sessionId,String marketId,String assetId) {
		if (userId==null) {
			System.out.println("NO userId found.  Using System.in to file data");
			return readAll();
		} 
		else return IEMTool.readAllSocket("GET "+"/webex/WebEx.dll?TradeHistoryHandler?USERTYPE=trader&LOGIN="+userId+"&SESSIONID="+sessionId+"&LANGUAGE=english&Markets="+marketId+"&Asset="+assetId+"&Panel_id=main"+" HTTP/1.0\r\n\r\n","","iemweb.biz.uiowa.edu",80);
	}
