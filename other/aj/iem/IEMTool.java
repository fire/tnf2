...
public class IEMTool implements Runnable{
	...
	static String buyAtMarket="P",sellAtMarket="S",placeBid="B",placeAsk="A",
		buyFixBundle="D",buyMarkBundle="N",sellFixBundle="C",sellMarkBundle="M";
	...
	static int LOWESTBIDALLOWED=10;//under 5 cent don't bid, over 95 don't ask
		...
		try {
			...
			//	reportOrders("found config line "+s);
				if (s.toUpperCase().startsWith("ALLOWEDMARKET")) {
					...
					//System.out.println("MINCASHRESERVE found="+MINCASHRESERVE);
				}
				else if (s.toUpperCase().startsWith("ORDERSCOMMANDFILE")) {
					orderscommandfile=s.substring(17).trim();
					//System.out.println("orderscommandfile found="+orderscommandfile);
				}
					...
					//System.out.println("ordersoutputlog found="+ordersoutputlogfile);
				}
				else if (s.toUpperCase().startsWith("STATUSORDERSLOGFILE")) {
					iemstatusordersfile=s.substring(19).trim();
					//System.out.println("iemstatusordersfile found="+iemstatusordersfile);
				}
					...
					EXTREAM_BUND_SIZE=Integer.parseInt(s.substring(17).trim());
				}
				else if (s.toUpperCase().startsWith("DISCOUNT_BUND_SIZE")) {
					DISCOUNT_BUND_SIZE=Integer.parseInt(s.substring(18).trim());
				}
	...
	public static void main(String s[]) {
		try {
			IEMTool.localhostId=new String(InetAddress.getLocalHost().getCanonicalHostName());
		} catch (IOException ioe) {}
		IEMTool iem=new IEMTool();
		for (int a=0;a<s.length;a++) {
			if (s[a].toUpperCase().indexOf("?")>=0 || 
			s[a].toUpperCase().indexOf("-HELP")>=0 ) {help();System.exit(0);}
			else if (s[a].toUpperCase().startsWith("-S")) {
				IEMTool.sessionId=s[a].substring(2);
				IEMTool.loginEnabled=false;continue;
			}
			else if (s[a].toUpperCase().startsWith("-U")) {
				IEMTool.userId=s[a].substring(2);continue;
			}
			else if (s[a].toUpperCase().startsWith("-P")) {
				IEMTool.password=s[a].substring(2);continue;
			}
	...
	public void doLogin() {
		...
		if (isPract())  {defaultFirstMarketId="3";}
		if (!loginEnabled) {
		...
		if (all!=null && all.indexOf("SESSIONID")>=0) {
			...
			reportTool("Login failure force sleep for "+(LOGINDELAY/1000/60)+" minutes");
			doLogout();
			LOGINDELAY*=2;
		}
		if (isLoggedIn()) LOGINDELAY=MINLOGINDELAY;
	}

	static public String doInterruptRequest(){
		reportTool("Placing interrupt");
		String request=
			"USERTYPE=trader&LOGIN="+userId+"&LANGUAGE=english&IPSecurity="+IPSEC+"&PASSWORD="+password+"\r\n";
		String header=
			"POST /webex/WebEx.dll?InterruptSessionHandler HTTP/1.0\r\n"+
			"Host: "+localhostId+"\r\n"+
	...
	static public String doLoginRequest() {
		String request=
			"Login="+userId+"&Password="+password+"&UserType=trader&Language=english&IPSecurity="+IPSEC+"\r\n";
		
		String header=
			"POST /webex/WebEx.dll?LoginHandler HTTP/1.0\r\n"+
			"Host: "+localhostId+"\r\n"+
			"User-Agent: Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.0.1) Gecko/20021003\r\n"+
			"Content-Type: application/x-www-form-urlencoded\r\n"+
			"Content-Length: "+(request.length()-2)+"\r\n"+"\r\n";
	...
	static public boolean isLoggedIn() {
		return userId!=null && sessionId!=null && userId.trim().length()>0 && sessionId.trim().length()>0;
	}
	...
	static synchronized public void reportStatus() {
			...
			int asset=0;
			String report="";
//correct MarketId required for limits.  Each market (CompRet, MSFT, FR) has unique MarketID.
			...
			report =new Date().toString()+(partial?" *partial*":"")+" time "+(elasp/1000.0)+" secs"+
				"\nCash     ="+(cash<100000?" ":"")+(cash<10000?" ":"")+(cash<1000?" ":"")+cash+
				"\nBundles  ="+(liquid-cash<100000?" ":"")+(liquid-cash<10000?" ":"")+(liquid-cash<1000?" ":"")+(liquid-cash)+
				"\nLiquid   ="+(liquid<100000?" ":"")+(liquid<10000?" ":"")+(liquid<1000?" ":"")+liquid+
				"\nAsset Val="+(asset<100000?" ":"")+(asset<10000?" ":"")+(asset<1000?" ":"")+asset+
				"\nGross Val="+(asset+liquid<100000?" ":"")+(asset+liquid<10000?" ":"")+(asset+liquid<1000?" ":"")+(asset+liquid)+"\n\n"+report;
			try {
	...
	static public synchronized void reportActive(int liquid,int assets,String line) {
		if (lastActive.equals(liquid+"")) return;
		else lastActive=(liquid+"");
		Vector v=getAllMarketsClone();
		for (int a=0;a<v.size();a++) {
			Market m=(Market)v.elementAt(a);
		...
		try {
			...
			String doww="";
			switch (dow) {
				case 1: doww="Su";break;
				case 2: doww="Mo";break;
				case 3: doww="Tu";break;
				case 4: doww="We";break;
				case 5: doww="Th";break;
	...
	static public synchronized void reportTool(String line) {
		try {
			...
			if (reportToolCount>60) {reportToolCount=0;ra.writeBytes(new Date().toString()+"\n");}
			ra.close();
		} catch (IOException ioe) {}
	}
...
//	public static synchronized String readAllSocket(String host, int port, String header,String req) {
	static public synchronized String readAllSocket(String header,String req, String host,int port) {
		if (host.equalsIgnoreCase("iemweb.biz.uiowa.edu")) host="128.255.244.60";
		if (host.equalsIgnoreCase("www.biz.uiowa.edu")) host="128.255.244.57";
		String urlstring="";
		urlstring="http://"+host+(port!=80?":"+port:"")+header.substring(4,header.indexOf("HTTP/1.0")).trim();
		...
		try {
			...
			try {
				...
				reportTool("MyError io error 15.  Read web page from socket.  Connect/read failure."+e1);
				e1.printStackTrace();
			}
		} catch (MalformedURLException e) {
			reportTool("MyError io error 16.  Read web page from socket.  Connect/read failure."+e);
			e.printStackTrace();
		}
		return null;
	}
	...
	public void doRelogin() {
		try {
			BufferedReader br=new BufferedReader(new FileReader(iemstatusordersfile));
			String s=br.readLine();
	...
	public void run() {
		...
		while (true) {
			reportStatus();
			try {
				Thread.sleep((int)(LOGINCHECKDELAY*(Math.random()*.1+1)));
			}catch (Exception ei) {}
			if (!isLoggedIn()) {
				doLogin();
				downloadMarketSeeds();
	...
	public void downloadMarketSeeds() {
		...
		String all=readAllSocket("GET "+"/webex/WebEx.dll?TraderInterfaceHandler?USERTYPE=trader&LOGIN="+userId+"&SESSIONID="+sessionId+"&LANGUAGE=english&Markets="+defaultFirstMarketId+"&Panel_id=menu"+" HTTP/1.0\r\n\r\n","","iemweb.biz.uiowa.edu",80);
		Thread.yield();
		if (all.indexOf("<SELECT NAME=\"Markets\">")>=0) {
			all=all.substring(all.indexOf("<SELECT NAME=\"Markets\">")+23);
			if (all.indexOf("</SELECT>")>=0) all=all.substring(0,all.indexOf("</SELECT>"));
			else {doLogout();return;}
			while (all.indexOf("VALUE=\"")>=0) {
				String mv=all.substring(all.indexOf("VALUE=\"")+7);
				if (mv.indexOf("\"")>=0) mv=mv.substring(0,mv.indexOf("\""));
				all=all.substring(all.indexOf("VALUE=\"")+7);
				String mn="";
				if (all.indexOf(">")>=0) mn=all.substring(all.indexOf(">")+1);
				if (mn.indexOf("<")>=0) mn=mn.substring(0,mn.indexOf("<"));
				mn=mn.trim();
				Market mm=new Market(mn,mv);
				if (!mm.isValidMarket()) {
					reportTool("MyError: market download failed ="+mn+" "+mv);
					doLogout();
					return;
				}
				newMarkets.add(mm);
	...
	static public void doLogout() {
		...
		if (sessionId!=null) lastLoginTry=System.currentTimeMillis();
		sessionId=null;
	}

	static public void placeMarketOrder(String transType,String price,String quant,String assetId,String marketId) {
		try {
			...
			if (transType.equals(buyMarkBundle)) price=""+Stuff.trunc(Math.min(.999,pricev),3);
			if (transType.equals(sellMarkBundle)) price=""+Stuff.trunc(Math.max(1.001,pricev),3);
			//if (transType.equals(sellFixBundle)) price="1.000";
			//if (transType.equals(buyFixBundle)) price="1.000";
		} catch (NumberFormatException nfe) {
			reportTool("MyError numberformat io error 7 in place market order for>"+price);
			return;
		}
		String request=
			"USERTYPE=trader&LOGIN="+userId+"&SESSIONID="+sessionId+"&LANGUAGE=english&Markets="+marketId+
			"&Panel_id=main&AssetId="+assetId+"&Quantity="+quant+"&Price="+price+"&OrderType="+transType+"\r\n";
		String header=
			"POST /webex/WebEx.dll?OrderConfirmHandler HTTP/1.0\r\n"+
		...
		int port=80;
//		if (transType.equals(buyMarkBundle) || transType.equals(sellMarkBundle)) {
//			reportOrders("MarkBund request ="+request);
//		}
		//log market buy and seel request format to see if error causing missed orders

		readAllSocket(header,request,host,port);
		Thread.yield();
	}

	static public void placeLimitOrder(String transType,String price,String quant,String assetId,String marketId,long delay) {
		try {
			double pricev=Double.parseDouble(price);
			int quan=Integer.parseInt(quant);
			if (quan<1) {
			...
			reportTool("MyError numberformat io error 17 in place limit order for>"+price);
			return;
		}
		String request=
			"USERTYPE=trader&LOGIN="+userId+"&SESSIONID="+sessionId+"&LANGUAGE=english&Markets="+marketId+
			"&Panel_id=main&RequestCode="+transType+assetId+"&LimitOrderPrice="+price+"&LimitOrderQuantity="+quant+
			"&LimitOrderExpirationDate="+limitTime(delay)+"Immediate=0\r\n";

		String header=
			"POST /webex/WebEx.dll?LimitOrderHandler HTTP/1.0\r\n"+
			"Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, application/vnd.ms-excel, application/msword, application/vnd.ms-powerpoint, */*\r\n"+
			"Accept-Language: en-us\r\n"+
			"User-Agent: Mozilla/4.0 (flandar@yahoo.com)\r\n"+
			"Host: "+localhostId+"\r\n"+
	...
	static public String limitTime(long delay) {
		...
		if (h>12) h=h-12;
 		int min=calendar.get(Calendar.MINUTE);
		return (m<10?"0":"")+m+"%2F"+(d<10?"0":"")+d+"%2F"+(y<10?"0":"")+y+"+"+(h<10?"0":"")+h+"%3A"+(min<10?"0":"")+min+(ap==1?"AM":"PM")+"&";
	}
