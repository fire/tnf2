...
public class LogicPuzzleGui extends JPanel {
	...
	private void drawOpen(Graphics g, int width, int height, String shape) {
		...
		if (shape == LogicPuzzle.OPEN) {
			...
			// g.drawLine(i+width/2,j-height,i+width,j-height);
		} else if (shape == LogicPuzzle.AND || shape == LogicPuzzle.OR
				|| shape == LogicPuzzle.NOR || shape == LogicPuzzle.XOR
				|| shape == LogicPuzzle.NAND) {
			g.setColor(wire);
			g.fillArc(-width / 2, -height / 2, width / 2 + 1, height + 1, -90,
					180);
			if (shape == LogicPuzzle.OR || shape == LogicPuzzle.NOR
					|| shape == LogicPuzzle.XOR) {
				g.setColor(Color.white);
			...
			if (shape == LogicPuzzle.XOR) {
				g.drawArc(-width / 2 - width / 24, -height / 2, width / 3,
						height, -70, 140);
			}
			g.setColor(wire);
