...
//		g.translate(getWidth() / 2, getHeight() / 2);
		...
		g.translate(getWidth()/2-(int)p.getX()*SCALE,getHeight()/2-(int)p.getY()*SCALE);
...
//		g.translate(-(int) (x * SCALE), -(int) (y * SCALE));
		for (int a = 0; a < gridMap.length; a++) {
			for (int b = 0; b < gridMap[a].length; b++) {
				...
				if (gridMap[a][b] == Grid.SOLID) {
					...
					g.fillRect((int) ( - SCALE / 2),
							(int) ( - SCALE / 2), SCALE, SCALE);
					g.setColor(Color.black);
					g.drawRect((int) ( - SCALE / 2),
							(int) (- SCALE / 2), SCALE, SCALE);
				}
				if (gridMap[a][b] == Grid.DOOR) {
					g.drawLine((int) ( - SCALE / 2),
							(int) ( - SCALE / 2),
							(int) ( + SCALE / 2),
							(int) ( + SCALE / 2));
					g.drawLine((int) ( + SCALE / 2),
	...
	public void mouseClicked(MouseEvent e) {
		...
		double y=(c.getY()-getHeight()/2)/30+parent.getPlayer().getY();
		
////		System.out.println("raw clicked "+c);
//		c=new Point((int)(c.getX()-getWidth()/2),(int)(c.getY()-getHeight()/2));
////		System.out.println("screen adjust clicked "+c);
//		c=new Point((int)Math.round(c.getX()/30),(int)Math.round(c.getY()/30));
