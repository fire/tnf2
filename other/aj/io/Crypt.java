...
public class Crypt {
	...
	public Crypt(String argv[]) {
		...
		if (argv.length < 1 || argv.length > 3) {
			...
			System.out
					.println("<sfile> <dfile>: reads and writes any size file");
			System.exit(0);
		}
		if (argv.length == 3) {
			if (argv[1].equals(argv[2])) {
				System.out
						.println("Source and Destination cannot be the same:  see java Crypt");
			}
		}
		...
		try {
			if (argv.length == 1) {
				...
				if (FI.available() > MAXCHUNCK) {
					System.out
							.println("File too large, for help use: java Crypt");
					System.exit(0);
				}
	...
	private static byte[] encode(byte Key[], byte b[]) {
		for (int a = 0; a < b.length; a++) {
			b[a] = (byte) ((~b[a] | Key[a % Key.length]) & (b[a] | ~Key[a
					% Key.length]));
		}
		return b;
	}
