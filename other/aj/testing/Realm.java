...
//screen control chars
	...
	public void run() {
		//for (int a=0;a<SIZE*SIZE;a++) map[a/SIZE][a%SIZE]=(char)Math.random()*
//read command
//write command
//update display
	}
	...
	void updateDisplay(int x,int y) {
		...
		for (int a=-WID;a<WID+1;a++) {
			for (int b=-WID;b<WID+1;b++) {
				char disp=' ';
				if ((a==WID || a==-WID) && (b==WID || b==-WID)) disp='+';
				else if (a==-WID || a==WID) disp='|';
				else if (b==-WID || b==WID) disp='-';
				else if (y+a>=0 && y+a<SIZE && x+b>=0 && x+b<SIZE) disp=map[a][b];
