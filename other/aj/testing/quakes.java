...
public class quakes {
	...
	public static void main(String s[]) throws IOException {
		...
		try {
			...
			// String
			// masterlist="http://quake.wr.usgs.gov/recenteqs/Quakes/quakes0.html";
			// BufferedReader br=new BufferedReader(new InputStreamReader(new
			// URL(masterlist).openStream()));
			...
			while (all.indexOf("MAP</A>") >= 0) {
				...
				// //map</A> 1.6 <A
				// HREF="/recenteqs/Quakes/ci14155392.html">2005/06/16 14:36:24
				// 34.049N 116.990W 11.6</A> 6 km ( 4 mi) ENE of Yucaipa, CA <A
				// HREF="/recenteqs/Maps/117-34.html">
				String lat = "", lon = "", link = "", icon = "", mag = "", time = "";
				if (sub.indexOf("<A HREF=\"") >= 0)
					sub = sub.substring(sub.indexOf("<A HREF=\"") + 9).trim();
				if (sub.indexOf("\">") >= 0)
				...
				if (sub.indexOf(":") >= 0)
					time = sub.substring(0, sub.lastIndexOf(":") + 3).trim();
				if (sub.indexOf(":") >= 0)
					sub = sub.substring(sub.lastIndexOf(":") + 3).trim();
				// if (sub.indexOf("\"")>=0)
				// sub=sub.substring(sub.indexOf("\"")+1);
				if (sub.indexOf(" ") >= 0)
					lat = sub.substring(0, sub.indexOf(" "));
				if (sub.indexOf(" ") >= 0)
					sub = sub.substring(sub.indexOf(" ")).trim();
				if (sub.indexOf(" ") >= 0)
					lon = sub.substring(0, sub.indexOf(" "));
				if (sub.indexOf(" ") >= 0)
					sub = sub.substring(sub.indexOf(" ")).trim();
				if (sub.indexOf("<") >= 0)
					mag = sub.substring(0, sub.indexOf("<"));
	...
	public static void readBouy(String s) throws IOException {
		...
		while (true) {
			...
			if ((ss.indexOf(" N ") >= 0 || ss.indexOf(" S ") >= 0)
					&& (ss.indexOf(" W ") >= 0 || ss.indexOf(" E ") >= 0)) {
				ss = MLtoText.cutMLaddSpaces(ss);
				if (ss.indexOf("(") >= 0)
					ss = ss.substring(0, ss.indexOf("("));
				String name = s.substring(s.indexOf("station=") + 8);
				if (ss.indexOf(" N ") >= 0)
					ss = ss.substring(0, ss.indexOf(" N ") + 3) + ","
							+ ss.substring(ss.indexOf(" N ") + 3);
				if (ss.indexOf(" S ") >= 0)
					ss = ss.substring(0, ss.indexOf(" S ") + 3) + ","
							+ ss.substring(ss.indexOf(" S ") + 3);
