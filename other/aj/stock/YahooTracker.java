...
class YahooTracker extends JPanel implements ActionListener {
	...
	public int getDateValue(String date) {
		...
		if (t.length > 1) {
			if (t[1].equalsIgnoreCase("Jan"))
				res += 30;
			if (t[1].equalsIgnoreCase("Feb"))
				res += 30 * 2;
			if (t[1].equalsIgnoreCase("Mar"))
				res += 30 * 3;
			if (t[1].equalsIgnoreCase("Apr"))
				res += 30 * 4;
			if (t[1].equalsIgnoreCase("May"))
				res += 30 * 5;
			if (t[1].equalsIgnoreCase("Jun"))
				res += 30 * 6;
			if (t[1].equalsIgnoreCase("Jul"))
				res += 30 * 7;
			if (t[1].equalsIgnoreCase("Aug"))
				res += 30 * 8;
			if (t[1].equalsIgnoreCase("Sep"))
				res += 30 * 9;
			if (t[1].equalsIgnoreCase("Oct"))
				res += 30 * 10;
			if (t[1].equalsIgnoreCase("Nov"))
				res += 30 * 11;
			if (t[1].equalsIgnoreCase("Dec"))
				res += 30 * 12;
		}
		if (t.length > 2) {
	...
	public YahooTracker(String s[]) {
		...
		if (!nogui) {
			...
			System.out.println(";high value change stock ="
					+ dataList.getItemAt(greatestValueIndex()));
	...
	public void paintComponent(Graphics g) {
		...
		for (int a = 0; a < allSymbols.size(); a++) {
			...
			if (!s.equalsIgnoreCase(relsym) && single)
				continue;
		...
		for (int a = 0; a < allDates.size(); a++) {
			...
			if (a % spread == 0) {
				g.drawLine((int) (drawa * dx), 0, (int) (drawa * dx),
						getHeight() - 20);
				if (dat.equals(today))
					continue;
				if (isFriday(dat)) {
					...
					g.drawLine((int) (drawa * dx), 0, (int) (drawa * dx),
							getHeight() - 20);
					g.drawLine((int) (drawa * dx + 1), 0,
							(int) (drawa * dx + 1), getHeight() - 20);
		...
		for (int a = 0; a < getMaxIndex(); a++) {
			for (int b = 0; b < symlist.size(); b++) {
				...
				if (!sym.equalsIgnoreCase(relsym) && single)
					continue;
				double nex = getValue(sym, a);
				double las = nex;
				if (a != 0)
					las = getValue(sym, a - 1);
				int drawa = getMaxIndex() - a;
				g.drawLine((int) ((drawa) * dx), (int) ((maxY - las) * dy),
						(int) ((drawa - 1) * dx), (int) ((maxY - nex) * dy));
				g
						.drawLine((int) ((drawa) * dx), (int) ((maxY - las)
								* dy + 1), (int) ((drawa - 1) * dx),
								(int) ((maxY - nex) * dy + 1));
				g
						.drawLine((int) ((drawa) * dx - 1),
								(int) ((maxY - las) * dy), (int) ((drawa - 1)
										* dx - 1), (int) ((maxY - nex) * dy));
				g
						.drawLine((int) ((drawa) * dx), (int) ((maxY - las)
								* dy - 1), (int) ((drawa - 1) * dx),
								(int) ((maxY - nex) * dy - 1));
				...
				disval = (nex >= 0 ? "+" : "")
						+ disval.substring(0, (nex < 0 ? 5 : 4));
				disval = "" + disval + " " + sym;
				if (a == 0)
					g.drawString(disval, getWidth() - 80,
							(int) ((maxY - nex) * dy));
				if (a == 0)
					g.drawString(disval, getWidth() - 80 - 1,
							(int) ((maxY - nex) * dy));
				if (a == 0)
					g.drawString(disval, getWidth() - 80, (int) ((maxY - nex)
							* dy - 1));
			}
	...
	public int greatestValueIndex() {
		...
		for (int a = 0; a < dataList.getItemCount(); a++) {
			if (((String) dataList.getItemAt(a)).equalsIgnoreCase(relsym))
				return a;
		}
		return 0;
	}
	...
	public void readRealTime(String sym, int realInd) {
		...
		if (log)
			System.out.println(";realtime connecting " + sym);
		...
		while (all.indexOf(",") >= 0)
			all = all.substring(0, all.indexOf(","))
					+ all.substring(all.indexOf(",") + 1);

		if (all.indexOf("Last Trade:") >= 0) {
			all = all.substring(all.indexOf("Last Trade:") + 11).trim();
			if (all.indexOf(" ") >= 0) {
				try {
					double val = Double.parseDouble(all.substring(0,
							all.indexOf(" ")).trim());
					setValue(today, sym, val);
					if (log)
						System.out.println(";quote " + sym + " "
								+ all.substring(0, all.indexOf(" ")).trim()
								+ " rt" + " index=" + realInd);
				} catch (NumberFormatException nfe) {
					System.out.println(";MyError: realtime1 not available for "
							+ sym + " switching to 20min quote");
			...
			if (all.indexOf(" ") >= 0) {
				try {
					...
					System.out.println(";MyError: realtime2 not available for "
							+ sym + " switching to 20min quote");
					read20MinQuote(sym, realInd);
				}
			}
		} else {
			System.out.println(";MyError: realtime3 not available for " + sym
					+ " switching to 20min quote");
			read20MinQuote(sym, realInd);
		}
	}
	...
	public void read20MinQuote(String sym, int realInd) {
		...
		if (log)
			System.out.println(";20minQuote connecting " + sym);
		String all = readUrl(loadUrl);
		if (logALL)
			System.out.println(all);
		...
		if (all.indexOf("Last Trade:") >= 0) {
			...
			if (all.indexOf(" ") >= 0) {
				try {
					...
					setValue(today, sym, val);
					if (log)
						System.out.println(";20quote " + sym + " "
								+ all.substring(0, all.indexOf(" ")).trim()
								+ " rt" + " index=" + realInd);
				} catch (NumberFormatException nfe) {
	...
	public void readHistorical(String sym) {
		...
		if (log)
			System.out.println(";connecting " + sym);
		if (log)
			System.out.println(";downloading " + sym);
		String all = readUrl(loadUrl);
		if (logALL)
			System.out.println("ALL HIST received=" + all);
		all = aj.io.MLtoText.cutMLaddSpaces(all);
		if (histPage)
			System.out.println(";ML free>>\n" + all);
		...
		if (all.indexOf("Adj Close*") >= 0) {// html download only
			...
			if (histPage)
				System.out.println(";\"Adj Close*\" cut \n" + all);
			mm = aj.misc.Stuff.getTokens(all, "\n \t");
		}
		int nextIndex = 1;
		for (int a = 0; a < mm.length - 1; a++) {
			...
			if (mm[a].indexOf("-") != mm[a].lastIndexOf("-")) {// ie
				...
				while (mm[a + 6].indexOf(",") >= 0)
					mm[a + 6] = mm[a + 6].substring(0, mm[a + 6].indexOf(","))
							+ mm[a + 6].substring(mm[a + 6].indexOf(",") + 1);
				double val = 0;
				try {
				...
				System.out.print(".");
				if (log)
					System.out.println(";quote " + sym + " " + mm[a + 6]
							+ " on " + mm[a] + " index=" + nextIndex);
				if (mm[a].toUpperCase().indexOf(stopDate.toUpperCase()) >= 0)
					break;
				if (mm[a].toUpperCase().indexOf(stopDate2.toUpperCase()) >= 0)
					break;
				nextIndex++;
			}
		}
	...
	public double getValue(String sym, int index) {
		...
		String relStr = "aapl";
		if (dataList != null)
			relStr = (String) dataList.getSelectedItem();
		...
		// return
		// yd.getVal(original,single,percent,relitive,startVal,currRelVal);
	...
	public double getRawValue(String sym, int index) {
		...
		for (int a = 0; a < allValues.size(); a++) {
			...
			if (t.sym.equalsIgnoreCase(sym) && t.date.equalsIgnoreCase(d)) {
				// System.out.println("Raw Value of "+sym+" at "+index+" is
				// "+t.val);
				return t.val;
			}
		}
		...
		System.out.println("missed lookup FULL MISS for " + d + " with sym "
				+ sym);
		return 1;
	}
...
class Trip {
	...
	public Trip(String d, String s, double v) {
		val = v;
		sym = s;
		date = d;
	}
