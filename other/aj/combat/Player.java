...
public class Player implements KeyListener, MouseListener, MouseMotionListener {
	...
	private static String REPOPULATE = "rep", CREATE = "new ",
			DESTROY = "dest ", UPDATE = "mov ";
			...
			public void actionPerformed(ActionEvent e) {
				...
				okay.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (!scores.getPlayerName().equalsIgnoreCase(
	...
	private void fireLaser() {
		...
		if (lastfire + MINSHOTDELAY < System.currentTimeMillis()
				&& activeLaserCount < MAXLASERSHOTS) {
	...
	private void fireMissile() {
		...
		if (lastfire + MINSHOTDELAY < System.currentTimeMillis()
				&& activeMissileCount < MAXMISSILEHOTS) {
	...
	private void checkCollision() {
		for (int a = 0; a < allItems.size(); a++) {
			...
			// don't do asteroid check here, check only if it hits My stuff
			// below
			...
			if (myItems.contains(t)) {
				...
				for (int b = 0; b < allItems.size(); b++) {
					...
					if (myItems.contains(hit) && !(hit instanceof Asteroid))
						continue;
					if (t.inside(hit)) {
		...
		new Thread() {
			public void run() {
				try {
					...
					while (true) {
						Thread.sleep(System.currentTimeMillis() - last
								+ REDRAWDELAY);
			...
			public void run() {
				try {
					...
					if (r.indexOf("JOINED") >= 0 && id == -1)
						id = Integer.parseInt(r.substring(r.lastIndexOf(" "))
								.trim());
					p.join();
					while (socket != null) {
						Thread.yield();
					...
					System.out
							.println("MyError network message received error");
					e.printStackTrace();
				}
			}
		}.start();
	...
	public void keyReleased(KeyEvent evt) {
		char ch = Character.toUpperCase(evt.getKeyChar());
		if (currKeys.indexOf(ch) >= 0)
			currKeys = currKeys.substring(0, currKeys.indexOf(ch))
					+ currKeys.substring(currKeys.indexOf(ch) + 1);
	}
	...
	private void receiveUserCommand() {
		...
		if (currKeys.indexOf("X") >= 0 || currKeys.indexOf("Q") >= 0) {// ch ==
																		// 'X'
																		// || ch
																		// ==
																		// 'Q')
		...
		if (currKeys.indexOf("2") >= 0 || currKeys.indexOf("K") >= 0
				|| currKeys.indexOf("W") >= 0) {
			moveUp();
		}
		if (currKeys.indexOf("8") >= 0 || currKeys.indexOf("J") >= 0
				|| currKeys.indexOf("S") >= 0) {
			moveDown();
		}
		if (currKeys.indexOf("4") >= 0 || currKeys.indexOf("H") >= 0
				|| currKeys.indexOf("A") >= 0) {
			moveLeft();
		}
		if (currKeys.indexOf("6") >= 0 || currKeys.indexOf("L") >= 0
				|| currKeys.indexOf("D") >= 0) {
			moveRight();
		}
	...
	public void mouseDragged(MouseEvent e) {
		...
		double dir = Math.atan2(1.0 * p.y - mapView.getHeight() / 2.0, 1.0
				* p.x - mapView.getWidth() / 2.0);
		targetDir = dir;
	}
