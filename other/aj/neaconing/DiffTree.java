...
public class DiffTree {
	...
	public DiffTree() {
		...
		for (int a = 0; a < allLines.size(); a++) {
			...
			if (l.toUpperCase().startsWith("FINGERPRINT")) {
				...
				System.out.println("MyError: unhandeled text on line " + a + " " + l);
			}
		}
		if (f != null) {
			allFinger.addElement(f);
	...
	public void compair(String s) {
		...
		for (int a=0;a<allFinger.size();a++) {
			...
			if (g.name.equalsIgnoreCase(s)) {f=g;break;}
		}
		int MAXDIFF=7;
		int MAXSHOWDIFF=20;
		int diff[]=new int[MAXDIFF];
		if (f==null) {System.out.println("Cannot compair. "+s+" not found");return;}
//match rate table
...
//top 10 matches
		for (int a=0;a<MAXSHOWDIFF;a++) {
			...
			for (int b=1;b<allFinger.size();b++) {
				...
				if (FingerPrint.dist(f,h)<dd) {g=h;dd=ddd;}
			}
			if (f!=g) System.out.println(dd+" from "+g.sname+"\n"+FingerPrint.change(f,g));
