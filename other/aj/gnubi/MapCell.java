...
public class MapCell {
	...
	public Vector getAllBlocks(){
		...
		for (int a=0;a<array.length;a++) {
			...
			for (int b=0;b<array[a].length;b++) {
				if (array[a][b]!=null && array[a][b].isSolid()) v.addElement(array[a][b]);
			}
		}
		return v;
	}
	...
	public static MapCell createRandom() {
		...
		for (int a=0;a<defaultSize;a++){
			...
			for (int b=0;b<defaultSize;b++){
				...
				if (a==0 || b==0 || a==defaultSize-1 || b==defaultSize-1)
	...
	public Vector getAllBlocks(int cx, int cy, int i) {
		...
		for (int a=0;a<array.length;a++) {
			if (array[a]==null) continue;
			for (int b=0;b<array[a].length;b++) {
				if (b<cy+i && b>cy-i && a<cx+i && a>cx-i&&  array[a][b]!=null && array[a][b].isSolid()) v.addElement(array[a][b]);
			}
		}
		return v;
	}
