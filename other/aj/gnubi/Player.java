...
public class Player extends JComponent implements MapItem, Movable, KeyListener {
	...
	public int getCellX() {
		return (int) cellPos.getX();
	}

	public int getCellY() {
		return (int) cellPos.getY();
	}
	...
	public void doMove() {
		if (typed.indexOf("L") >= 0 || pressed.indexOf("L") >= 0)
			xv = Math.max(0, xv + ACCELL);
		if (typed.indexOf("H") >= 0 || pressed.indexOf("H") >= 0)
			xv = Math.min(0, xv - ACCELL);
		if (typed.indexOf("K") >= 0 || pressed.indexOf("K") >= 0)
			yv = Math.min(0, yv - ACCELL);
		if (typed.indexOf("J") >= 0 || pressed.indexOf("J") >= 0)
			yv = Math.max(0, yv + ACCELL);
		...
		if (xpos > .5) {
			xpos = xpos - 1;
			setCellPos((int) (cellPos.getX() + 1), (int) (cellPos.getY()));
		}
		if (xpos < -.5) {
			xpos = xpos + 1;
			setCellPos((int) (cellPos.getX() - 1), (int) (cellPos.getY()));
		...
		if (ypos > .5) {
			ypos = ypos - 1;
			setCellPos((int) (cellPos.getX()), (int) (cellPos.getY() + 1));
		}
		if (ypos < -.5) {
			ypos = ypos + 1;
			setCellPos((int) (cellPos.getX()), (int) (cellPos.getY() - 1));
