...
public class MyHost implements Serializable {
	...
	public String toSaveString() {
		return getIp() + ":" + port + " " + totalConnectAttempts + " "
				+ Stuff.trunc(successRate, 2) + " " + longestConnect + " "
				+ lastTry;
	}
	...
	public MyHost(String ip, int port, int att, double rate, int ctime,
			long last) {
		setIp(ip);
		this.port = port;
	...
	public String getIp() {
		return Header.fixByte(ipb[0]) + "." + Header.fixByte(ipb[1]) + "."
				+ Header.fixByte(ipb[2]) + "." + Header.fixByte(ipb[3]);
	}
	...
	public boolean tryConnect() {
		...
		if ((lastConnectTime != 0 && lastFailConnectTime != 0 && lastConnectLostTime != 0)
				|| lastSeenTime + NEVERCONNECTED > currtime) {
			return false;
		}
		return true;
	}
	...
	public int getRate() {
		if (connectCount - connectFailCount > MAXRATE)
			return MAXRATE;
		else if (connectCount - connectFailCount < -1 * MAXRATE)
			return -1 * MAXRATE;
		else
			return (connectCount - connectFailCount) % MAXRATE;
	}
	...
	public void goodConnect() {
		...
		successRate = (successRate * totalConnectAttempts + 1)
	...
	public void failedConnect() {
		...
		connectFailCount++;
		lastTry = System.currentTimeMillis();
		successRate = (successRate * totalConnectAttempts)
				/ (totalConnectAttempts + 1);
		totalConnectAttempts++;
	}
	...
	public void connectLost() {
		...
		connectLost++;
		if (lastDisconnectTime + MINCONNECTTIMEFORLOST < lastConnectTime)
			failedConnect();
		else {
	...
	public MyHost(String i, int por, Pong p) {
		...
		for (int a = 0; a < 4; a++) {
			try {
				...
				System.out.println("Bad number in IP position " + a + " of "
						+ s[a] + " in ip of " + i);
				ipb[a] = 0;
			}
		}
	...
	public String toString() {
		long maxtime = Math.max(lastConnectTime, Math.max(lastFailConnectTime,
				lastConnectLostTime));
		String vis = "";
		long dismaxtime = (System.currentTimeMillis() - maxtime) / 1000;
		if (maxtime < lastSeenTime && lastConnectTime == 0
				&& lastFailConnectTime == 0 && lastConnectLostTime == 0) {
			vis = "Never Age 0";
		} else if (maxtime == lastConnectLostTime)
			vis = "LOST Age " + dismaxtime;
		else if (maxtime == lastConnectTime)
			vis = "Recent Age " + dismaxtime;
		else if (maxtime == lastFailConnectTime)
			vis = "FAILED Age " + dismaxtime;
		String share = "files:" + numshareFiles + " at " + numshareMegabytes
				+ "MB";
		if (numshareFiles == 0)
			share = "";
		if (numhops > 0)
			share = " hops:" + numhops + " " + share;
		return getIp() + ":" + port + " " + vis + " connect (" + connectCount
				+ "/" + connectFailCount + ") " + share;
	}
