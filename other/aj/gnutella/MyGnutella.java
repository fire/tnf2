...
public class MyGnutella implements Runnable {
		...
		for (int a=0;a<subPortList.length;a++) {
			try {
				...
				for (int b=0;b<subPortList.length;b++) {
					if (b<a) t[b]=subPortList[b];
					if (b>a) t[b-1]=subPortList[b];
				}
				subPortList=t;
				a--;
			}
	...
	static boolean hosttimereset=false;//clears old LOST, FAILED and Recent connect times
	...
	public boolean alreadySentHeader(Header h,GnuProtocol gp) {
		...
		for (int a=0;a<v.size();a++) {
			...
			if (h2.equals(h)) sentBefore=true;;
		}
		if (sentBefore) {
			gp.dropSentBefore++;
			return true;
	...
	public void sendHeader(Header h, GnuProtocol gp) {
		gp.send(h);
		if (!sentHeaders.contains(h)) sentHeaders.addElement(h);
	...
	//MyHost.RECONNECTTIME=6000;
		...
		if (s.length==0 || s[0].indexOf("?")>=0) {
			...
			System.out.println("   -OTHERPONG Reply to ping with someone elses pong");
			System.out.println("   -NOGUI no gui support");
			System.out.println("   -LOG<PING|PONG|QUERY|HIT|PUSH|UNK|CONNECT|FILTER>");
			...
			System.out.println("   -LIESHARE lie about number and size of share");
			System.out.println("   -HOSTTIMERESET drops all delays for connecting to hosts");
	...
	public static String[] getOptions(String s[]) {
		for (int a=0;a<s.length;a++) {
			...
			if (s[a].equalsIgnoreCase("-HOSTTIMERESET")) {
				...
				hosttimereset=true;
				for (int b=a+1;b<s.length;b++) t[b-1]=s[b];
				s=t;a--;
			}
	...
	public void run() {
		while (true) {
			...
			for (int a=0;a<v.size();a++) {
				...
				if (gp.active) gp.sendPing();
			}
			int activeCount=0;
			for (int a=0;a<v.size();a++) {
				GnuProtocol gp=(GnuProtocol)v.elementAt(a);
				gp.checkTimeOut();
				if (gp.active) {activeCount++;}
				if (gp.bad) {
					killGnuThread(gp);
					//System.out.println("Dead connection. "+gp.mh.getIp()+":"+gp.mh.port+" dropping!");
				}
			}
	...
	public MyHost getBestHost() {
		...
		while (nogood) {
			count++;
			if (count>hostList.size()) return null;
			...
			if (mh.getIp().equals(myip)) continue;
			if (!mh.tryConnect()) continue;
			double chance=(1.0*mh.connectCount+1)/(mh.connectFailCount+1);
			if (chance<1) {
				if (Math.random()>chance) {
				//System.out.println("Skipping!  Low available ="+Stuff.trunc(chance,2));
					continue;
				}
				//System.out.println("Trying!  Low available ="+Stuff.trunc(chance,2));
			}
			boolean fresh=true;
...
//key 1 = connect count,  #2 last seen, #3 lowest failcount
		...
		connectedList.addElement(gp);
		connectedThreadList.addElement(ttt);
		//System.out.println("New auto connect to "+mh.getIp()+":"+mh.port+" connectedList size= "+connectedList.size());
		return true;
	}
	...
	private void readHostFile(String hostfilename) {
		try {
			...
			BufferedReader br=new BufferedReader(new FileReader(shareDir+hostfilename));
	...
	public boolean validMyHost(MyHost mh) {
		if (mh.getIp().startsWith("10.") || mh.getIp().startsWith("192.") || mh.getIp().startsWith("127.") || 
			mh.getIp().startsWith("0.") || mh.port<1 || mh.port>65535) return false;
		return true;
	}

	long lastWrite=System.currentTimeMillis();
	public synchronized void writeHostList() {
		if (lastWrite+BACKUPHOSTTIME>System.currentTimeMillis()) return;
	...
	public void addHost(String ip,int port,Pong p) {
		...
		for (int a=0;a<v.size();a++) {
			...
			if (mh.equals(mh2)) {
				mh2.foundPong(p);
				if (mh2.port!=port) {
//System.out.println("New port found for host "+ip+":"+port);
					mh2.port=port;
				}
	...
	public Ping getSeenPing(String desId) {
		for (int a=0;a<seenPings.size();a++) {
			...
			if (p.desId.equals(desId)) return p;
		}
		return null;
	}
	...
	public QueryHit getSeenQueryHit(String servantid) {
		for (int a=0;a<seenQueryHit.size();a++) {
			...
			if (p.servantid.equals(servantid)) return p;
		}
		return null;
	}
	...
	public void receiveReply(Header h, GnuProtocol gp) {
		if (h instanceof Pong) {
			gp.myPongCount++;
			if (logToMe) System.out.println("REPLY: Pong receive in reply to my Ping found");
		}
		else if (h instanceof QueryHit) {
			if (logToMe) System.out.println("REPLY: QueryHit receive in reply to my Query");
		}
		else if (h instanceof Push) {
			if (logToMe) System.out.println("REPLY: Push received at request froma QueryHit I sent");
		}
		else {
			gp.dropLoop++;
	...
	public void forward(Header h, GnuProtocol gp) {
		...
		if (alreadySentHeader(h2,gp)) {return;}
			//System.out.println("Dropping TTL");
		String type="";
		if (h instanceof Ping) {
			if (logping) System.out.println(">>Ping found desId="+aj.io.Encode.encodeString(h.desId,"0123456789ABCDEF").substring(17));
			type="Ping";
			...
			Vector v=(Vector)connectedList.clone();
			gp.pingCount++;
			if (logping && logrout) System.out.println("forwarding "+type+" to "+(v.size()-1)+" hosts");
			for (int a=0;a<v.size();a++) {
				GnuProtocol gp2=(GnuProtocol)v.elementAt(a);
				if (gp2.bad) {
					killGnuThread(gp2);
					if (logping && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
					continue;
				}
				if (!gp2.active) continue;
				if (gp2==gp) continue;
				if (logping && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
				sendHeader(h2,gp2);
			}
			return;
		}
			...
			type="Pong";
			if (logpong) System.out.println(">>Pong found");
			Ping p=getSeenPing(h.desId);
			if (p==null) {
				gp.dropUnseen++;
				if (logpong) System.out.println("Unseen ping for pong found "+aj.io.Encode.encodeString(h.desId,"0123456789ABCDEF").substring(17));
				return;
			}
			gp.pongCount++;
			Pong PP=(Pong)h;
			if (!validPort(PP.port) && filterPong){
				gp.filterPort++;
				if (logFilter) System.out.println("port filtered on Pong forward "+PP.port);
				return;
			}
			if (!p.gp.bad && p.gp.active) {
				if (logpong && logrout) System.out.println("forwarding "+type+" to 1 hosts");
				if (logpong && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
				sendHeader(h2,p.gp);
			}
			else if (!p.gp.bad) {
				if (logpong && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
			}
			return;
		}
		else if (h instanceof Push) {
			type="Push";
			Push pp=(Push)h;
			if (logpush) System.out.println(">>Push found"+pp);
			QueryHit p=getSeenQueryHit(pp.servantid);
			if (p==null) {
				gp.dropUnseen++;
				if (logpush) System.out.println("Unseen QueryHit for Push found");
				return;
			}
			gp.pushCount++;
			Push PP=(Push)h;
			if (!validPort(PP.port) && filterPush){
				gp.filterPort++;
				if (logFilter) System.out.println("port filtered on Push forward "+PP.port);
				return;
			}
			if (!p.gp.bad && p.gp.active) {
				if (logpush && logrout) System.out.println("forwarding "+type+" to 1 hosts");
				if (logpush && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
				sendHeader(h2,p.gp);
			}
			else if (!p.gp.bad) {
				if (logpush && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
			}
			return;
		}
		else if (h instanceof QueryHit) {
			...
			if (logqueryhit) System.out.println(">>QueryHit found: "+((QueryHit)h));
			...
			if (p==null) {
				gp.dropUnseen++;
				if (logqueryhit) System.out.println("Unseen Query for QueryHit found");
				return;
			}
			gp.queryHitCount++;
			...
			if (!validPort(QQ.port) && filterHit){
				gp.filterPort++;
				if (logFilter) System.out.println("port filtered on QueryHit forward "+QQ.port);
				return;
			}
			if (!p.gp.bad && p.gp.active) {
				if (logqueryhit && logrout) System.out.println("forwarding "+type+" to 1 hosts");
				if (logqueryhit && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
				sendHeader(h2,p.gp);
			}
			else if (!p.gp.bad) {
				if (logqueryhit && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+p.gp.mh.getIp()+":"+p.gp.mh.port);
			}
			return;
		}
			...
			if (logquery) System.out.println(">>Query found: "+((Query)h));
			...
			gp.queryCount++;
			Vector v=(Vector)connectedList.clone();
			if (logquery && logrout) System.out.println("forwarding "+type+" to "+(v.size()-1)+" hosts");
			for (int a=0;a<v.size();a++) {
				GnuProtocol gp2=(GnuProtocol)v.elementAt(a);
				if (gp2.bad) {
					killGnuThread(gp2);
					if (logquery && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
					continue;
				}
				if (!gp2.active) continue;
				if (gp2==gp) continue;
				if (logquery && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
				sendHeader(h2,gp2);
			}
			return;
		}
			...
			type="Unknown";
			Vector v=(Vector)connectedList.clone();
			if (logunk && logrout) System.out.println("forwarding "+type+" to "+(v.size()-1)+" hosts");
			for (int a=0;a<v.size();a++) {
				GnuProtocol gp2=(GnuProtocol)v.elementAt(a);
				if (gp2.bad) {
					killGnuThread(gp2);
					if (logunk && logrout) System.out.println("  Cannot relay connection down.  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
					continue;
				}
				if (!gp2.active) continue;
				if (gp2==gp) continue;
				if (logunk && logrout) System.out.println("  Sending "+type+" from "+gp.mh.getIp()+":"+gp.mh.port+" to "+gp2.mh.getIp()+":"+gp2.mh.port);
				sendHeader(h2,gp2);
			}
		}
