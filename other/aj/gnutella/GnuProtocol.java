...
public class GnuProtocol implements Runnable{
	...
	int dropTTL=0,dropLoop=0,dropUnseen=0,dropSentBefore=0,dropBand=0;
	int filterPort=0;
	boolean bad=false;
	...
	public void sendPing() {
		if (lastPingTime+MINPINGDELAY>=System.currentTimeMillis()) return;
		recoverCount=0;
		...
		try {
			...
			//System.out.println("MyError: cannot send ping close connection "+ioe);
			killSelf(true);
		}
	}
//HERE
	...
	public void sendPong(String desId) {
		...
		//System.out.println("Sending Ping/Pong reply to "+s.getRemoteSocketAddress().toString());
		if (MyGnutella.otherpong) {
			if (mg.otherPongSave==null) return;//dont send one, don't have one yet
		...
		try {
			...
			//System.out.println("MyError: cannot send pong close connection "+ioe);
			killSelf(true);
		}
	}
	...
	public void send(Header h) {
		if (h!=null && !bad && oo!=null) {
			...
			try {
				...
				System.out.println("MyError: Unable to write "+ip+":"+port+" connection closed "+e);
				killSelf(true);
			}
		}
		else if (oo==null) {
...
*/
	public void connectServer(){
		if (MyGnutella.logconnect) System.out.println("Try connect v0.6 from "+ip+":"+port);
		try {
			...
			while (true) {
				...
				if (t.trim().equalsIgnoreCase("")) break;
				tt.addElement(t);
			...
			if (tt.size()>0) {
				...
				if (!log.toUpperCase().startsWith("GNUTELLA CONNECT/")) {
					oo.write("GNUTELLA/0.6 400 UNKNOWN PROTOCOL\n\n".getBytes());
					killSelf(true);
					return;
				}
			...
			if (ver==.4){
				...
				while (true) {
					...
					if (t.trim().toUpperCase().startsWith("GNUTELLA/") && t.trim().endsWith("200 OK")) {
						}
	...
	public void connectClientv6(){
		if (MyGnutella.logconnect) System.out.println("Try connect v0.6 to "+ip+":"+port);
		try {
			...
			while (true) {
				String t=myReadLine(ii);
				active=true;
				if (t==null) {killSelf(false);return;}
				if (t.trim().equals("")) break;
				countTraffic(t.length());
			...
			if (log.toUpperCase().trim().startsWith("GNUTELLA/") && log.toUpperCase().trim().endsWith("200 OK")) {
				connectVersion=log.trim().substring("GNUTELLA/".length());
				connectVersion=connectVersion.substring(0,connectVersion.indexOf(" OK")).trim();
				oo.write("GNUTELLA/0.6 200 OK\n\n".getBytes());
				oo.flush();
				sendPing();
				mh.goodConnect();
				mg.writeHostList();
				if (MyGnutella.logconnect) System.out.println("Good v0.6 Connect to "+ip+":"+port+" total="+mg.connectedList.size());
			}
				...
				mh.goodConnect();
				mg.writeHostList();
				if (MyGnutella.logconnect) System.out.println("Lazy v0.4 Connect to "+ip+":"+port+" total="+mg.connectedList.size());
			}
				...
				if (MyGnutella.logconnect) System.out.println("Bad v0.6 Connect to "+ip+":"+port+" reply of "+log);
				s.close();
				connectClientv4();
				return;
			}
		} catch (IOException ioe) {
			//System.out.println("MyError: IOError in connection.  Bad Connect to "+ip+":"+port+" dropping: "+ioe);
			killSelf(false);
		}
	}
...
*/
	public void connectClientv4(){
		if (MyGnutella.logconnect) System.out.println("Try connect v0.4 to "+ip+":"+port);
		try {
			tryconnectTime=System.currentTimeMillis();
			s=new Socket(ip,port);
			ii=s.getInputStream();
			...
			while (check!=b.length && check>0) {
				...
				//System.out.println("WARNING read mismatch "+b.length+" != "+check);
			}
			...
			if (!new String(b).toUpperCase().trim().equals("GNUTELLA OK")) {
				...
				if (MyGnutella.logconnect) System.out.println("Good Connect to "+ip+":"+port+" total="+mg.connectedList.size());
			}
		} catch (IOException ioe) {
			killSelf(false);
		}
	...
	public void killSelf(boolean lost) {
		if (!bad) {
			if (MyGnutella.logconnect && lost) System.out.println("Connection lost "+ip+":"+port);
			else if (MyGnutella.logconnect) System.out.println("Connection failed "+ip+":"+port);
			bad=true;
			if (!lost) {
				mh.failedConnect();
			}
	...
	public void checkTimeOut(){
		if (connectTime==0 && System.currentTimeMillis()-tryconnectTime>MAXTIMEOUT) {
			killSelf(false);
		}
	}
	public void run () {
		...
		try {
			while (!bad) {
				...
				countTraffic(b.length);
				active=true;
				int l=(b[19]<0?256+b[19]:b[19])+(b[20]<0?256+b[20]:b[20])*256+(b[21]<0?256+b[21]:b[21])*256*256+(b[22]<0?256+b[22]:b[22])*256*256*256;
				if (l<0 || l>MAXPAYLOAD) {//avoid hug payload memory leaks
					String type="UNK";
					if (b[16]==0x00) type="PING";
					if (b[16]==0x01) type="PONG";
					if (b[16]==0x40) type="PUSH";
					if (b[16]==0x80) type="QUERY";
					if (b[16]==0x81) type="QUERYHIT";
					System.out.println("WARNING : Bad payload length "+type+" found "+l+" "+b[19]+" "+b[20]+" "+b[21]+" "+b[22]+" on "+ip+":"+port+" b=0x"+aj.io.Encode.encodeString(new String(b),"0123456789ABCDEF").substring(17));
				...
				while (check!=payload.length && check>0) {
					...
					//System.out.println("WARNING read mismatch "+payload.length+" != "+check);
				}
				inMessages++;
				...
				if (b[16]==0x00 && payload.length==0) {//ping
					...
					if (pay.length()>80) pay=pay.substring(80)+"...trunc";
					System.out.println("MyError: Unknown type found: b=0x"+aj.io.Encode.encodeString(new String(b),"0123456789ABCDEF").substring(17)+" payload=0x"+aj.io.Encode.encodeString(new String(payload),"0123456789ABCDEF").substring(17));
					recover(ii);//try recover
				}
			}
		} catch (IOException ioe) {
			System.out.println("MyError: Unable to read "+ip+":"+port+" protocol lost "+ioe);
			killSelf(true);
		}
	}

	public String toString() {
		String res=""+myTime((System.currentTimeMillis()-connectTime)/1000);
		if (!active || bad) {
			if (tryconnectTime==0) res="0";
			else res=myTime((System.currentTimeMillis()-tryconnectTime)/1000);
		}
		if (isServer) res+=" Called...";
		else if (active && !bad) res+=" Active..v"+connectVersion;
		else if (!bad) res+=" Waiting.";
		else res+=" Failed..";
		res+=" "+ip+":"+port;
		if (active&& !bad) {
			...
			res+=" query:"+queryCount+" qhit:"+queryHitCount+" push:"+pushCount;
			res+=" unk:"+unknownCount;
			res+=" drop (ttl:"+dropTTL+" lp:"+dropLoop+" un:"+dropUnseen+" dup:"+dropSentBefore+" band:"+dropBand+" port:"+filterPort+")";
			res+=" I("+inMessages+" "+Stuff.trunc(lastTrafficRate,1)+"/"+Stuff.trunc(totalTrafficRate,1)+")";
			res+=" O("+outMessages+" "+Stuff.trunc(lastOutTrafficRate,1)+"/"+Stuff.trunc(totalOutTrafficRate,1)+")";
		}
		return res;
	}
	...
	public String myReadLine(InputStream ii) throws IOException{
		...
		while (true) {
			int c=ii.read();
			countTraffic(1);
			if (tryconnectTime>0 && connectTime==0) connectTime=System.currentTimeMillis();
			if (c==-1) break;
			res+=(char)c;
			if (c=='\n') return res;
		}
	...
	public void recover(InputStream ii) {
		recoverCount++;
		if (recoverCount>MAXRECOVERATTEMPTS) {killSelf(true);return;}
		...
		while (true) {
			...
			numread++;
			if (numread>4000) {killSelf(true);}
			if (numread>=24) {
				if (b[23]==0 && b[22]==0 && b[21]==0 && b[20]==0 && b[19]==0 && b[18]==14 && b[17]<Header.MAXTTL && b[16]<Header.MAXTTL) {
				...
				if (b[23]==0 && b[22]==0 && b[21]==0 && b[20]==0 && b[19]<Header.MAXTTL && b[18]<Header.MAXTTL) {
					System.out.println("Recover Ping Guessed");
					return;
				}
				if (b[23]==0 && b[22]==0 && b[21]==0 && b[20]==0 && b[19]==0 && b[18]>2 && b[17]<Header.MAXTTL && b[16]<Header.MAXTTL) {
