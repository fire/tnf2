...
public class Cards {
	...
	public void play() {
		...
		for (int a = 0; a < numberHandsToPlay; a++) {
			...
			System.out.println("Player= (" + display(playerCards) + ")="
					+ scoreHand(playerCards) + " Dealer=("
					+ display(dealerCards) + ")=" + scoreHand(dealerCards)
					+ " "
					+ (firstHandWins(dealerCards, playerCards) ? "DW" : "PW"));
		}
		System.out
				.println("Games Played=" + hands + " dealerWins=" + dealerWin);
	}
	...
	public boolean playerHit(int c[], int d) {
		...
		if (ha && sc == 13 && d != 5 && d != 6)
			return true;
		if (ha && sc == 14 && d != 4 && d != 5 && d != 6)
			return true;
		if (ha && sc == 15 && d != 4 && d != 5 && d != 6)
			return true;
		if (ha && sc == 16 && d != 4 && d != 5 && d != 6)
			return true;
		if (ha && sc == 17 && (d == 2 || d > 6))
			return true;
		if (ha && sc == 18 && d > 9)
	...
	public boolean firstHandWins(int c[], int d[]) {
		if (scoreHand(c) <= 21
				&& (scoreHand(d) < scoreHand(c) || scoreHand(d) > 21))
			return true;
		return false;
	}
...
/*
 * table delear players[]
 * 
 * game deck hands
 * 
 * hand player bet cards
 * 
 * player bank hand
 */
