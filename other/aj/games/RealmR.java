...
public class RealmR {
	...
	public static void displayHelp() {
		...
		System.out
				.println("-s <serverport>  wait for server connect to local port");
	...
	public static void main(String s[]) {
		...
		for (int a = 0; a < s.length; a++) {
			...
			if (s[a].startsWith("?")) {
				...
				try {
					...
					System.out
							.println("Passive Server mode.  Waiting for server call.");
		...
		if (playername == null) {
			System.out
					.println("MyError: missing username.   Please use ? for help.");
			System.exit(0);
		}
