...
public class RealmC implements KeyListener,ActionListener {
	...
	public static void displayHelp() {
		...
		System.out.println ("-s <serverport>  wait for server connect to local port (default mode)");
		System.out.println ("-h <host[:port]>   client connect to host ip at port");
		System.out.println ("-g             use text (no GUI) mode (requires hit or winhit");
	...
	public static void main (String s[]) {
		...
		for (int a = 0; a < s.length; a++) {
			...
			if (s[a].startsWith ("?")) {
				...
				try {
					...
					System.out.println ("Passive Server mode.  Waiting for server call on port " + port + ".");
		...
		if (nl == null) {
			try {
				System.out.println ("Passive Server mode.  Waiting for server call on port " + DEFAULTCLIENTPORT + ".");
				ServerSocket ss = new ServerSocket (DEFAULTCLIENTPORT);
				Socket sss = ss.accept();
				nl = sss;
			}
			catch (IOException IOE) {
				System.out.println ("MyError: bad network connection.   Please use ? for help. 2");
			}
		}
		if (nl == null) {
		...
		if (playername == null)new RealmC (nl, "User" + ((int) (Math.random() * 100)));
		else new RealmC (nl, playername);
	}
	...
	public RealmC (final Socket nl, String name, Applet a) {
		...
		a.add ("Center", dis);
		if (SYSOPMODE)a.add ("South", sysop);
	...
	public RealmC (final Socket nl, String name) {
		...
		if (GUI) {
			...
			f.add ("Center", dis);
			if (SYSOPMODE)f.add ("South", sysop);
			f.addWindowListener (new SimpleWindowManager());
			f.pack();
			f.setVisible (true);
		}
		if (SYSOPMODE || !GUI) {
			InputListener il = new InputListener (new BufferedReader (new InputStreamReader (System.in)));
	...
	public void updateDisplay() {
		...
		if (screen == null) {
			...
			for (int a = 0; a < SCREENY * SCREENX; a++)screen[a] = ' ';
		}
		for (int a = 0; a < SCREENY; a++)screen[a * SCREENX + SCREENX - 1] = '\n';
		for (int a = - 1 * VIEW; a < VIEW + 1; a++) {
			for (int b = - 1 * VIEW; b < VIEW + 1; b++) {
				if (y + a >= 0 && y + a < map.length && x + b < map[y + a].length && x + b >= 0) {
					screen[ (VIEW + a + VIEWYPOS) * SCREENX + VIEW + b + VIEWXPOS] = map[y + a][x + b];
				}
				else {
					screen[ (VIEW + a + VIEWYPOS) * SCREENX + VIEW + b + VIEWXPOS] = ' ';
				}
			}
		}
		...
		for (int a = 0; a < users.size(); a++) {
			...
			if (ru.name.equals (name) || ru.status == 0)continue;
			...
			if (Math.abs (showx) <= VIEW && Math.abs (showy) <= VIEW) {
				...
				screen[ (VIEW + showy + VIEWYPOS) * SCREENX + VIEW + showx + VIEWXPOS] = ru.dir;
				if (youSee.length() == 0)youSee += "You see ";
				else youSee += ", ";
				youSee += ru.name;
			}
			else if (Math.abs (showx) <= SENSERAN && Math.abs (showy) <= SENSERAN) {
				if (youSense.length() == 0)youSense += "You sense someone near you";
			}
		}
		if (status != 0)screen[ (VIEW + 0 + VIEWYPOS) * SCREENX + VIEW + 0 + VIEWXPOS] = dir;
		...
		if (youSee.length() > 0 && status != 0) {
			scr = scr.substring (0, scr.lastIndexOf ("\n")) + youSee + scr.substring (scr.lastIndexOf ("\n"));
		}
		else if (youSense.length() > 0 && status != 0) {
			scr = scr.substring (0, scr.lastIndexOf ("\n")) + youSense + scr.substring (scr.lastIndexOf ("\n"));
		}
		if (GUI) {
	...
	public void keyTyped (KeyEvent e) {
		...
		if (lastcommandtime + MINTIME > System.currentTimeMillis()) {
			//System.out.println("NOTE users must wait "+MINTIME+" NOT "+(System.currentTimeMillis()-lastcommandtime));
			return;
		}
	...
	public void sendMove (char c) {
		...
		else if (c != '2' && c != '4' && c != '6' && c != '8' && c != '5' && c != '0') {
			//System.out.println("NOTE users must key 2,4,6,8,5,0 (also h,j,k,l) not "+c);
			return;
		}
		if (status == 0 && c != '0') {
	...
	public void actionPerformed (Socket S,String c) {
		...
		if (c.trim().equalsIgnoreCase ("connection_closed")) {
			...
			if (c.toUpperCase().indexOf ("LOGGEDIN") >= 0) {
				name = c.substring (c.toUpperCase().indexOf ("LOGGEDIN") + 8).trim();
			}
	...
	public void relogin() {
		if (loggedOut)send(nl,"LOGIN " + name);
		loggedOut = false;
	}
}
