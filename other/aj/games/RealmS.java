...
public class RealmS   {
	...
	int UPDATETIME = 400;//MILLIS TILL AUTOTURN PASS AND SEND UPDATE  200= 5/sec
	int SHOTRANGE = 3;
	...
	public static void main (String s[]) {
		...
		for (int a = 0; a < s.length; a++) {
			...
			if (s[a].toLowerCase().startsWith ("p")) {
				s[a] = s[a].substring (1);
				if (s[a].length() < 1 && s.length - 1 > a)a++;
				try {
					rs = new RealmS (Integer.parseInt (s[a]));
				}
				catch (NumberFormatException nfe) {
					System.out.println ("MyError: bad number in port command." + s[a]);
					System.exit (0);
				}
			}
		}
	...
	public void addCalledUser (String host, int port) {
		try {
			...
			System.out.println ("MyError: cannot connect to user at " + host + " " + port);
		}
	}
	...
	public void makeDefaultMap() {
		...
		for (int a = 0; a < MAPSIZE; a++) {
			...
			for (int b = 0; b < MAPSIZE; b++) {
				map[a][b] = ' ';
				if (a == 0 || b == 0 || a == MAPSIZE - 1 || b == MAPSIZE - 1)map[a][b] = 'X';
			}
		}
		buildMapSend();
	}
	...
	public void actionPerformed (Socket nl,String c) {
		...
		if (true) {
			...
			if (c.toLowerCase().trim().equalsIgnoreCase ("connection_closed")) {
				//loggin out
				//System.out.println("NOTE user logging out "+ae.getActionCommand());
				...
				int port = Integer.parseInt (c.substring (c.indexOf (":") + 1).trim());
				...
				if (me == null) {
					System.out.println ("MyError: Move found with no user.  Ignoring command :" + c);
					return;
				}
				//check user time
				if ( !readyToMove (me)) {
					//System.out.println("NOTE: user not allowd to move.  Too fast.");
					return;
				}
				char cmd = c.charAt (5);
				if (cmd == '2' || cmd == '4' || cmd == '6' || cmd == '8') {
					...
					//check map collision
					if (STAYONMAP && ! (y >= 0 && x >= 0 && y < map.length && x < map[y].length)) {
						//System.out.println("NOTE moved off map not allowed");
						y = me.y;
						x = me.x;
					}
					if (OBSTICALSBLOCK && y >= 0 && x >= 0 && y < map.length && x < map[y].length && map[y][x] != ' ') {
						//System.out.println("NOTE movement over obsticals not allowed");
						y = me.y;
						x = me.x;
					}
					...
					for (int a = 0; a < users.size(); a++) {
						RealmU ru = (RealmU)users.elementAt (a);
						if (ru.x == x && ru.y == y && ru != me && ru.status != 0 && NOJUMPING) {
							//System.out.println("NOTE movement over users not allowed");
							y = me.y;
							x = me.x;
						}
					}
					...
					for (int a = 1; a < SHOTRANGE; a++) {
						...
						if (ONMAPONLYSHOT && ! (yy >= 0 && xx >= 0 && yy < map.length && xx < map[yy].length)) {
							//System.out.println("NOTE stop bullit leaving map");
							//REFLECTSHOT
							break;
						}
						if (WALLSTOPSHOT && yy >= 0 && xx >= 0 && yy < map.length && xx < map[yy].length && map[yy][xx] != ' ') {
						...
						for (int b = 0; b < users.size(); b++) {
							...
							if (ru.x == xx && ru.y == yy && (ru != me || KILLSELFALLOWED)) {
								ru.status = 0;
								changed = true;
								//ru.killer=me.name;
								if ( !MULTIKILLSHOT)break;
							}
						}
					}
					if ( !changed) {
				...
				System.out.println ("MyError: Connect to self.  Received map. Killing connection. " + c);
	...
	public boolean readyToMove (RealmU me) {
		if (me.turnNumber < currTurn) {
			//System.out.println("NOTE "+me.name+" moving in old turn "+currTurn);
			return true;
		...
		if (me.lastMoveTime + UPDATETIME < System.currentTimeMillis()) {
			...
			currTurn++;
			//System.out.println("NOTE user "+me.name+" moving UPDATETIME passed.  New turn "+currTurn);
			sendUpdate();
			return true;
		}
		...
		for (int a = 0; a < users.size(); a++) {
			...
			if (ru.turnNumber < currTurn && ru.status != 0) {
				//System.out.println("NOTE waiting for user "+ru.name+" to finish turn "+currTurn+" still has "+((me.lastMoveTime+UPDATETIME)-System.currentTimeMillis()));
				alldone = false;
				break;
			}
		}
		if (alldone) {
			currTurn++;
			//System.out.println("NOTE all users moved early. New turn "+currTurn+" Actual turn time="+(System.currentTimeMillis()-me.lastMoveTime));
			sendUpdate();
		}
		return alldone;
	}
...
user -
  logout            (based on connection)
  login <name>      (one login per connection)
  move <2,4,6,8,5,0>
  call <ip[:port]>
  speed <millis>
  ghost <name>          //login with dead status


server -
  M <rows> <cols> <data>
  U <users>[' '<users>]*
  # echo notes (including LOGIN FAIL, CALLING, SPEED SET)
...
status -
  ...
  passwall (points)
  offmap
  shootpasswall (points)
  shootoffmap
  change map (points and bandwidth warning)
  hitpoints (1-10)
  shieldpoints (1-10)
  non-reporting (sysop no datamode)
  score  
  team
  shotlimit
...
flags
  ...
  add shieldpoint
  add shotlimit
  add clone/onelife robot helper
  kill all opponents (capture flag)
  kill non-team
  create new flag
  increase sense/vis
  increase shotrange
  add multi shot - rev/lef/rig
  resurect point (nopickup)
  nextflag point (nopickup)
  kill self (mine)
  
  flags invisible or visible
  flags pickup or nopickup
  flags repeating, flag group, replace, use up
