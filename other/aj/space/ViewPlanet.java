	...
	public ViewPlanet(Vector v, GmlPair n) {
		...
		if (g != null) {
			...
			if (g.getString()==null) {
				...
				color=new Color((int)(gr.getDouble()),(int)(gg.getDouble()),(int)(gb.getDouble()));
			}
			else {
				if (g.getString().equalsIgnoreCase("YELLOW")) color=Color.yellow;
				if (g.getString().equalsIgnoreCase("RED")) color=Color.red;
				if (g.getString().equalsIgnoreCase("BLUE")) color=Color.blue;
				if (g.getString().equalsIgnoreCase("GREEN")) color=Color.green;
				if (g.getString().equalsIgnoreCase("PINK")) color=Color.pink;
//				if (g.getString().equalsIgnoreCase("YELLOW")) color=yellow;
		...
		if (orbitsPlanet != null) {
			period = 2 * Math.PI * orbitRadious / Math.pow(6.67e-11 * orbitsPlanet.mass / orbitRadious, .5);
		}
		double sa=4*Math.PI*Math.pow(radious,2);
		//int ts=(int)(sa/3.11e10*4);// based on earth 256x64 map (sector size=64) total square = 16384,
		int ts=(int)(sa/3.11e10*4);// based on earth 512x128 map (sector size=128) total square = 16384x4,
	...
	public DPoint getMax() {
		if (!isMoon()) {
			...
			return new DPoint(c.getX() + orbitRadious * Math.cos(0), 
					c.getY() + orbitRadious * Math.sin(0));
		}
	}
	public DPoint getMin() {
		if (!isMoon()) {
			...
			return new DPoint(c.getX() + orbitRadious * Math.cos(180), 
					c.getY() + orbitRadious * Math.sin(180));
		}
	}
	public DPoint getCenter(long time) {
		if (!isMoon()) {
			...
			double localtime = ((time + name.hashCode() * 5) % (int) period) / period * 2 * Math.PI;
			return new DPoint(c.getX() + orbitRadious * Math.cos(localtime), 
					c.getY() + orbitRadious * Math.sin(localtime));
		}
	}


	public void drawOrbit(Graphics g, double dx, double dy, double scale, double or, long time) {
/*
		DPoint d = getCenter(time);
		int x = (int) ((d.getX() - dx) * scale);
		int y = (int) ((d.getY() - dy) * scale);
		int r = Math.max(1, (int) (or * scale));
		g.setColor(Color.lightGray);
		if (r <= Gravity.MAXOVAL && r > 5) {
...
*/
	...
	public void draw(Graphics g, double dx, double dy, double scale, long time) {
		...
		if (x < Gravity.MAXVIEW && y < Gravity.MAXVIEW && x > -Gravity.MAXVIEW && y > -Gravity.MAXVIEW) {
			g.setColor(color);
