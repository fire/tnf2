...
public class Gravity extends Canvas implements MouseListener, MouseMotionListener {
	...
	public Gravity() {
		...
		try {
			...
			if (f.canRead()) g=GmlPair.parse(new File(file));
			else {
...
/*
double dx=(dloc.getX()-dl.getX());
        double dy=(dloc.getY()-dl.getY());
        double d=Math.pow((dx*dx)+(dy*dy),.5);
int t=(int)(Math.pow(d/.0098,.5)*4)*1000;
t=(int)Math.max(t,2*DAY);
return t;

//earth to moon= 3 days
//earth to mars = 12-20 days
t=2 days= 60*60*24*2 =172800
d=4.5e8
a=?
d=.5at^2
4.5e8*2/t^2
...
//			int maxy = (int) Math.max(downP.getY() + offP.getX(), downP.getY() + offP.getY());
	...
	public void mouseClicked(MouseEvent mouseEvent) {
		...
		if ((mouseEvent.getModifiers() & InputEvent.BUTTON3_MASK) != 0) {
			...
			centerAtNearestPlanetToScreenPos(new Point(mouseEvent.getX(), mouseEvent.getY()));
...
//		int maxy = (int) Math.max(downP.getY() + offP.getX(), downP.getY() + offP.getY());
		...
		if (offP.getX() > 4 && offP.getY()>4 ) {
			if (downButton3) {
				centerAtScreenPos(new Point((int) downP.getX(), (int) downP.getY()));
			}
			else if (downButton1) {
				centerAtNearestPlanetToScreenPos(new Point((int) downP.getX(), (int) downP.getY()));
			}
