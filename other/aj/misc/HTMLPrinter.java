...
public class HTMLPrinter {
	...
	public static String format(String ind, String stop, String all) {
		...
		while (all.length() > 0) {
			...
			fulltoke = fulltoke.substring(0, fulltoke.toUpperCase().indexOf(
					toke.toUpperCase()))
					+ toke
		...
		System.out
				.println("MYERROR: </"
						+ stop
						+ "> not found at "
						+ Stuff.superTrim(bak.substring(0, Math.min(bak
								.length(), 60))));
		return all;
	}
	...
	public static void main(String s[]) {
		if (s.length != 0) {
			System.out
					.println("Usage: java aj.misc.HTMLPrinter <infile >outfile");
			System.exit(0);
		}
		...
		if (all.startsWith("<") && all.indexOf(">") > 0) {
			...
			String toke = all.substring(0, all.indexOf(">")).trim()
					.toUpperCase();
			all = all.substring(all.indexOf(">") + 1);
