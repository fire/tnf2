...
public class Bnf {
	...
	public static void main(String s[]) {
		if (s.length == 0) {
			System.out
					.println("FORMAT: java aj.testing.Bnf \"string\" <bnf_file> [<seed>]");
			System.exit(0);
		}
		GmlPair g = null;
		try {
	...
	public String process(String s) {
		while (s.indexOf("<") >= 0 && s.lastIndexOf(">") > s.indexOf("<")) {
			String sub = s.substring(s.indexOf("<") + 1, s.indexOf("<") + 1
					+ s.substring(s.indexOf("<") + 1).indexOf(">"));
			String rep = findRep(sub);
			s = s.substring(0, s.indexOf("<")) + rep
					+ s.substring(s.indexOf("<") + 2 + sub.length());
		}
		return s;
	}
