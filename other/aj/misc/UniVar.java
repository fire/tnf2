...
public class UniVar {
	...
	public String toString() {
		//return "\"" + sval + "\"";
                try {
                                ...
                                if (sval.length()>0 && sval.indexOf(" ")<0 && sval.indexOf("\t")<0 && sval.indexOf("_")<0 && sval.indexOf("\n")<0 && sval.indexOf(".")<0 && sval.indexOf(",")<0 && !Character.isDigit(sval.charAt(0))&& sval.indexOf("-")<0 && sval.indexOf(">")<0 && sval.indexOf("<")<0 && sval.indexOf(":")<0)
                                        return  sval ;
                                else
                                       return "\"" + sval + "\"";
                        }
	...
	public UniVar unsubstring(UniVar s) {
		try {
			...
			if (d1 > 0) {
				...
				return new UniVar(s1.substring(s1.length() + d1));//d1 is negtive
			}
		}
		catch (NumberFormatException NFE) {
	...
	public UniVar substring(UniVar s) {
		try {
			...
			if (d1 > 0) {
				...
				return new UniVar(s1.substring(0, s1.length() + d1));//note d1 is negitave
			}
		}
		catch (NumberFormatException NFE) {
			return new UniVar("Error3");
	...
	public static void main(String s[]) {
		for (int a = 0; a < s.length; a++) {
			System.out.println("get value of <" + s[a] + "> "+getValue(s[a]).toString());
		}
		try {
	...
	public static String tokenBefore(String e, int i) {
		...
		if (t.trim().endsWith("\"")) {
			...
			if (s.indexOf("\"") >= 0) {
				...
				//System.out.println("token before "+e.substring(i,i+1)+" in ("+e+") is ("+ans+")");
				return ans;
			}
			else {
			...
			int start = i - 1;
			while ((e.charAt(start) == ' ' || e.charAt(start) == '\t') && start > 0) {
				start--;
			}
			while (start > 0 && (
	...
	public static String tokenAfter(String e, int i) {
		...
		if (t.trim().startsWith("\"")) {
			...
			if (s.indexOf("\"") >= 0) {
				ans = e.substring(start, start + s.indexOf("\"") + 1 + t.indexOf("\"") + 1);
				//System.out.println("token after "+e.substring(i,i+1)+" in ("+e+") is ("+ans+")");
				return ans;
			}
			else {
				ans = tokenAfter(e, start + t.substring(t.indexOf("\"") + 1).length());
				//System.out.println("token after "+e.substring(i,i+1)+" in ("+e+") is ("+ans+")");
				return ans;
			}
		}
...
//only allow - at begin of UniVar
	...
	public static String spaceOperators(String eq) {
		if (DEBUG) System.out.println("spacing eq="+eq);
...
//if (DEBUG) System.out.println("eq="+eq+" toke before="+t2);
		...
		if (DEBUG) System.out.println("spaced eq="+eq);
		while (eq.indexOf("- ") >= 0) {
			eq=eq.substring(0,eq.indexOf("- "))+"-"+eq.substring(eq.indexOf("- ")+2);
		}
		while (eq.indexOf(" -") >= 0) {
			eq=eq.substring(0,eq.indexOf(" -"))+"-"+eq.substring(eq.indexOf(" -")+2);
		}
		...
		if (DEBUG) {System.out.println("- - + - +- -- fix/ eq="+eq);}
		return eq;
	}
	...
	static UniVar getValue(String eq) {
		...
		//paren
		if (DEBUG) {System.out.println("eq="+eq);}
		while (eq.indexOf(")") > 0 && eq.indexOf("(")>=0 && eq.indexOf("(") < eq.indexOf(")")) {
			int end = eq.indexOf(")");
			int start = eq.substring(0, end).lastIndexOf("(");
			eq = eq.substring(0, start) + getValue(eq.substring(start + 1, end)) + eq.substring(end + 1);
		}
		...
		while (eq.indexOf("==") >= 0) {
			...
			UniVar u = new UniVar(t1);
			UniVar u2 = new UniVar(t2);
			if (DEBUG) {System.out.println("tok1=("+t1+")");}
			if (DEBUG) {System.out.println("tok2=("+t2+")");}
			u = u.myEquals(u2);
			eq = eq.substring(0, start) + u + eq.substring(end + 1);
			//    System.out.println("fixed eq=("+eq+")");
		}
