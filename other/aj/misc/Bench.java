...
public class Bench {
	...
	static String showResult() {
		...
		String res = "";
		if (cc > 1000000000)
			res += (cc / 1000000) / 1000.0 + "B";
		else if (cc > 1000000)
			res += (cc / 1000) / 1000.0 + "M";
		else if (cc > 1000)
			res += cc / 1000.0 + "K";
		else
			res += cc;
		String tr = "Overall Benchmark: " + OPTIME / 1000 + " secs result in "
				+ res + " operations";
		res = "";
	...
	static String show(String s, long cc) {
		...
		if (cc > 1000000000)
			...
			res += cc;
		return "Benchmark: " + s + " " + OPTIME / 1000 + " secs result in "
				+ res + " operations";
	}
	...
	public static String matrix() {
			...
			for (b = 0; b < MAX; b++)
				for (c = 0; c < MAX; c++)
					for (d = 0; d < MAX; d++)
						for (e = 0; e < MAX; e++)
							for (f = 0; f < MAX; f++) {
								m[a][b][c] = m[c][b][a] * m[d][e][f]
										* m[b][c][a] * m[e][f][d];
							}
		long cc = (long) Math.pow(MAX, 6) * 1000
		...
		if (cc > 1000000000)
			...
			res += cc;
		return "Benchmark Matrix of 12: " + (System.currentTimeMillis() - t)
				/ 1000 + " secs " + res + " ops/sec";
	}
