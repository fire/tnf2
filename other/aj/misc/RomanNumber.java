...
public class RomanNumber extends Number {
	...
	// MDCLXVI
	private String fix[] = { "I", "IV", "V", "IX", "X", "XL", "L", "XC", "C",
			"CD", "D", "CM", "M" };
	 ...
	 * Numbers end at 1000, so large numbers like 20K will be written with 20
	 * Ms. Roman Numbers are best in the range of 4K-1. I Don't believe their is
	 * a zero.
	 * 
	 * @param s
	 *            Description of Parameter
	...
	public RomanNumber(String s) throws NumberFormatException {
		...
		x = 0;
		if (s.indexOf("IIII") >= 0 || s.indexOf("VV") >= 0
				|| s.indexOf("XXXX") >= 0 || s.indexOf("LL") >= 0
				|| s.indexOf("CCCC") >= 0 || s.indexOf("DD") >= 0) {
	...
	public static void main(String s[]) {
		for (int a = 0; a < s.length; a++) {
			try {
				System.out.println("Test RomanNumber.encode(" + s[a] + ") = "
						+ new RomanNumber(s[a]).toString());
			} catch (NumberFormatException e) {
			}
		}
		System.out.println("Test RomanNumber.encode(147) = "
				+ new RomanNumber(147).toString());
		System.out.println("Test RomanNumber.encode(6147) = "
				+ new RomanNumber(6147).toString());
		System.out.println("Test RomanNumber.encode(\"MM\") = "
				+ new RomanNumber("MM").toString());
		System.out.println("Test RomanNumber.encode(1997) = "
				+ new RomanNumber(1997).toString());
	}
}
