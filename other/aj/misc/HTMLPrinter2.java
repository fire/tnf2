...
public class HTMLPrinter2 {
	...
	static String nott[] = { "HR", "BR", "P", "FRAME", "IMG", "BASE", "META",
			"LI", "DD", "PARAM", "AREA", "META" };
	...
	public static void parse(String all) {
		...
		while (all.length() > 0) {
			...
			if (all.indexOf("<") == 0 && all.indexOf(">") > 0) {
				...
				if (fulltoke.startsWith("!--") && all.indexOf("-->") > 0) {
					...
					System.out.println("MyError: nested < <>> at "
							+ all.substring(60));
				...
				if (toke.indexOf(" ") > 0) {
					toke = toke.substring(0, toke.indexOf(" ")).toUpperCase()
							.trim();
				}
				fulltoke = fulltoke.substring(0, fulltoke.toUpperCase()
						.indexOf(toke.toUpperCase()))
						+ toke
						+ fulltoke.substring(fulltoke.toUpperCase().indexOf(
								toke.toUpperCase())
								+ toke.length());
		...
		for (int a = 0; a < v.size(); a++) {
			...
			if (t.startsWith("OPEN")) {
				...
				if (closeto == 0)
					System.out.println("CLOSE TO NOT FOUND " + closeto);
				else
					System.out.println("FOUND!! " + closeto);
	...
	public static void main(String s[]) {
		if (s.length != 0) {
			System.out
					.println("Usage: java aj.misc.HTMLPrinter2 <infile >outfile");
			System.exit(0);
		}
