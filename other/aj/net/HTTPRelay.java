...
public class HTTPRelay implements Runnable {
	...
	static String test[] = { "href=www.yahoo.com", "href=\"www.yahoo.com\"",
			"href=\'www.yahoo.com\'", "href = www.yahoo.com> ",
			"href=www.yahoo.com:8088/r/s.html",
		...
		try {
			...
			if (rhost == null || rport == 0) {
				...
				try {
					...
					System.out
							.println("MyError: bad host request cannot make socket to "
									+ rhost + " on port " + rport);
					close();
					return;
				}
	...
	public String fixHead(String s) {
		...
		if (s.toUpperCase().indexOf("REFERER: ") >= 0) {
			fix = "Referer: "
					+ s.substring(s.toUpperCase().lastIndexOf("HTTP://"));
		}
		if (s.toUpperCase().indexOf("HTTP://") > 0
				&& s.toUpperCase().indexOf(" HTTP/") > s.toUpperCase().indexOf(
						" HTTP://")) {
			fix = (get ? "GET" : "POST") + " " + req
	...
	public void processHeaderLine(String s) {
		...
		if (s.toUpperCase().indexOf("HTTP://") >= 0
			...
			if (s.toUpperCase().indexOf("HTTP://") >= 0) {
				s = s.substring(s.toUpperCase().lastIndexOf("HTTP://") + 7)
						.trim();
			}
			if (s.toUpperCase().indexOf("HTTP:\\\\") >= 0) {
				s = s.substring(s.toUpperCase().lastIndexOf("HTTP:\\\\") + 7)
			...
			if (s.toUpperCase().indexOf("HTTP//") >= 0) {
				s = s.substring(s.toUpperCase().lastIndexOf("HTTP//") + 6)
						.trim();
			}
			if (s.toUpperCase().indexOf("HTTP\\") >= 0) {
				s = s.substring(s.toUpperCase().lastIndexOf("HTTP\\") + 6)
			...
			if (logTrans)
				System.out.println("rhost1=" + rhost);
			...
			if (rhost.indexOf("/") >= 0) {
				...
				if (logTrans)
					System.out.println("rhost2=" + rhost);
			}
			if (!req.startsWith("/")) {
				req = "/" + req;
			}
			...
			if (rhost.indexOf(":") >= 0) {
				try {
					...
					System.out
							.println("MyError: bad number in host port request "
									+ portstring);
					close();
				}
				rhost = rhost.substring(0, rhost.indexOf(":"));
	...
	public String fixRelay(String s) {
		if (raw)
			return s;
		String correcthttp = "http://" + localhost
				+ (localport == 80 ? "" : ":" + localport) + "/http://";
	...
	public String subfix2(String org, String look) {
		...
		while (cap.indexOf(look) >= 0) {
			...
			if (ref == null || ref.toUpperCase().startsWith("HTTP://")) {
				org = org.substring(0, cap.indexOf(look)) + JUNK
						+ org.substring(cap.indexOf(look) + look.length());
				ref = null;
			} else {
				...
				org = org.substring(0, cap.indexOf(look)) + JUNK + "=\"" + ref
						+ "\"" + org.substring(end);
			}
			cap = org.toUpperCase();
		}
		while (org.indexOf(JUNK) >= 0) {
			org = org.substring(0, org.indexOf(JUNK)) + look
					+ org.substring(org.indexOf(JUNK) + JUNK.length());
		}
		return org;
	}
	 ...
	 * @return Description of the Returned Value
	 */
	public String replaceSubExclusiveIgnoreCase(String org, String find,
			String rep) {
		String JUNK = myrand() + "" + org.hashCode();
		while (org.indexOf(rep) >= 0) {
			org = org.substring(0, org.indexOf(rep)) + JUNK
					+ org.substring(org.indexOf(rep) + rep.length());
		}
		while (org.toUpperCase().indexOf(find.toUpperCase()) >= 0) {
			org = org.substring(0, org.toUpperCase()
					.indexOf(find.toUpperCase()))
					+ JUNK
					+ org.substring(org.toUpperCase().indexOf(
							find.toUpperCase())
							+ find.length());
		}
		while (org.indexOf(JUNK) >= 0) {
			org = org.substring(0, org.indexOf(JUNK)) + rep
					+ org.substring(org.indexOf(JUNK) + JUNK.length());
		}
		return org;
	}
	...
	public static String replaceSub(String org, String find, String rep) {
		...
		while (org.indexOf(find) >= 0) {
			org = org.substring(0, org.indexOf(find)) + JUNK
					+ org.substring(org.indexOf(find) + find.length());
		}
		while (org.indexOf(JUNK) >= 0) {
			org = org.substring(0, org.indexOf(JUNK)) + rep
					+ org.substring(org.indexOf(JUNK) + JUNK.length());
	...
	public static String replaceToken(String org, String find, String rep) {
		...
		while (repl.toUpperCase().indexOf(find.toUpperCase()) >= 0) {
			...
			if (b.endsWith("<")
					|| (b.endsWith("/") && b.substring(0, b.length() - 1)
							.trim().endsWith("<"))) {
				repl = repl.substring(0, bi) + JUNK
						+ repl.substring(bi + find.length());
			} else {
				String f = repl.substring(bi, bi + find.length());
				f = f.substring(0, f.length() / 2) + JUNK2
						+ f.substring(f.length() / 2);
				repl = repl.substring(0, bi) + f
						+ repl.substring(bi + find.length());
			}
		}
	...
	public static void main(String s[]) {
		if (s.length == 0) {
			System.out
					.println("FORMAT: java aj.net.HTTPRelay <port> [options]");
