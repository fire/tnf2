...
public class StuffIt {
	 ...
	 */
	public static String slist[] = { "http://clubs.lycos.com/live/Directory/PhotoAlbum.asp?ACT=VOTE&CG=llfae5c88va138400188mohh08&AID=61792&PID=415159&F=1&query=&SI=13&TOK=2000081616408v467mobd19niet38b1q7f76k8" };
	...
	public static void main(String s[]) {
		...
		Vector v = new Vector();
		v
				.addElement("User-Agent: Mozilla/4.0 (compatible; MSIE 5.5; Windows NT 4.0; TUCOWS)");
		...
		if (s.length == 0) {
			System.out
					.println("FORMAT: java aj.net.StuffIt [<procs> [<millis> [readsome|readall|readnone scrape [<URL>]]]]");
			System.exit(0);
		}
		...
		for (int a = 2; a < s.length; a++) {
			...
			if (s[a].toUpperCase().indexOf("MAXSCRAPEREUSE:") >= 0) {
				try {
					s[a] = s[a].substring(s[a].toUpperCase().indexOf(
							"MAXSCRAPEREUSE:"));
					maxScrapeReuse = Integer
							.parseInt(s[a].substring(15).trim());
					System.out.println("max scrape rate = " + maxScrapeReuse);
				} catch (NumberFormatException NFE) {
					System.out
							.println("MyError: Bad number in maxScrapeReuse, using 1");
				}
			}
	...
	public static String removeWebEncoding(String s) {
		...
		while (s.indexOf("%") >= 0) {
			...
			s = s.substring(0, s.indexOf("%")) + (char) (e)
	...
	public static String replace(String s, String find, String rep) {
		...
		while (s.indexOf(find) >= 0) {
			s = s.substring(0, s.indexOf(find)) + rep
					+ s.substring(s.indexOf(find) + find.length());
		}
		return s;
	}
	...
	public synchronized static String scrape(String url, String begin,
			String end) {
		...
		try {
			...
			while (true) {
				...
				if (all.indexOf(begin) >= 0
						&& all.indexOf(end) > all.indexOf(begin)) {
					br.close();
					break;
				}
			}
			...
			System.out
					.println("MyError: Unable to scrape.  Cannot find web page");
		}
		if (all.indexOf(begin) >= 0) {
...
class Stuffer implements Runnable {
	...
	public Stuffer(String urlname, int delay) {
		// System.out.println("Stuffer running doread=" + doread + " readall=" +
		// readall + " doscrape=" + doscrape + " urlname=" + urlname + " delay="
		// + delay);
...
/*
 * location =
 * "../Directory/PhotoVoteCheck.asp?R=%2E%2E%2FDirectory%2FPhotoAlbum%2Easp%3FACT%3DVOTE%26CG%3Dllfae5c88va138400188mohh08%26AID%3D61792%26PID%3D" +
 * pic_id +
 * "%26F%3D1%26query%3D%26SI%3D1%26TOK%3D20000816171233ut7km7cprukb40t18bnkd7is";
 * location =
 * "../Directory/PhotoVoteCheck.asp?R=%2E%2E%2FDirectory%2FPhotoAlbum%2Easp%3FACT%3DVOTE%26CG%3Dllfae5c88va138400188mohh08%26AID%3D61792%26PID%3D" +
 * pic_id +
 * "%26F%3D1%26query%3D%26SI%3D1%26TOK%3D2000081617133i21ll5nfk2eavp0ac6fqsgtf8";
 * location =
 * "../Directory/PhotoVoteCheck.asp?R=%2E%2E%2FDirectory%2FPhotoAlbum%2Easp%3FACT%3DVOTE%26CG%3Dllfae5c88va138400188mohh08%26AID%3D61792%26PID%3D" +
 * pic_id +
 * "%26F%3D1%26query%3D%26SI%3D1%26TOK%3D20000816171233ut7km7cprukb40t18bnkd7is";
 ...
 * url=scrape(url,"location = \"../"," + pic_id + \", String
 * line=scrapeLine(sourceUrl,beginpattern,endpattern); String
 * line=scrapeLine("http://clubs.lycos.com/live/Directory/PhotoAlbum.asp?CG=llfae5c88va138400188mohh08&AID=61792&F=1"
 * ,"location = \"../", ,"\";"); line=removeWebEncoding(line);
 * line=replace(line,pattern,pattern) line=replace(line,"\" + pic_id +
 * \"",415159); String newUrl="http://clubs.lycos.com/live/"+line;
 */
