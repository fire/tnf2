...
public class HTTP implements Runnable {
	...
	String info[][] = {
			{".html", IMGTEXT, "Content-type:  text/html ", "HyperText Document"}, 
			{".htm", IMGTEXT, "Content-type:  text/html ", "HyperText Document"}, 
			...
			{".ps", IMGTEXT, "Content-type:  text/postscript", "Postscript Document"}, 
			{".pdf", IMGTEXT, "Content-type:  application/pdf", "Acrobat File"}, 
			{".zip", IMGBINARY, "Content-type:  application/zip", "Zip Compressed File"}, 
			{".gz", IMGBINARY, "Content-type:  application/zip", "Gzip Compressed File"}, 
			{".tar", IMGBINARY, "Content-type:  application/zip", "Tape Archive Record"}, 
			{".jar", IMGBINARY, "Content-type:  application/zip", "Jave Archive Record"}, 
			{".class", IMGBINARY, "Content-type:  application/class", "Java Class File"}, 
			...
			{".dll", IMGBINARY, "Content-type:  file/dll", "Dynamic Linked Library"}, 
	 ...
	 *
	 *@param  socket           Description of Parameter 
...
//System.out.println("fileName=<"+fileName+">");
		...
		while (fileName.indexOf("//") >= 0) {
			fileName = fileName.substring(0, fileName.indexOf("//")) + "/" + fileName.substring(fileName.indexOf("//") + 2);
		}
		while (fileName.indexOf("../") >= 0) {
		...
		if (file.isDirectory()) {
			...
			fileName = fileName.substring(fileName.lastIndexOf("-") + 1).trim() + ".gif";
			sendInternalFile();
		}
		else if (!file.exists()) {
	 ...
	 *  Description of the Method 
	 *
	 *@param  sock  Description of Parameter 
	 */
	...
	public void log(String file) {
		while (file.indexOf("\\") >= 0) {
			file = file.substring(0, file.indexOf("\\")) + "/" + file.substring(file.indexOf("\\") + 1);
		}
		while (file.indexOf("//") >= 0) {
	...
	public void sendDirectory() {
		...
		if (dirList != null) {
			for (a = 0; a < dirList.length; a++) {
				...
				String SIZE = ((int) (subFileFile.length() / 10.240) / 100.0) + "K           ";
				if (subFileFile.length() > 1024 * 1024) {
					SIZE = ((int) (subFileFile.length() / 10.240 / 1024) / 100.0) + "M           ";
				}
				SIZE = SIZE.substring(0, 10);
				String DATE = (new java.util.Date(subFileFile.lastModified())).toString();
				String NAME;
				NAME = dirList[a].substring(0, Math.min(dirList[a].length(), 30)).trim();
				NAME += "</a>                                      ";
				NAME = NAME.substring(0, 35);
				String MISC = getMisc(subFile);
				body += "<a href=\"" + dirList[a] + "\">" + IMG + NAME + " " + SIZE + " " + DATE + " " + MISC + "\n";
			}
		}
		body += "</pre></body></html>";
