...
public class MyFtp {
	...
	public static String getBanner() {
		if (localHost.indexOf("/") > 0)
			localHost = localHost.substring(0, localHost.indexOf("/")).trim();
		if (localHost.indexOf(".") > 0)
			localHost = localHost.substring(0, localHost.indexOf(".")).trim();
		return "220 " + localHost + " aj.net.MyFtp FTP server " + version
				+ " ready.";
	...
	public MyFtp(String s[]) {
		...
		for (int a = 0; a < s.length; a++) {
			if (s[a].toUpperCase().indexOf("USER:") >= 0
					&& s[a].indexOf(":") < s[a].lastIndexOf(":")) {
				// user:j:p:r:RW
				...
				try {
					port = Integer.parseInt(s[a].substring(s[a].toUpperCase()
							.indexOf("PORT:") + 5));
	 ...
	 *            Description of Parameter
	 */
	public static void get(String name, String host, int port, Socket nl,
			boolean b) {
		// ascii vs binnary
		try {
	...
	public static void get(String name, Socket s, Socket nl, boolean b) {
		send(nl, "150 Opening " + (b ? "BINARY" : "ASCII") + " socket for "
				+ name);
		try {
	...
	public static boolean login(String u, String p, FtpConnection fc) {
		...
		for (int a = 0; a < login.size(); a++) {
			...
			if (l[0].equals(u) && l[1].equals(p)) {
				System.out.println(fc.socket.getInetAddress() + " "
						+ new Date() + ":LOGIN: " + u);
				return true;
			}
		}
		if (MyFtp.log) {
			System.out.println(fc.socket.getInetAddress() + " " + new Date()
					+ ":login fail: " + u + " " + p);
		}
		return false;
	}
	 ...
	 *            Description of Parameter
	 */
	public static void send(String name, String host, int port, Socket nl,
			boolean b, boolean details) {
		// System.out.println("sending" + name + " " + host + " " + port + " " +
		// b);
		// ascii vs binnary
		Socket s = null;
		try {
	...
	public static void send(String name, Socket s, Socket nl, boolean b,
			boolean details) {
		send(nl, "150 Opening " + (b ? "BINARY" : "ASCII") + " socket for "
				+ name);
		OutputStream o = null;
		try {
			...
			if (name.indexOf("*") >= 0 || name.indexOf("?") >= 0) {
				...
				look = name.substring(name.substring(0,
						name.length() - look.length()).lastIndexOf("/") + 1);
...
class FtpConnection implements Runnable {
		...
		while (true) {
			...
			if (cmd.toUpperCase().startsWith("NOOP")) {
				...
				if (!loggedIn) {
					...
					|| cmd.toUpperCase().startsWith("XPWD")) {
	...
	public void passiveConnect(Socket nl) {
		...
		try {
			...
			send(nl, "227 Entering Passive Mode (" + hosts[0] + "," + hosts[1]
					+ "," + hosts[2] + "," + hosts[3] + "," + (repport / 256)
					+ "," + (repport % 256) + ")");
...
/*
 * Type: ASCII Non-print, IMAGE, LOCAL 8 Mode: Stream Structure: File, Record
 * Commands: (DONE) (USER), (PASS), ACCT, (PORT), (PASV), TYPE, MODE, STRU,
 * (RETR), (STOR), APPE, RNFR, RNTO, DELE, (CWD), CDUP, RMD, MKD, (PWD), (LIST),
 * (NLST), (SYST), STAT, HELP, (NOOP), (QUIT). todo HELP, MKD, RMD, DELE
 */
