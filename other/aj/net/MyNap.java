	...
	public void attachTo(String s) {
		...
		if (s.indexOf(":") <= 0) {
			...
			try {
				...
				System.out.println("MyError: bad port in connection " + t + ", using default");
			}
		}
		try {
...
//System.out.println(fd.toGmlString());
	...
	public void run() {
		...
		while (true) {
			try {
				...
				System.out.println("MyError: connection error in general connect");
			}
		}
	}
	...
	public Socket lookupConnection(String host,int port) {
		try {
			...
			final BufferedReader br=new BufferedReader(new InputStreamReader(nl.getInputStream()));
			new Thread() {
				public void run() {
					while (true) {
						String s;
	...
	public void addConnection(Socket nl) {
		if (connectList.contains(nl)) return;
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource()==downloadButton) {
			//file dialoge
			FileDialog fd=new FileDialog(searchFrame,"Save Location",FileDialog.SAVE);
			fd.setVisible(true);//();	
...
//				System.out.println("MyError: unparsed search request"+st);
			...
			//look for old Socket
			Socket nl=lookupConnection(gm.getOneByName("host").getString(),(int)(gm.getOneByName("port").getDouble()));
			if (nl==null) {
				System.out.println("MyError: cannot connect to host "+gm.getOneByName("host").getString());
				return;
			}
...
//System.out.println("command found="+c);
			...
			if (c.toUpperCase().startsWith("REQUESTFILE ")) {
				...
				if (port > 0) {
//					System.out.println("file "+fileName+" AKA "+fd.location+" available at port "+port);
			...
			if (c.toUpperCase().startsWith("SEND ")) {
				//send filename port
				try {
					int port = Integer.parseInt(c.substring(c.toUpperCase().indexOf("PORT:") + 5).trim());
					//check remaining pickup list
					//get save file name from pickup list hash
					String fileName = c.substring(c.indexOf(" ") + 1, c.lastIndexOf(" ")).trim();
					if (fileName.equals("*")) {
						String ind = "";
						for (Enumeration e = shareList.keys(); e.hasMoreElements(); ) {
							FileData ffd = (FileData) shareList.get(e.nextElement());
							ind += ffd.toString() + "\n";
						}
					...
					transfer(new File(fd.location), new Socket(nl.getInetAddress(), port));
				}
				catch (NumberFormatException NFE) {
					...
					send(nl,"MYNAPERROR: UNABLE TO OPEN PORT on host " + nl.getInetAddress());
				}	
				catch (Exception E) {
...
//				System.out.println("pickup order "+c);
				...
				try {
					...
					//check remaining pickup list
					//get save file name from pickup list hash
					String pickupFileName = c.substring(c.indexOf(" ") + 1, c.lastIndexOf(" ")).trim();
					GmlPair foundReply = (GmlPair) requestList.get(pickupFileName);
					String destFileName=foundReply.getOneByName("saveName").getString();
//remove pickup from request list
//so as to not accept additional pick
//requests
					if (destFileName == null) {
						send(nl,"MYNAPERROR: Did not request file " + pickupFileName);
						return;
					}
					transfer(new Socket(nl.getInetAddress(), port), new File(destFileName));
					...
					System.out.println("MyError: bad pickup orders. connection."+IOE);
					//check firewall block connect
				}
				catch (Exception E) {
						System.out.println("MyError: bad pickup orders. bad file name."+E);
				}
		}
	}
	...
	public void makeRequest(GmlPair foundReply, String saveName, Socket nl) {
		...
		requestList.put(foundReply.getOneByName("file").getString(), foundReply);
		send(nl,"REQUESTFILE " + foundReply.getOneByName("file").getString());
	}


//	public void makeFireWallRequest(String requestName, String saveName, Socket nl) {
//		foundReply.add(new GmlPair("saveName",saveName));
...
//	}
	...
	public void transfer(Socket socket, File file) {
		try {
			transfer(socket.getInputStream(), new FileOutputStream(file), 0, maxFileSize);
		}
		catch (IOException IOE) {
...
//System.out.println("file "+file+" transfering via socket");
			transfer(new FileInputStream(file), socket.getOutputStream(), 0, 30000 * 100000L);
		}
		catch (IOException IOE) {
			System.out.println("MyError: transfer error. in file to socket="+IOE);
		}
	}
	...
	public void transfer(InputStream i, OutputStream o, long start, long end) {
		if (i == null || o == null) {
			System.out.println("MyError: transfer failed.  Unable to open input or output");
		}
...
//			System.out.println("rate =" + rate + " KBytes/sec");
	...
	public void firewallTest() {
		...
		while (connectList.size() > 0) {
			nl = (Socket) connectList.elementAt((int) (connectList.size() * Math.random()));
			if (nl.isClosed()) {
				removeConnection(nl);
			}
		}
	...
	public void receiveFound(GmlPair g) {
		...
		if (mySearch.contains(id)) {
			for (int a=0;a<found.size();a++) {
				...
				if (c1!=null && c2!=null) {
					if (c1.getString().trim().equalsIgnoreCase(c2.getString().trim())) return;
				}
			}
//check share list for current copy
			...
			for (Enumeration e = shareList.keys(); e.hasMoreElements(); ) {
				FileData fd = (FileData) shareList.get(e.nextElement());
				if (fd.crc==crc) {
//					System.out.println("CRC duplicat file found in search "+g);
					return;
				}
			}
...
//			System.out.println("checking file "+fd.shareAs);
	...
	public void sendFound(GmlPair G, FileData fd, Socket nl) {
		...
		send(nl,g.toString());
		//found [ id searchhash file name crc num size num date num host name port num ]
	}
	...
	public void doSearch(String key) {
		...
		Vector v = new Vector();
		String id = (connectServer.getInetAddress().getHostAddress() + ":" + cp + new Date()).hashCode() + "";
...
//System.out.println("search created = "+g);
	...
	public static void main(String s[]) {
		System.out.println("Format: java aj.net.MyNap [server:port] [NOGUI] [<remotehost>:port]*");
		MyNap mn;
		int port = defaultConnectPort;
		for (int a = 0; a < s.length; a++) {
			if (s[a].toUpperCase().startsWith("NOGUI")) MyNap.gui=false;
		...
		new Thread(mn).start();
		for (int a = 0; a < s.length; a++) {
			if (!s[a].toUpperCase().startsWith("SERVER:") && !s[a].toUpperCase().startsWith("NOGUI")) {
				mn.attachTo(s[a]);
			}
		}
	}
...
class FileData {
	...
	public String fileForm(String f) {
		if (f==null) return "";
		while (f.indexOf("\\")>=0) f=f.substring(0,f.indexOf("\\"))+"/"+f.substring(f.indexOf("\\")+1);
		return f;
	}

	public boolean isValid() {
		return crc!=-1 && size!=-1 && date!=-1 && !shareAs.equals("") && !location.equals("");
	}
	...
	public String toString() {
		return shareAs + " " + (size > 1000000 ? size / 1000000 + "M" : (size > 1000 ? size / 1000 + "K" : size + "B")) + " CRC:" + crc + " " + new Date(date) + " " + desc;
	}
...
/*
 * search [ id num key word ttl num ]
 * found [ id num name file  ]
 * MYNAPERROR
 * SEARCHFOR [ key word1 key word2 key word3]
 * SEARCHFOR [ key word1 size "1000000-" ]
 * SEARCHFOR [ key word1 size "1000000+" ]
 * SEARCHFOR [ key word1 size "1000000+" size "2000000-" ]
 * SEARCHFOR [ crc "3525235252" ]
 * REQUESTFILE [ name sharename ]
 * SEND [ name sharename port 5235 ]
 * PICKUP [ name sharename port 5352 ]
 * Vector
 * searchResults  foundstring Socket
 */

/*
 * GUI
 * sharelist - what I am sharing
 * traffic list - searches and founds relaying
 * (string = 1000 lines max constant string)
 * search results - founds to me
 * (store 10,000 - searches start here)  (search  or  newSearch)
 * transfer list - uploads and downloads in progress
 * (sending %done)
 * (receiveing %done)
 * commands - start search
 * request download
 * active hosts
 * all hosts
 * search
 * request  name  host
 * search [ id hash ttl 5 key word  ]
 * new search [ id oldhash ttl 4 key word ]
 * receive search -
 * check ttl - drop
 * check if id already serviced - drop
 * do internal seach and reply with found
 * save to relay founds
 * relay to all attached hosts  new search
 * found [ id searchhash file name crc num size num desc string host name port num ]
 * receive found -
 * if (mysearch) {
 * save result
 * }
 * check if relay found
 * relay to source
 */
...
class multirequest
  multirequest(foundReply) {
    get all CRC that match
    GmlPair allFound[]
    long fsize
    long tsize=fsize/allFound.length+1;
    long fsize=fsize%tsize;
    partialRequest pq[]=new partialrequest[allFound.length];
    for(int a=0;a<allFound;a++) {
      pq[a]=new partialRequest(,tsize*a,tsize*(a+1),this)
  }
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    if (error) pq[a]=new Partialrequest(fn,tsize*a,  . .. )

    if (all complete) {
       rebuildFinal
		FileOutputStream fo
		for (int a=0;a<pq.length;a++) {
			transalldata(partFail[a],fo);
			delfile(partfile[a]);
		}
    }
  }
