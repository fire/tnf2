...
public class Send {
	...
	String USAGE = "Usuage: java aj.net.Send [<file_name>] [-verbose]\n"
			...
			+ "   *TIME: <long>  timestamp " + System.currentTimeMillis() + "="
			+ new Date() + ".\n"
	...
	public void read(BufferedReader BR) {
		...
		try {
			...
			while (t != null && readingHeader) {
				...
				if (t.toUpperCase().startsWith("POP")) {
					...
					if (pophost != null && pophost.indexOf(":") > 0) {
						try {
							popport = Integer.parseInt(pophost
									.substring(pophost.indexOf(":") + 1));
						} catch (NumberFormatException NFE) {
						}
						pophost = pophost.substring(0, pophost.indexOf(":"));
					}

				} else if (t.toUpperCase().startsWith("FROMHOST")
						|| t.toUpperCase().startsWith("FROM HOST")) {
					...
					fromhost = t;
				} else if (t.toUpperCase().startsWith("FROMUSER")
						|| t.toUpperCase().startsWith("FROM USER")) {
					...
					from = t;
				} else if (t.toUpperCase().startsWith("TOHOST")
						|| t.toUpperCase().startsWith("TO HOST")) {
					t = t.substring(t.toUpperCase().indexOf("HOST") + 4);
					t = t.trim();
					if (t.startsWith(":")) {
						t = t.substring(1).trim();
					...
					if (tohost.indexOf(":") > 0) {
						try {
							toport = Integer.parseInt(tohost.substring(tohost
									.indexOf(":") + 1));
						} catch (NumberFormatException NFE) {
						}
						tohost = tohost.substring(0, tohost.indexOf(":"));
					}
				} else if (t.toUpperCase().startsWith("TOUSER")
						|| t.toUpperCase().startsWith("TO USER")) {
					t = t.substring(t.toUpperCase().indexOf("USER") + 4);
					t = t.trim();
					if (t.startsWith(":")) {
						t = t.substring(1).trim();
			...
			message.addElement("Message-Id: <" + SendVersion + "." + myDate
					+ "." + num + "@" + localhost + ">");

			message.addElement("X-Sender: " + from);
			message.addElement("X-Mailer: aj.net.Send for GalaxyChart version "
					+ SendVersion);

			String ls[] = Stuff.getTokens(new java.util.Date() + "", " ");
			if (time != 0)
				ls = Stuff.getTokens(new java.util.Date(time) + "", " ");
			String tz = ""
					+ (calendar.get(Calendar.ZONE_OFFSET) / (60 * 60 * 1000) * 100);
			String ld = ls[0] + ", " + ls[2] + " " + ls[1] + " " + ls[5] + " "
					+ ls[3] + " " + tz + " (" + ls[4] + ")";
			...
			message
					.addElement("Content-Type: text/plain; charset=\"us-ascii\"");
			message.addElement("");
	...
	/**
	 * This method actually processes the delevery. It opens the socket and
	 * sends the message.
	 * 
	 * @exception IOException
	 *                when any delivery error has occured. This includes bad to
	 *                address and bad to hosts.
	 */
	public void deliver() throws IOException {
		...
		if (pophost != null && popname != null && poppass != null) {
			if (verbose) {
				System.out.println("Pop authenticating " + pophost
						+ " at port " + popport + " user " + popname
						+ " and pass " + poppass);
			}
		...
		for (int a = 0; a < message.size(); a++) {
			...
			if (verbose
					&& (message.elementAt(a).toString().startsWith("HELO")
							|| message.elementAt(a).toString().startsWith(
									"RSET")
							|| message.elementAt(a).toString().startsWith(
									"MAIL")
							|| message.elementAt(a).toString().startsWith(
							...
							|| message.elementAt(a).toString().startsWith(
									"DATA")
							|| message.elementAt(a).toString().equals(".") || message
							.elementAt(a).toString().startsWith("QUIT"))) {
...
/*
 * notes sendln("HELO "+localHostName); sendln("MAIL FROM:"+msg.getSource());
 * sendln("RCPT TO:"+msg.getDestination()); sendln("DATA"); sendln("Subject:
 * "+msg.getSubject()); sendln("."); sendln("QUIT");
 */
