...
public class Pop {
	...
	public Pop(String config) throws IOException {
		...
		if (n.length != 0) {
			if (n[0].getString().equalsIgnoreCase("FALSE")
					|| n[0].getString().equalsIgnoreCase("NO")
					|| n[0].getString().equals("0"))
				deleteOkay = false;
			else
				deleteOkay = true;
		}
		if (VERBOSE)
			System.out.println("deleteokay=" + deleteOkay);
		if (VERBOSE)
	...
	public Pop(String h, int p, String u, String pa, boolean au)
			throws IOException {
		if (VERBOSE)
	...
	public void process(String line) throws IOException {
		...
		if (tm.length > 1) {
			...
			try {
				...
				System.out.println("MyError: bad response to STAT.  Received "
						+ t);
				System.exit(0);
			}
			...
			for (int a = 0; a < num; a++) {
				...
				if (requireText == null
						|| all.toUpperCase().indexOf(requireText.toUpperCase()) < 0) {
					if (VERBOSE)
						System.out
								.println("Message doesn't meed required context.  Do not process or delete");
					continue;
				}
				if (deleteOkay) {
					if (VERBOSE)
				...
				if (line == null) {
					...
					makeFile(all);
						...
						&& line.indexOf("@") > 0) {
					if (VERBOSE)
						System.out.println("Message forwarding to email "
								+ line);
					forwardTo(line, all);
				} else {
					if (VERBOSE)
						System.out.println("Message forwarding to program "
								+ line);
					execute(line, all);
				}
			}
	...
	public String readMessage(int a) throws IOException {
		if (VERBOSE)
			System.out.println("receiving message " + a);
		String all = "";
		send(s, "RETR " + a);
		BufferedReader br = new BufferedReader(new InputStreamReader(s
				.getInputStream()));
	...
	public void deleteMessage(int a) throws IOException {
		if (VERBOSE)
			System.out.println("deleting message " + a);
		send(s, "DELE " + a);
		BufferedReader br = new BufferedReader(new InputStreamReader(s
				.getInputStream()));
		String t = br.readLine();
	...
	public static void main(String s[]) {
		if (s.length != 5 && s.length != 4 && s.length != 1) {
			System.out
					.println("FORMAT: java aj.net.Pop <host> <port> <user> <pass> [<command>]");
			System.out.println("FORMAT: java aj.net.Pop <pop.cfg file>");
			System.out
					.println("   command = email or program else makes files");
			System.exit(0);
		}
		...
		try {
			...
			if (s.length >= 4)
				System.out.println("MyError: unable to connect host (" + s[0]
						+ ") port (" + s[1] + ") user (" + s[2] + ") pass ("
						+ s[3] + ")");
			else
				System.out
						.println("MyError: unable to connect host defined in file "
								+ s[0]);
			System.out
					.println("FORMAT: java aj.net.Pop <host> <port> <user> <pass> [<command>]");
			System.out.println("FORMAT: java aj.net.Pop <pop.cfg file>");
