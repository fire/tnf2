...
public class RelayServer {
		...
		while (true) {
			...
			new Thread() {
				public void run() {
					BufferedReader br;
					try {
						br = new BufferedReader(new InputStreamReader(SS
								.getInputStream()));
						while (true) {
	...
	public void actionPerformed(Socket S, String line) throws IOException {
		...
		if (tok.length == 0) {
		...
		else if (tok[0].equalsIgnoreCase("__DESTROY")
				|| tok[0].equalsIgnoreCase("__DELETE")) {
			// name pass
			...
			for (int a = 0; a < GROUP.size(); a++) {
				...
				if (RG.getName().equals(test.getName())
						&& RG.getPass().equals(test.getPass())) {
			...
			for (int a = 0; a < GROUP.size(); a++) {
				...
				if (v.contains(S)) {
					// System.out.println("Broad casting "+line+" in
					// "+v.getName());
					v.broadcast(line, S);
				}
			}
...
class RelayGroup {
	...
	public String add(Socket NL) {
		for (int a = 0; size < max && a < ALL.length; a++) {
			if (ALL[a] == null) {
				...
				if (size == max) {
					open = false;
					return "__JOINED " + getName() + " CHANEL_FULL "
							+ getLocation(NL);
				}
				return "__JOINED " + getName() + " CHANEL_FILLING "
						+ getLocation(NL);
			}
		}
	...
	public String toString() {
		return "GROUP> " + name + " " + pass + " (" + size + "/" + max
				+ ") open=" + open + " refill=" + refill;
	}
