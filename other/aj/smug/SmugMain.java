...
public class SmugMain {
	...
	public static void main(String s[]) {
		if (s.length>0 && s[0].toUpperCase().indexOf("EXEC")>=0){
			...
			System.out.println("Format: java aj.smug.Main <execturn|exec|mail|procmail>");
			System.exit(0);
		}
	}
...
System.out.println("mailing receipt");
	...
	public Planet getPlanetAtLoc(String s) {
		s=s.trim();
		if (s.toUpperCase().endsWith("S")) s=s.substring(0,s.length()-2);
		if (s.toUpperCase().endsWith("O")) s=s.substring(0,s.length()-2);
		for (int a=0;a<planets.size();a++) {
			Planet p=(Planet)planets.elementAt(a);
			if (s.trim().equals(p.loc.trim())) return p;
		}
		return null;
	}
	public Planet getPlanetByName(String s) {
		for (int a=0;a<planets.size();a++) {
			Planet p=(Planet)planets.elementAt(a);
			if (s.equals(p.name)) return p;
		}
		return null;
	}
	public Item getRandomItem(){
		while (true) {
			Item i=(Item)items.elementAt((int)(items.size()*Math.random()));
			if (i.stealable()) return i;
		}
	}
	public Item getItemByName(String s) {
		for (int a=0;a<items.size();a++) {
			Item i=(Item)items.elementAt(a);
			if (s.equals(i.name)) return i;
		}
		return null;
	}
	public Product getProductByName(String s) {
		for (int a=0;a<products.size();a++) {
			...
			if (s.equalsIgnoreCase(p.name)) return p;
		}
		for (int a=0;a<products.size();a++) {
			Product p=(Product)products.elementAt(a);
			if (s.equalsIgnoreCase(p.nick)) return p;
		}
		return null;
	}
	...
	public Vector getIllegalCargo() {
		if (illc==null) {
			...
			for (int a=0;a<products.size();a++) {
				Product p=(Product)products.elementAt(a);
				if (!p.legal) illc.addElement(p);
				else lc.addElement(p);
			}
		}
		return illc;
	}
	public Vector getLegalCargo() {
		if (lc==null) getIllegalCargo();
		return lc;
	}
}
