...
public class PlanetDisplay {
	...
	public PlanetDisplay(Color c, int cx, int cy, int siz, int nummoons,
			int numsats, int neb, int seed) {
		if (seed == -1)
			r = new Random((int) (Math.random() * Integer.MAX_VALUE));
		else
			r = new Random(seed);
		if (c == null) {
			c = new Color((int) (100 + 155 * r.nextDouble()),
					(int) (100 + 155 * r.nextDouble()), (int) (100 + 155 * r
							.nextDouble()));
		}
		if (neb == -1)
			this.neb = r.nextDouble() * 2 > 1;
		else
			this.neb = neb == 1;
		if (c != null)
			lite = c;
		...
		this.siz = siz;
		if (nummoons < 0)
			nummoons = (int) (r.nextDouble() * (maxMoons - minMoons) + minMoons);
		if (numsats < 0)
			numsats = (int) (r.nextDouble() * (maxSats - minSats) + minSats);
		// if (nummoons>maxMoons) nummoons=maxMoons;
		// if (numsats>maxSats) numsats=maxSats;
		for (int b = 0; b < numsats; b++) {
			Sat ss = new Sat(null, this, (int) (siz / 2 + siz / 6
					* r.nextDouble()), (int) (r.nextDouble() * 2 + 1));
			this.addSat(ss);
		}
		for (int a = 0; a < nummoons; a++) {
			Color cc = new Color((int) (100 + 155 * r.nextDouble()),
					(int) (100 + 155 * r.nextDouble()), (int) (100 + 155 * r
							.nextDouble()));
			Sat ss = new Sat(cc, this, (int) (siz / 2 + siz * r.nextDouble()),
					(int) (siz / 4 * r.nextDouble() + 3));
			this.addSat(ss);
		}
	}
	...
	public static void display(Graphics g, int cx, int cy, int siz, double day,
			Color on) {
		...
		if (r < .25) {
			...
			g.fillOval((int) (cx - s / 2 + s / 2 * r), cy - s / 2, (int) (s - s
					* r), s);
		} else if (r < .5) {
			r = 1 - (r - .25) * 4;
			g.setColor(off);
