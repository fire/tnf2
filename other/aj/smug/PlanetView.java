...
public class PlanetView extends Canvas {
	...
	public static void main(String s[]) {
		...
		try {
			if (s.length == 1) {
				...
				pv = new PlanetView(Integer.parseInt(s[0]), Integer
						.parseInt(s[1]), -1, -1, -1);
			} else if (s.length == 3) {
				pv = new PlanetView(Integer.parseInt(s[0]), Integer
						.parseInt(s[1]), Integer.parseInt(s[2]), -1, -1);
			} else if (s.length == 4) {
				pv = new PlanetView(Integer.parseInt(s[0]), Integer
						.parseInt(s[1]), Integer.parseInt(s[2]), Integer
						.parseInt(s[3]), -1);
			} else if (s.length == 5) {
				pv = new PlanetView(Integer.parseInt(s[0]), Integer
						.parseInt(s[1]), Integer.parseInt(s[2]), Integer
						.parseInt(s[3]), Integer.parseInt(s[4]));
			} else {
				pv = new PlanetView(-1, -1, -1, -1, -1);
				System.out
						.println("FORMAT: java aj.smug.PlanetView <diameter|-1> <moons|-1> <satalites|-1> <neb 0,1|-1> <seed|-1>");
			}
	...
	public void paint(Graphics g) {
		...
		if (i2 == null) {
			...
			Color c = new Color((int) (100 + 155 * r.nextDouble()),
					(int) (100 + 155 * r.nextDouble()), (int) (100 + 155 * r
							.nextDouble()));
			if (p.hasNebula()) {
				aj.awt.Neb.build(G2, c, screensize, 6, screensize * screensize
						* 60, false, false, r);
			}
			// stars
			for (int a = 0; a < screensize * screensize / 30; a++) {
				c = new Color((int) (100 + 155 * r.nextDouble()),
						(int) (100 + 155 * r.nextDouble()),
						(int) (100 + 155 * r.nextDouble()));
				G2.setColor(c);
