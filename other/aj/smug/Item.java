...
public class Item {
	...
	static String types[] = { "equipment", "weapon", "gun", "armor", "engine",
			"shield", "hull", "manuver", "weaponsystem", "computer", "sensor" };
	...
	public boolean stealable() {
		for (int a = 0; a < 3; a++)
			if (type.equalsIgnoreCase(types[a]))
				return true;
		return false;
	}
	...
	public static Item parse(GmlPair g) {
		...
		if (n != null)
			bonusskill = n.getString();
		n = g.getOneByName("bonusamount");
		if (n != null)
			bonusamount = (int) (n.getDouble());
		String available = "";
		n = g.getOneByName("available");
		if (n != null)
			available = n.getString();
