...
static int GALSIZ=150,NUMPLANETS=300,GALHEI=50;
  ...
  public static void main(String s[]) {
    ...
    for (a=0;a<NUMPLANETS;a++) {
      ...
      if (r2>hei/2) r2=r2-hei/2;
      r2=hei/2-r2;
      int quad=(int)(Math.random()*4);
      int r3=(int)(Math.random()*360);
      int t=(int)(r2*Math.sin(r3)),tt=(int)(r2*Math.cos(r3));
      if (quad==0) {x[a]=-rad;y[a]=tt;z[a]=t;}
      if (quad==1) {x[a]=rad;y[a]=-tt;z[a]=t;}
      if (quad==2) {x[a]=-tt;y[a]=rad;z[a]=t;}
      if (quad==3) {x[a]=tt;y[a]=-rad;z[a]=t;}
      double d=Math.sqrt(x[a]*x[a]+y[a]*y[a]);//+z[a]*z[a]);
      double bang=myatan(x[a],y[a]);
      double rot=-Math.PI*Math.sqrt(GALSIZ/2-rad)/Math.sqrt(GALSIZ/2)+bang;
      x[a]=(int)(d*Math.cos(rot));y[a]=(int)(d*Math.sin(rot));
//tip galaxy
...
//x[a]=(int)(len*Math.sin(rot));z[a]=(int)(len*Math.cos(rot));
  ...
  public MakeGalaxy(int x[],int y[], int z[],Choice ra) {
    this.x=x;this.y=y;this.z=z;r=new int[z.length];this.ra=ra;
    int a;
    for (a=0;a<races.length;a++) ra.add(races[a]);
  }

  public Color clookup(int a) {
    if (a==0) return Color.black;
    if (a==1) return Color.blue;
    if (a==2) return Color.red;
    if (a==3) return Color.green;
    if (a==4) return Color.pink;
    if (a==5) return Color.orange;
    if (a==6) return Color.magenta;
    if (a==7) return Color.cyan;
    if (a==8) return Color.lightGray;
...
//EVENT FUNCITONS
  ...
  public void mouseClicked(MouseEvent ME) {
    ...
    for (a=0;a<races.length;a++) if (na.equalsIgnoreCase(races[a])) count=a;
    boolean one=false;
    for (a=0;a<NUMPLANETS;a++) {
      int dx=x[a]-mx,dy=y[a]-my;
      if (Math.sqrt(dx*dx+dy*dy)<1) {r[a]=count;one=true;}
    }
    if (!one)
    for (a=0;a<NUMPLANETS;a++) {
      int dx=x[a]-mx,dy=y[a]-my;//,dz=z[a];
      if (Math.sqrt(dx*dx+dy*dy)<7) r[a]=count;
    }
    repaint();
  }
  ...
  String races[]={"Unknown","Terran","Brogians","Gecko","Mechbot","Rhesians","Zephyre","Piil","Free","Anarchy"};
  int ind[]={1,2,1,4,3,5,4,4,4,1},min[]={1,3,4,2,5,1,3,4,4,1},
      agr[]={1,3,5,4,1,3,1,4,4,1},tec[]={1,5,1,3,4,3,3,2,4,1};
  ...
  public void printPlanets() {
    ...
    for (a=0;a<x.length;a++) {
      ...
      System.out.println("minerals "+Stuff.trunc(Math.random()*10*min[r[a]]/5,2));
      System.out.println("technology "+Stuff.trunc(Math.random()*10*tec[r[a]]/5,2));
      System.out.println("agraculture "+Stuff.trunc(Math.random()*10*agr[r[a]]/5,2));
      System.out.println("industry "+Stuff.trunc(Math.random()*10*ind[r[a]]/5,2));
