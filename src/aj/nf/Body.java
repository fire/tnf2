...
public class Body {
	...
	public String getSectorScan(Active A, int sec) {
		...
		if(A.hasTech("T38")) {
			...
			for(int a = 0; a < size * 2; a++) {
				...
				r += a % 10 + " ";
				r += t.substring(0 + size * 2 * sec, size * 2 * sec + size * 2) + EOL;
			}
		}
		if(A.hasTech("T39")) {
			r += EOL + "Threm" + EOL;
		...
		if(A.hasTech("T40")) {
			...
			for(int a = 0; a < size * 2; a++) {
				...
				if(!A.hasTech("T42")) {
					while (t.indexOf("R")>0) {
						t=t.substring(0,t.indexOf("R"))+"H"+t.substring(t.indexOf("R")+1);
					}
					while (t.indexOf("r")>0) {
						t=t.substring(0,t.indexOf("r"))+"h"+t.substring(t.indexOf("r")+1);
					}
				...
				r += a % 10 + " ";
				String n = t.substring(0 + size * 2 * sec, size * 2 * sec + size * 2) + EOL;
				r += n;
			}
		}
		return r;
	...
	public DPoint getDPoint(long t) {
		...
		if (OB!=null) d=OB.getDPoint(t);
		double period = radious;
		//sec/rotation
		d = new DPoint(d.getX() + Math.cos(t / period) * radious, d.getY() + Math.sin(t / period) * radious);
		return d;
	}
	...
	public int getMaterialDensity(Location l, String na) {
		...
		for (int a=0;a<t.length;a++) {
			try {
				...
				System.out.println("MyError: bad resource found in density check "+t[a]);
			}
		}
		return 0;
	}
	...
	public String getResourceAtLocation(Location l) {
		...
		if (l==null) return "-";
		if (name.equals("Earth")) return ".";
		double mod=1;
		if (l.isLevel()) mod=Universe.SUBTERMOD;
		double TRACE = .1*mod;
		...
		for(int a = 0; a < solids.size(); a++) {
			MarketItem mi = Universe.getMarket().getItemByName((String)solids.elementAt(a));
		...
		for(int a = 0; a < solids.size(); a++) {
			MarketItem mi = Universe.getMarket().getItemByName((String)solids.elementAt(a));
			if (mi==null) {
				System.out.println("MyError: unknown material in "+this+" "+solids.elementAt(a));
			}
			else {
		...
		if(mother && pos.size()>0) {
			do {
				MarketItem mi = (MarketItem)pos.elementAt((int)(pos.size() * r.nextDouble()));
				if(res.indexOf("x"+mi.getName()) < 0) {
					if(res.length() > 0)res += MATSEP;
					res += ((int)(r.nextDouble() * 2) + 4) + "x" + mi.getName();
				}
			}
		...
		if(vein && pos.size()>0) {
			do {
				...
				if(res.indexOf("x"+mi.getName()) < 0) {
					...
					res += ((int)(r.nextDouble() * 2) + 2) + "x" + mi.getName();
				}
			}
		...
		if(trace && pos.size()>0) {
			do {
				MarketItem mi = (MarketItem)allpos.elementAt((int)(allpos.size() * r.nextDouble()));
				if(res.indexOf("x"+mi.getName()) < 0) {
					if(res.length() > 0) {
						res += MATSEP;
					}
		...
		if (sur.equalsIgnoreCase("w")) {
			for (int a=0;a< liquids.size();a++) {
				MarketItem mi = Universe.getMarket().getItemByName((String)liquids.elementAt(a));
				if (mi!=null) {
					if(res.length() > 0) {
						res += MATSEP;
					}
		...
		for (int a=0;a< gasses.size();a++) {
			MarketItem mi = Universe.getMarket().getItemByName((String)gasses.elementAt(a));
			if (mi!=null) {
				if(res.length() > 0) {
					res += MATSEP;
				}
...
//if (Main.DEBUG) System.out.println("DEBUG: resource check before deplete "+res);
...
//if (Main.DEBUG) System.out.println("DEBUG: Body "+l+" has depleted size of "+depleted.size());
		for (int a=0;a<depleted.size();a++) {
			...
			//if (Main.DEBUG) System.out.println("DEBUG: "+m+" amt="+mamnt+" mname="+mname+" mloc="+mloc);
			if (!mloc.equals(l.toString())) continue;
			//if (Main.DEBUG) System.out.println("DEBUG: Locaiton "+l+" has depleted "+m);
		...
		while (res.indexOf("x")>=0) res=res.substring(0,res.indexOf("x"))+"."+res.substring(res.indexOf("x")+1);
		//if (Main.DEBUG) System.out.println("DEBUG: resource check after deplete "+res);
//add liquids && gasses (after deplete so they don't deplete)
	...
	public void consume(String materialname, int amnt,Location lo) {
		...
		if (den>5) return;//don't deplete gass or liquids
		if (Main.DEBUG) System.out.println("DEBUG: deplete check for "+materialname+" "+amnt+" chance="+Math.pow(1-DEPLETERATE,amnt)+"%");
		boolean depleteIt=Math.pow(1-DEPLETERATE,amnt)<Math.random();
		if (depleteIt) {
			if (Main.DEBUG) System.out.println("DEBUG: deplete "+materialname);
			depleted.addElement("-1x"+materialname+"x"+lo.toString());
			depleted=Universe.mergeXList(depleted);
		}
	}
	
	public Body(double mass, double surfrad, String n, Location l, String o, double r, boolean sol, double at, double we, double mint, double maxt, double tet, double rad, Vector sv, Vector lv, Vector gv) {
		this.mass = mass;
...
//System.out.println("name "+name+" loc "+loc+" size "+size);
	...
	public void saveFile() {
		...
		s += "END\n";
		try {
			PrintWriter pw=new PrintWriter(new FileWriter(Main.DIRBODIES+fileName));
			pw.println(s);
			pw.close();
	...
	public void setSurfaceLocation(Location l,char ch) {
		if (Main.DEBUG) System.out.println("DEBUG: changing planet surface to "+ch);
		if (l==null) return;
		if (Main.DEBUG) System.out.println("DEBUG: changing planet surface 2 to "+ch);
		String t = "";
		int s = l.getSector();
		int r = l.getRow();
		...
		if(r >= geophysical.size()) {
			System.out.println("MyError: surf row loc out of bounds (" + r + ")" + l);
		}
		t = (String)geophysical.elementAt(r);
		if(c >= t.length()) {
			System.out.println("MyError: surf col loc out of bounds (" + c + ")" + l);
		}
		if (Main.DEBUG) System.out.println("DEBUG: changing planet surface 3 to "+ch);
		t=t.substring(0,c)+ch+t.substring(c+1,t.length());
		geophysical.setElementAt(t,r);
		if (Main.DEBUG) System.out.println("DEBUG: changing planet surface 4 to "+ch);
		saveFile();
		if (Main.DEBUG) System.out.println("DEBUG: changing planet surface 5 to "+ch);
	}

	
	public String getSystemReport() {
		String res=loc.toString()+"  "+name;
		if (orbits.equalsIgnoreCase("na")) res+=" a Star";
		else if (!solid) res+=" a gas giant";
		else if (loc.isPlanet()) res+=" a size "+size+" planet";
		else if (loc.isMoon() && loc.getMoon().toUpperCase().startsWith("A")) res+=" a size "+size+" minor planet";
		else if (loc.isMoon() && !loc.getMoon().toUpperCase().startsWith("A")) res+=" a size "+size+" moon";
		if (!orbits.equals("na")) {
			res+=" orbiting "+orbits+" at distance "+Stuff.trunc(radious/AU,2)+" AU  1G =";
			double acc = .61;
			double t = Math.pow(radious / 2 / acc, .5) * 2;
			t *= 1000;
			if (t>Universe.MONTH) res+=Stuff.trunc(t/Universe.MONTH,2)+" Months";
			else if (t>Universe.DAY) res+=Stuff.trunc(t/Universe.DAY,2)+" Days";
			else res+=Stuff.trunc(t/Universe.HOUR,2)+" Hours";
		}
		return res;
	...
	public String getRec(Location l) {
		...
		c = Math.max(0, Math.min(c, size * 2 * 4 - 1));
		if(r >= resource.size()) {
			System.out.println("MyError: temp row loc out of bounds (" + r + ")" + l);
		}
		t = (String)resource.elementAt(r);
		if(c >= t.length()) {
			System.out.println("MyError: temp col loc out of bounds (" + c + ")" + l);
		}
		t = t.substring(c, c + 1);
		if(ll >0 ) {
			...
			if (rr.nextDouble()<Universe.RECCHANCE*Universe.SUBTERMOD) {
				...
				if (rr.nextDouble()>Universe.MOTHERCHANCE*Universe.SUBTERMOD) {
					t=t.toLowerCase();
				}
			}
			else t=".";
	...
	public void readFile() {
		//read body file
		try {
			BufferedReader br = new BufferedReader(new FileReader(Main.DIRBODIES+fileName));
			while(true) {
...
//add C and c
	...
	public void print() {
		...
		Vector v = geophysical;
		for(int a = 0; a < v.size(); a++) {
			System.out.println((a < 10?"0" + a:"" + a) + ":" + v.elementAt(a));
		}
		v = thermal;
	...
	public static Body parse(GmlPair g) {
		...
		Location l = null;
		if(n != null)l = Location.parse(n.getString());
		if(l == null) {
			System.out.println("MyError: Bad location found in body parsing sval" + n.getString() + " nval " + n.getDouble());
			return null;
		}
		boolean s = true;
		n = g.getOneByName("solid");
		if(n != null)s = n.getDouble() != 0;
		double d = 1;
		n = g.getOneByName("distance");
		if(n != null)d = n.getDouble();
		double mass = 1;
		n = g.getOneByName("mass");
		if(n != null)mass = n.getDouble();
		double surfrad = 1;
		n = g.getOneByName("surfrad");
		if(n != null)surfrad = n.getDouble();
		String or = null;
		...
		if(n != null)we = n.getDouble();
		n = g.getOneByName("mintemp");
		if(n != null)mintemp = n.getDouble();
		n = g.getOneByName("maxtemp");
		if(n != null)maxtemp = n.getDouble();
		n = g.getOneByName("tectonic");
		if(n != null)tet = n.getDouble();
		n = g.getOneByName("radation");
		if(n != null)rad = n.getDouble();
		...
		Body b=new Body(mass, surfrad, na, l, or, d, s, at, we, mintemp, maxtemp, tet, rad, solids, liquids, gasses);
		n = g.getOneByName("resourceTypes");
		if(n != null) b.resourceTypes=n.getString();
	...
	public static void makeRandom(Location l,String type,String name,String orbits) {
		...
		for (int a=0;a<v.size();a++) {
			...
			else if (!b.solid && lll.isSolar() && type.equalsIgnoreCase("Star")) {
				mat.addElement(b);
			}
			else if (b.solid && lll.isPlanet() && type.equalsIgnoreCase("Planet")) {
				mat.addElement(b);
			}
			else if (lll.isMoon() && !lll.getMoon().toUpperCase().startsWith("A") && type.equalsIgnoreCase("Moon")) {
				mat.addElement(b);
			}
			else if (lll.isMoon() && lll.getMoon().toUpperCase().startsWith("A") && type.equalsIgnoreCase("Minor")) {
				mat.addElement(b);
			}
		}
		...
		for (int a=0;a<mat.size();a++) {
			Body b=(Body)mat.elementAt(a);
			if (min==0 && max==0) max=min=b.surfrad;
			min=Math.min(b.surfrad,min);
			max=Math.max(b.surfrad,max);
		}
		double surfrad=min+(max-min)*rate;
		//System.out.println(" surfrad min = "+min+"  max="+max+"  act="+surfrad);
		min=0;max=0;
		for (int a=0;a<mat.size();a++) {
			Body b=(Body)mat.elementAt(a);
			if (min==0 && max==0) max=min=b.radious;
			min=Math.min(b.radious,min);
			max=Math.max(b.radious,max);
		}
		double dist=min+(max-min)*Math.random();
		//System.out.println("  radious/dist min = "+min+"  max="+max+"  act="+dist);
		boolean solid=!type.equalsIgnoreCase("Gass") && !type.equalsIgnoreCase("STAR");
		...
		//System.out.println("RESULT "+name+" radious "+dist+" surfrad "+surfrad);
		Body newB=new Body(mass,surfrad,name,l,orbits,dist,solid,at,we,mintemp,maxtemp,tet,rad,new Vector(),new Vector(),new Vector());
