...
public class StockPile implements NFObject {
	...
	public boolean merge(StockPile s) {
		if (s == this)
			return false;
		if (s.corpTick.equals("AUCTION"))
			return false;// don't merge auction items
		if (s.corpTick.equalsIgnoreCase(corpTick)
				&& s.material.equalsIgnoreCase(material) && s.loc.equals(loc)) {
			amount += s.amount;
			s.amount = 0;
			return true;
		}
	...
	public String display() {
		return getId() + ": Stockpile of " + amount + " " + material + " at "
