...
public class GenPlanet {
	...
	public void generateAll(String s[]) {
		...
		for (int a = 0; a < v.size(); a++) {
			...
			if (s.length == 0 && !found
					&& B.getName().equalsIgnoreCase("Earth")) {
	...
	public String getThrm(int row, int col, double div) {
		...
		return ""
				+ Math.max(1, Math.min(
						(int) (thrm * 8 + 1 + Math.random() * 4 - 2), 9));
	}
	...
	public void makeResourceTypes(Market m, Body B) {
		...
		for (int a = 0; a < v.size(); a++) {
			...
			if (mi != null) {
				for (int b = 0; b < mi.occuranceRate * 20; b++)
					resourceTypes += mi.getType();
			} else {
				System.out.println("Unknown element " + s + " please fix in "
						+ B.getName());
			}
		}
	...
	public String[][] genGeo(Body B) {
		...
		for (int a = 0; a < nummount; a++) {
			nn[(int) (Math.random() * size * 2)][(int) (Math.random() * size
					* 2 * 4)] = "M";
		}
				...
				for (int b = 0; b < nn[a].length; b++) {
					if (nn[a][b].equalsIgnoreCase("P")
							&& ((a > 0 && nn[a - 1][b].equalsIgnoreCase("W"))
									|| (a < nn.length - 1 && nn[a + 1][b]
											.equalsIgnoreCase("W"))
									|| (b > 0 && nn[a][b - 1]
											.equalsIgnoreCase("W")) || (b < nn[a].length - 1 && nn[a][b + 1]
									.equalsIgnoreCase("W")))) {
						nn[a][b] = "w";
						a = -1;
						b = -1;
	...
	public String[][] softenIt(String nn[][]) {
		...
		for (int a = 0; a < nn.length; a++) {
			for (int b = 0; b < nn[0].length; b++) {
				int mc = 0, bc = 0;
				// -1 0
				if (a > 0 && nn[a - 1][b].equals("B"))
					bc += 10;
				if (a > 0 && nn[a - 1][b].equals("M"))
					mc += 10;
				if (a > 0 && nn[a - 1][b].equals("b"))
					bc += 1;
				if (a > 0 && nn[a - 1][b].equals("m"))
				...
				if (Math.random() > ROUNDIT) {
					if (a > 0 && b > 0 && nn[a - 1][b - 1].equals("B"))
						bc += 10;
					if (a > 0 && b > 0 && nn[a - 1][b - 1].equals("M"))
						mc += 10;
					if (a > 0 && b > 0 && nn[a - 1][b - 1].equals("b"))
						bc += 1;
					if (a > 0 && b > 0 && nn[a - 1][b - 1].equals("m"))
				...
				// 0 -1
				if (b > 0 && nn[a][b - 1].equals("B"))
					bc += 10;
				if (b > 0 && nn[a][b - 1].equals("M"))
					mc += 10;
				if (b > 0 && nn[a][b - 1].equals("b"))
					bc += 1;
				if (b > 0 && nn[a][b - 1].equals("m"))
				...
				// +1 0
				if (a < nn.length - 1 && nn[a + 1][b].equals("B"))
					bc += 10;
				if (a < nn.length - 1 && nn[a + 1][b].equals("M"))
					mc += 10;
				if (a < nn.length - 1 && nn[a + 1][b].equals("b"))
					bc += 1;
				if (a < nn.length - 1 && nn[a + 1][b].equals("m"))
				...
				// +1 +1
				if (Math.random() > ROUNDIT) {
					if (a < nn.length - 1 && b < nn[a + 1].length - 1
							&& nn[a + 1][b + 1].equals("B"))
						bc += 10;
					if (a < nn.length - 1 && b < nn[a + 1].length - 1
							&& nn[a + 1][b + 1].equals("M"))
				...
				// 0 +1
				if (b < nn[a].length - 1 && nn[a][b + 1].equals("B"))
					bc += 10;
				if (b < nn[a].length - 1 && nn[a][b + 1].equals("M"))
					mc += 10;
				if (b < nn[a].length - 1 && nn[a][b + 1].equals("b"))
					bc += 1;
				if (b < nn[a].length - 1 && nn[a][b + 1].equals("m"))
