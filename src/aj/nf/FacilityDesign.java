...
public class FacilityDesign implements ITThing {
	...
	public FacilityDesign(String n, Vector p, Vector f, Vector pc, double lp, int sid) {
	...
	public boolean hasTech(String s) {
		for(int a = 0; a < functions.size(); a++) {
			...
			if(sss.toUpperCase().equals(s.toUpperCase()))return true;
		}
		return false;
	}
	...
	public void buyMarketItems() {
		...
		for(int a = 0; a < v.size(); a++) {
			...
			try {
				...
				System.out.println("MyError: bad buyMarketItems in Facility design");
			}
		}
	}
	...
	public String display(String tic) {
		...
		boolean sec=isPartner("none")||name.toUpperCase().startsWith("BASIC");
		pub=pub&&!sec;
		part=part&&!sec;
		priv=priv&&!sec;
...
//TODO check for existing actives using design before delete
	...
	public static FacilityDesign parse(GmlPair g) {
		...
		for(int a = 0; a < n.length; a++) {
			if(n[a] == null)continue;
			String tt[] = Stuff.getTokens(n[a].getString());
			for(int b = 0; b < tt.length; b++)f.addElement(tt[b]);
		}
		...
		for(int a = 0; a < n.length; a++) {
			if(n[a] == null)continue;
			String tt[] = Stuff.getTokens(n[a].getString());
			for(int b = 0; b < tt.length; b++)pc.addElement(tt[b]);
		}
	...
	public Vector getProvideList() {
		Vector v=new Vector();
		if (hasTech("T84")) return v;
		for(int a = 0; a < functions.size(); a++) {
			String s = (String)functions.elementAt(a);
			Tech t = Universe.getTechByName(s);
			if (t==null) {
				System.out.println("MyError: bad provide list in facilitydesign "+getId());
				continue;
			}
