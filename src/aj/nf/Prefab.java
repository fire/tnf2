...
public class Prefab implements NFObject {
	...
	public boolean canConstructAt(Location l) {
		...
		if (facilityDesign.hasTech("T51")) {// distil
			if (!b.getSurf(l).equalsIgnoreCase("w"))
				return false;
		}
		...
		if (l.isLevel() && facilityDesign.hasTech("T65"))
			return false;// solar on surface or orbit only
		if (facilityDesign.hasTech("T100")) {// geothermal
			if (!l.isLevel())
				return false;
			if (b == null || b.tectonic == 0)
				return false;
		}
		...
		if (!l.isOrbit() && facilityDesign.hasTech("T102"))
			return false;// magentic
		return true;
	}
	...
	public String display() {
		return getId() + ": " + facilityDesign.getName() + " at "
				+ loc.toString() + " " + Stuff.money(getValue());
	}
