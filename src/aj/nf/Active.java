...
public class Active implements NFObject {
	...
	public Active(String cn, String rdid, Location l, long ft, String fc,
			String ls, Hashtable vs) {
		finishTime = ft;
		finishCommand = fc;
		lines = ls;
		vars = vs;
	...
	public Active(String cn, String rdid, Location l, int sid, long ft,
			String fc, String ls, Hashtable vs) {
		finishTime = ft;
		finishCommand = fc;
		lines = ls;
		vars = vs;
	...
	public int getCargoMass() {
		...
		// if (Main.DEBUG) System.out.println("DEBUG: "+v.size()+" objects found
		// inside "+getId()+" with inside of "+getInsideLoc());
		...
		if (all.equals(""))
			all = "-";
		// if (Main.DEBUG && !ori.equals(all)) System.out.println("DEBUG:
		// cargomass error fixed "+ori+" to "+all);
		// if (Main.DEBUG) System.out.println("DEBUG: cargomass called with
		// carog= "+all);
		vars.put("_cargo", all);
		return tot;
	}
	...
	public void attackedBy(Active A) {
		...
		if (hasTech("T20"))
			armtype += 1;// lowtemp
		if (hasTech("T21"))
			armtype += 1;// very lowtemp
		if (hasTech("T22"))
			armtype += 1;// hight temp
		if (hasTech("T23"))
			armtype += 1;// very high temp
		if (hasTech("T24"))
			armtype += 3;// earthquake
		if (hasTech("T25"))
			armtype += 1;// rad
		if (hasTech("T26"))
			armtype += 1;// heavy rad
		if (hasTech("T27"))
			armtype += 3;// pressure fit
		if (hasTech("T28"))
			armtype += 5;// extra pressure fit
		if (hasTech("T29"))
			armtype += 1;// armor
		if (hasTech("T30"))
			armtype += 4;
	...
	public void applyDamage(double d) {
		...
		if (Main.DEBUG)
			System.out.println("DEBUG: damage warning = " + getId() + " "
					+ getLocation() + " " + s + " landing");
		vars.put("_damage", s + "");
		int count = 0;
		if (comp > 50)
			count++;
		...
		if (hull > 50)
			count++;
		if (count > 3 || comp >= 100 || sens >= 100 || eng >= 100
				|| weap >= 100 || hull >= 100) {
			destroy();
		}
	}
	...
	public void enviromentalEffects() {
		...
		if (isRobot()) {
			try {
				double e = Stuff.parseDouble(s);
				e = Math.max(0, (e * Universe.MONTH - Universe.CHECKTIME)
						/ Universe.MONTH);
				vars.put("_endurance", "" + Stuff.trunc(e, 2));
			} catch (NumberFormatException nfe) {
				System.out.println("MyError:  Bad endurance found " + s);
		...
		boolean liquid = Universe.getSurfaceByLocation(loc).equalsIgnoreCase(
				"W");
	...
	public void uploadProgram(String s, String d) {
		if (finishTime == -1) {
			...
			if (s.length() > 0) {
				if (Main.DEBUG)
					System.out.println("MYDEBUG: program truncated.  Too long");
			}
			s = s2;
			// count lines
			lines = s;
	...
	public String displayHeader() {
		String s = "";
		s += (nick.length() > 0 ? nick + ":" : "") + getId() + ":"
				+ (activeDesign.isShip() ? "SHIP" : "ROBOT") + ": "
				+ activeDesign.getName() + " at " + loc.toString();
		if (!isPowered()) {
			s += " Out of power ";
			if (finishTime == -1)
				s += " Not running.";
		...
		s += " End (" + vars.get("_endurance") + "/"
				+ activeDesign.getDesignEndurance() + ") Dam "
				+ Stuff.trunc(dr * 100, 2) + "%";
	...
	public String display() {
		...
		if (verbose) {
			...
			while (lines.indexOf(";_") >= 0) {
				...
				if (hide.indexOf(";") >= 0)
					hide = hide.substring(0, hide.indexOf(";"));
				String rep = hide;
				if (rep.indexOf(":") >= 0)
					rep = rep.substring(1, rep.lastIndexOf(":") + 1)
							+ "***LEASED CODE***";
				else
					rep = "***LEASED CODE***";
				lines = lines.substring(0, lines.indexOf(hide)) + rep
						+ lines.substring(lines.indexOf(hide) + hide.length());
			}
			...
			if (finishCommand.indexOf(":") >= 0) {
				finishCommand = finishCommand.substring(0, finishCommand
						.lastIndexOf(":"));
			}
			if (finishCommand.length() > 0
					&& lines.indexOf(" " + finishCommand) >= 0) {
				lines = lines.substring(0, lines.indexOf(finishCommand)) + "*"
						+ lines.substring(lines.indexOf(finishCommand));
			}
			s += "\n" + lines;
		}
		s += "\n" + log.toString();
	...
	public GmlPair toGmlPair() {
		...
		while (keys.hasMoreElements()) {
			...
			if (!key.equalsIgnoreCase("_location")
					&& !key.equalsIgnoreCase("_time")
					&& !key.equalsIgnoreCase("_cc")
					&& !key.equalsIgnoreCase("_cm")) {
	...
	public void loadNextCommand() {
		...
		if (finishTime == 0) {
			...
			while (cp.toUpperCase().indexOf("VAR ") >= 0) {
				String key = cp.substring(cp.toUpperCase().indexOf("VAR ") + 3)
						.trim();
				...
				for (int a = 0; a < allkey.length; a++) {
					if (!allkey[a].startsWith("$")) {
						log("In line: " + finishCommand
								+ " WARNING: Invalid var name in declaration "
								+ allkey[a] + " assuming $" + allkey[a]);
					} else {
						allkey[a] = allkey[a].substring(1);
					}
					if (!vars.containsKey(allkey[a])
							&& !allkey[a].startsWith("_")) {
						vars.put(allkey[a], "-");
					}
				}
			}
			...
			if (finishCommand.indexOf(":") >= 0)
				finishCommand = finishCommand.substring(0, finishCommand
						.lastIndexOf(":") + 1);
			String next = lines.substring(
					lines.indexOf(finishCommand) + finishCommand.length())
					.trim();
			if (next.indexOf(";") < 0) {
		...
		if (Main.DEBUG)
			System.out.println("DEBUG: Timing command " + finishCommand);
		if (Main.DEBUG)
			System.out.println("DEBUG: execute time = "
					+ Stuff.trunc(1.0 * tc / DAY, 2) + " days");
		if (finishTime >= 0)
			finishTime = currTime + tc;
	}

	public void checkCommand(String s) {
		if (Main.DEBUG)
			System.out.println("DEBUG: check command " + s);
		while (s.indexOf(":") >= 0)
			s = s.substring(s.indexOf(":") + 1);
		...
		if (t.startsWith("IF ")) {
			...
			if (t.indexOf("ELSE ") > 0
					&& t.indexOf("ELSE ") < t.lastIndexOf("THEN ")) {
		...
		if (t.startsWith("VAR ") || t.startsWith("LOG ")
				|| t.startsWith("SEND ") || t.startsWith("REM"))
			return;
		if (t.startsWith("SET ")) {
			t = t.substring(4);
		...
		if (n[0].equals("NOOP") || n[0].equals("RETURN") || n[0].equals("EXIT")
				|| n[0].equals("VERBOSE") || n[0].equals("BRIEF")) {
			if (n.length != 1) {
		...
		if (n[0].equals("REFUEL")) {
			if (n.length != 1 && n.length != 2 && n.length != 3) {
				terminate("Zero or One parameter required in use of " + n[0]
						+ " :" + t);
			}
		}
		...
		if (n[0].equals("GOTO") || n[0].equals("WAIT") || n[0].equals("MOVE")
				|| n[0].equals("ATTACK") || n[0].equals("CONSTRUCT")
				|| n[0].equals("GOSUB") || n[0].equals("RECOVER")) {
			if (n.length != 2) {
				terminate("Exactly one parameter required in use of " + n[0]
						+ " :" + t);
			}
		}
		if (n[0].equals("REFINE") || n[0].equals("COLLECT")) {
			if (n.length != 3) {
				terminate("Exactly two parameter required in use of " + n[0]
						+ " : " + t);
			}
		}
		if (n[0].equals("LOAD")) {
			if (n.length > 4 || n.length < 2) {
				terminate("Exactly one, two, or three parameters required in use of "
						+ n[0] + " :" + t);
			}
		}
		if (n[0].equals("REPAIR") || n[0].equals("UNLOAD")
				|| n[0].equals("PRODUCE") || n[0].equals("SCAN")
				|| n[0].equals("SCANLOG")) {
			if (n.length != 3 && n.length != 2) {
				terminate("Exactly one or two parameters required in use of "
						+ n[0] + " :" + t);
			}
		}
	}
	...
	public static String validCommand(String S) {
		...
		if (t.startsWith("IF ")) {
			...
			String case1 = t.substring(t.toUpperCase().indexOf(" THEN ") + 6)
					.trim();
			String case2 = "NOOP";
			if (case1.toUpperCase().indexOf(" ELSE ") > 0) {
				case2 = case1.substring(
						case1.toUpperCase().indexOf(" ELSE ") + 6).trim();
				case1 = case1.substring(0,
						case1.toUpperCase().indexOf(" ELSE ")).trim();
			}
		...
		} else if (n[0].equals("NOOP") || n[0].equals("RETURN")
				|| n[0].equals("EXIT") || n[0].equals("VERBOSE")
				|| n[0].equals("BRIEF")) {
			if (n.length != 1) {
				return "INVALID COMMAND>:Wrong number of args." + S;
			}
		...
		} else if (n[0].equals("REPAIR") || n[0].equals("UNLOAD")
				|| n[0].equals("PRODUCE")) {
			...
			if (!n[1].startsWith("TEM") && !n[1].startsWith("SUR")
					...
					&& !n[1].startsWith("SYS") && n[1].indexOf("$") < 0) {
				return "INVALID COMMAND>:SCAN types must be SYS,SEC,TEM,SUR,RES,STO,ROB,SHI,FAC,OBJ,TAR, or PRE."
						+ S;
			}
		} else {
	...
	public long timeCommand(String s) {
		...
		if (Main.DEBUG)
			System.out.println("DEBUG: Timing command " + s);
		...
		if (s.toUpperCase().startsWith("REM")
				|| s.toUpperCase().startsWith("NOOP")
				|| s.toUpperCase().startsWith("SET ")
				|| s.toUpperCase().startsWith("VAR ")
				|| s.toUpperCase().startsWith("VERBOSE")
				|| s.toUpperCase().startsWith("BRIEF")) {
			return MINUTE * 10;
		}
		if (s.toUpperCase().startsWith("IF ")) {
			...
			String test = s.substring("IF ".length(),
					s.toUpperCase().indexOf(" THEN ")).trim();
			test = UniVar.getValue(subVars(test) + "") + "";// required

			String case1 = s.substring(s.toUpperCase().indexOf(" THEN ") + 6)
					.trim();
			...
			if (Main.DEBUG)
				System.out.println("DEBUG: case1=" + case1 + " time case1="
						+ timeCommand(case1));
			if (Main.DEBUG)
				System.out.println("DEBUG: case2=" + case2 + " time case2="
						+ timeCommand(case2));
			...
			if (Main.DEBUG)
				System.out.println("DEBUG: subCommand=" + subCommand
						+ " time subCommand=" + timeCommand(subCommand));
		...
		if (s.toUpperCase().startsWith("WAIT ")) {
			...
			try {
				...
				if (d.length == 1) {
					del = (long) (HOUR * Math.max(0, Math.min(24, Stuff
							.parseDouble(d[0]))));
				}
				if (d.length == 2) {
					del = (long) (HOUR * Math.max(0, Math.min(24, Stuff
							.parseDouble(d[1]))));
					del += (long) (DAY * Math.max(0, Math.min(31, Stuff
							.parseDouble(d[0]))));
				}
				if (d.length == 3) {
					del = (long) (HOUR * Math.max(0, Math.min(24, Stuff
		...
		if (s.toUpperCase().startsWith("LOAD ")) {
			...
			if (n.length < 2 || n.length > 4) {
				if (Main.DEBUG)
					log("DEBUG: error in args in LOAD");
				return ERRORTIME;
			}
			...
			if (n.length > 2) {
				...
				if (loadloc != null) {
					...
					if (moveType == Location.INTERPLANET
							|| moveType == Location.ESCAPE
							|| moveType == Location.REENTRY) {
						return ERRORTIME;
					}
					// SECURITY HERE
					if (loadloc.isInside()) {
						NFObject nfoo = Universe.getNFObjectById(loadloc
								.getInsideWhat());
						if (nfoo instanceof Active) {
							Active aaa = (Active) nfoo;
							if (aaa.hasTech("T57")
									&& !aaa.getCorpTick().equalsIgnoreCase(
											corpTick)) {
								return ERRORTIME;
							}
						}
			...
			if (nfo == null && Market.getMarketName(n[1]) == null) {
				if (Main.DEBUG)
					log("DEBUG: error material name in LOAD");
				return ERRORTIME;
			}
			if (nfo == null) {
				...
				totalMass = cc;
				if (Main.DEBUG)
					log("DEBUG: material load found");
				if (Main.DEBUG)
					log("DEBUG: cargo capacity = " + totalMass);
				try {
					if (n.length == 3) {
						totalMass = (int) (Stuff.parseDouble(n[2]));
						if (Main.DEBUG)
							log("DEBUG: limit number found= " + totalMass);
					}
				} catch (NumberFormatException NFE) {
					if (Main.DEBUG)
						log("DEBUG: error in number in LOAD");
					return ERRORTIME;
				}
				int ava = Universe.availableMaterial(loadloc, n[1]);
				if (Main.DEBUG)
					log("DEBUG: avail limit number found= " + ava);
				totalMass = Math.min(totalMass, ava);
				if (Main.DEBUG)
					log("DEBUG: new totalmass found = " + totalMass);
			} else if (nfo != null) {
				if (nfo == this) {
					if (Main.DEBUG)
						log("DEBUG: error in self load in LOAD");
					return ERRORTIME;
				}
				if (nfo instanceof Facility) {
					if (Main.DEBUG)
						log("DEBUG: error in load cannot load facility ");
					return ERRORTIME;
				}
				if (!nfo.getLocation().equals(loadloc)) {
					if (Main.DEBUG)
						log("DEBUG: error in bad locaiton in LOAD");
					return ERRORTIME;
				}
				if (cc < nfo.getMass()) {
					if (Main.DEBUG)
						log("DEBUG: error over full cargo requested in LOAD");
					return ERRORTIME;
				}
			...
			if (!hasTech("T35") && !hasTech("T36") && !hasTech("T88")) {
				if (Main.DEBUG)
					log("DEBUG: error bad tech in LOAD");
				return ERRORTIME;
			}
			if (totalMass <= 0) {
				if (n.length > 2)
					if (Main.DEBUG)
						log("DEBUG: error negitiave load in LOAD =" + totalMass
								+ " n[1]=" + n[1] + " n[2]=(" + n[2] + ")"
								+ " parsed=" + (int) Stuff.parseDouble(n[2]));
				if (n.length == 2)
					if (Main.DEBUG)
						log("DEBUG: error negitiave load in LOAD =" + totalMass
								+ " n[1]=" + n[1]);
			...
			if (Main.DEBUG)
				log("DEBUG: load for " + totalMass + " requested");
			;
			if (hasTech("T88"))
		...
		if (s.toUpperCase().startsWith("UNLOAD ")) {
			...
			if (nfo == null && Market.getMarketName(n[1]) != null) {
				n[1] = Market.getMarketName(n[1]);
				totalMass = Universe.availableMaterial(Location.parse("INSIDE."
						+ getId()), n[1]);
				try {
					if (n.length == 3)
						totalMass = Math.max(0, Math.min(totalMass, (int) Stuff
								.parseDouble(n[2])));
				} catch (NumberFormatException NFE) {
					return ERRORTIME;
				}
			} else if (nfo != null) {
				if (!nfo.getLocation().equalsIgnoreCase(
						Location.parse("INSIDE." + getId()))) {
					return ERRORTIME;
				}
				totalMass = nfo.getMass();
			} else
		...
		if (s.toUpperCase().startsWith("SCAN ")
				|| s.toUpperCase().startsWith("SCANLOG ")) {
			s = UniVar.getValue(subVars(s) + "") + "";
			if (s.startsWith("\""))
				s = s.substring(1);
			if (s.endsWith("\""))
		...
		if (s.toUpperCase().startsWith("MOVE ")) {
			...
			if (moveType == Location.LOAD || moveType == Location.UNLOAD) {
				return HOUR;
				// tech mobility check or ERRORTIME is HOUR so no error check
				// required
			}
			if (moveType == Location.SURFACE) {
				...
				if (atm && hasTech("T6"))
					return DAY / 4;// wings
				if (Universe.locationContainsTechnology(l, "T58")
						&& hasTech("T1"))
					return DAY / 3;// wheels on road
				if (Universe.locationContainsTechnology(l, "T58"))
					return DAY;// road
				if (Universe.locationContainsTechnology(l, "T59"))
					return DAY;// tunnel

				if (hasTech("T8") && l.isOrbit())
					return HOUR;
				if (sur.equalsIgnoreCase("P"))
					return DAY;
				if (sur.equalsIgnoreCase("W"))
					return 2 * DAY;
				if (sur.equalsIgnoreCase("B"))
					return 2 * DAY;
				if (sur.equalsIgnoreCase("M"))
					return 4 * DAY;
				return DAY;
			}
			...
			if (moveType == Location.ESCAPE) {
				...
				// TODO todo check if escape facility has takeoff ability
				// LANDINGZONE or error time
				return DAY;
			}
			if (moveType == Location.INTERPLANET) {
				double acc = .62;
		...
		if (s.toUpperCase().startsWith("REFINE ")) {
			...
			int ava = Universe.availableMaterial(Location.parse("INSIDE."
					+ getId()), reqmat);
		...
		if (s.toUpperCase().startsWith("SALVAGE ")) {
			...
			if (!Universe.locationHasTechnology(loc, "T48") && !hasTech("T48")) {
				...
				if (n.length == 2) {
					try {
						ava = Math.max(0, Math.min(ava, (int) Stuff
								.parseDouble(n[1])));
					} catch (NumberFormatException nfe) {
						return ERRORTIME;
					}
				...
				if (Universe.locationHasTechnology(loc, "T48")
						&& hasTech("T48")) {
		...
		if (s.toUpperCase().startsWith("PRODUCE ")) {
			...
			if (n.length == 2) {
				...
				if (itt instanceof ActiveDesign) {
					...
					int amt2 = availableMaterialList(Location.parse("INSIDE."
							+ getId()), loc, att.getAllProductionCosts());
					if (amt2 < 1)
						return ERRORTIME;
					if (Main.DEBUG)
						System.out
								.println("DEBUG: active PRODUCE AMOUNT CHECK available ="
										+ amt2);
					return (long) (HOUR + att.getDesignMass() * PRODRATE);
				} else if (itt instanceof FacilityDesign) {
					FacilityDesign att = (FacilityDesign) itt;
					int amt2 = att.getDesignMass()
							* availableMaterialList(Location.parse("INSIDE."
									+ getId()), loc, att
					...
					if (Main.DEBUG)
						System.out
								.println("DEBUG: facility PRODUCE AMOUNT CHECK  available ="
										+ amt2);
					return (long) (HOUR + att.getDesignMass() * PRODRATE);
				} else {
					return ERRORTIME;
				...
				try {
					...
					if (mi.getId().equals("M1")) {
						...
						if (!Universe.locationHasTechnology(loc, "T49")
								&& !Universe.locationHasTechnology(loc, "T50")) {
							return ERRORTIME;
						}
						int amt2 = 0;
						if (!Universe.locationHasTechnology(loc, "T49")) {
							amt2 += availableMaterialList(Location
									.parse("INSIDE." + getId()), loc, "M70");
						}
						if (!Universe.locationHasTechnology(loc, "T50")) {
							amt2 += availableMaterialList(Location
									.parse("INSIDE." + getId()), loc, "M68");
							amt2 += availableMaterialList(Location
									.parse("INSIDE." + getId()), loc, "M69");
						}
						...
						amt = amt
								+ (amt % unitsize != 0 ? unitsize
										- (amt % unitsize) : 0);
						if (Main.DEBUG)
							System.out.println("DEBUG: Material id " + n[1]
									+ " is " + mat + " is " + mi.getName()
									+ " with prod of "
									+ mi.getProducedMaterial());
						int amt2 = unitsize
								* availableMaterialList(Location
										.parse("INSIDE." + getId()), loc, mi
										.getProducedMaterial());
						if (Main.DEBUG)
							System.out
									.println("DEBUG: PRODUCE AMOUNT CHECK asked for "
											+ amt + " available =" + amt2);
						amt = Math.min(amt2, amt);
						return (long) (HOUR + amt * PRODRATE);
					} else {
		...
		if (s.toUpperCase().startsWith("RECHARGE")) {
			...
			if (n.length == 1) {
				if (loc.isInside()) {
					NFObject nfo = Universe
							.getNFObjectById(loc.getInsideWhat());
					if (nfo == null) {
						return ERRORTIME;
					}
					if (nfo == this) {
		...
		if (s.toUpperCase().startsWith("REFUEL ")) {
			...
			String tt = s.toUpperCase().trim();
			String n[] = Stuff.getTokens(tt, " \"\t");
			// no error checking required error time== execute time (error =
			// 2hour not one hour)
			if (n.length > 3)
				return ERRORTIME;
			return HOUR * 2;
		...
		if (s.toUpperCase().startsWith("REPAIR ")) {
			...
			if (n.length == 3) {
				try {
					amt = Math.max(0, Math.min(amt, (int) Stuff
							.parseDouble(n[2])));
				} catch (NumberFormatException nfe) {
					return ERRORTIME;
				}
			...
			if (Main.DEBUG)
				System.out.println("DEBUG: repair test parts ava " + ava);
			if (ava == 0 || amt == 0) {
				return ERRORTIME;
			}
			if (ava < amt) {
			...
			if (Main.DEBUG)
				System.out.println("DEBUG: repair test mass used " + amt);
			if (Universe.locationHasTechnology(loc, "T47") && hasTech("T47"))
				return HOUR + amt * REPAIRRATEFAC / 2;
			else if (Universe.locationHasTechnology(loc, "T47"))
				return HOUR + amt * REPAIRRATEFAC;
			else if (hasTech("T47"))
				return HOUR + amt * REPAIRRATE;
			else
				return ERRORTIME;
		}
		...
		if (s.toUpperCase().startsWith("ATTACK ")) {
			...
			NFObject nfo = Universe.getNFObjectById(n[1]);
			if (nfo == null || nfo instanceof StockPile
					|| nfo instanceof Prefab || nfo == this) {
				return ERRORTIME;
			}
	...
	public String subRandom(String s) {
		while (s.toLowerCase().indexOf("$_random") >= 0) {
			s = s.substring(0, s.toLowerCase().indexOf("$_random"))
					+ Math.random()
					+ s.substring(s.toLowerCase().indexOf("$_random")
							+ "$_random".length());
		}
		return s;
	}
	...
	public String subVars(String s) {
		...
		while (keys.hasMoreElements()) {
			...
			while (s.toLowerCase().indexOf("$" + key) >= 0) {
				s = s.substring(0, s.toLowerCase().indexOf("$" + key))
						+ val
						+ s.substring(s.toLowerCase().indexOf("$" + key) + 1
								+ key.length());
			}
		}
		return s;
	...
	public void executeCommand() {
		...
		if (Main.DEBUG)
			System.out.println("DEBUG: " + getId() + " Executing command ("
					+ finishCommand + ")");
		...
		if (workingCommand.indexOf(":") >= 0) {
			workingCommand = workingCommand.substring(
					workingCommand.lastIndexOf(":") + 1).trim();
		}
		if (Main.DEBUG)
			System.out.println("DEBUG: working command " + workingCommand);
		...
		// NOOP and VAR
		if (workingCommand.toUpperCase().startsWith("NOOP")
				|| workingCommand.toUpperCase().startsWith("VAR ")
		...
		// SET <VAR>=<EQ>
			...
			if (!vars.containsKey(var)) {
				log("In line " + workingCommand
						+ " WARNING: Undefined varabile " + var);
				return;
			}
			if (var.startsWith("_") && !Main.DEBUG) {
				log("In line " + workingCommand
						+ " WARNING: Cannot SET readonly varabile " + var);
				return;
			}
		...
		// LOG
			...
			if (lines.indexOf(line) <= 0) {
				log("In line: " + workingCommand
						+ " WARNING: Undefined line refrence " + eq);
				log("Program Terminated: ");
				finishCommand = "";
				finishTime = -1;
				return;
			}
			// if(Main.DEBUG)System.out.println("DEBUG: label to gosub to " +
			// line);
			...
			// if(Main.DEBUG)System.out.println("DEBUG finishCommand=" +
			// finishCommand);
			if (Main.DEBUG)
				System.out.println("DEBUG gotoLine=" + gotoLine);
			String next = lines.substring(
					lines.indexOf(finishCommand) + finishCommand.length())
					.trim();
			if (next.indexOf(";") < 0) {
		...
		// IF
			...
			String test = workingCommand.substring("IF ".length(),
					workingCommand.toUpperCase().indexOf(" THEN ")).trim();
			test = UniVar.getValue(subVars(test) + "") + "";
			String case1 = workingCommand.substring(
					workingCommand.toUpperCase().indexOf(" THEN ") + 6).trim();
		...
		// SCAN
		else if (workingCommand.toUpperCase().startsWith("SCAN ")
				|| workingCommand.toUpperCase().startsWith("SCANLOG ")) {
			boolean dolog = workingCommand.toUpperCase().startsWith("SCANLOG ");
			String eq = workingCommand.substring(workingCommand.indexOf(" "))
					.trim();
			eq = UniVar.getValue(subVars(eq) + "") + "";
			if (Main.DEBUG)
			...
			// SCAN <eq> : scan
			// terrain,resorces,temp,objects,targets,facilities,location
			String n[] = Stuff.getTokens(eq, " \t");
			// SCAN TEMP
			if (n[0].toUpperCase().startsWith("TEM")) {
				Location loc = this.loc;
				if (n.length == 2)
					loc = Location.parse(n[1]);
				if (loc == null || !loc.valid())
					loc = this.loc.reference(n[1]);
				if (loc == null || !loc.valid()) {
					log("In line: " + workingCommand
							+ " WARNING: invalid location " + n[1]
							+ " Scan canceled");
					return;
				}
				if (!Location.adjacentTo(loc, this.loc)) {
					log("In line: " + workingCommand
							+ " WARNING: Nonadjacent location " + eq
							+ " Scan canceled");
					return;
				}
				if (!hasTech("T39")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: No ThermalImage technology cannot do temp scan.");
					return;
				}
				if (dolog)
					log("Scan Temp: " + loc + " = "
							+ Universe.getTempByLocation(loc));
				vars.put("arg1", "" + Universe.getTempByLocation(loc));
			} else if (n[0].toUpperCase().startsWith("SYS")) {
				if (!hasTech("T95")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: No telescope technolgy availalbe.  Scan failed.");
					return;
				}
				if (!loc.isOrbit()) {
					log("In line: " + workingCommand
							+ " WARNING: Must be in orbit to scan system.  "
							+ loc + " is not.");
					return;
				}
				...
				if (n.length == 2) {
					try {
						...
						log("In line: " + workingCommand
								+ " WARNING: Bad sector " + eq + "");
						return;
					}
				}
				if (sec < 0 || sec > 3) {
					log("In line: " + workingCommand + " WARNING: Bad sector "
							+ eq + "");
					return;
				}
				Body b = Universe.getBodyByLocation(loc);
				if (b == null || !loc.isOrbit()) {
					log("In line: " + workingCommand
							+ " WARNING: Cannot scan sector from " + loc);
					return;
				}
			...
			// SCAN SURFACE
				...
				if (!hasTech("T38")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: No LowLightVideo technology cannot do surface scan.");
					return;
				}
				if (dolog)
					log("Scan Surface: " + loc + " = "
							+ Universe.getSurfaceByLocation(loc));
			...
			// SCAN RESOURCES
				...
				if (!hasTech("T40")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: No Spectroscope technology cannot do resource scan.");
					return;
				}
				String res = Universe.getResourcesByLocation(loc);
				if (dolog)
					log("Scan Resources: " + loc + " = " + res);
				vars.put("arg1", res);
			}
			// SCAN STOCKPILES
				...
				if (!hasTech("T37")) {
					log("In line: " + workingCommand
							+ " WARNING: No Radar technology cannot do scan.");
					return;
				}
				if (ans.length() == 0)
					ans = "-";
				if (dolog)
					log("Scan Stockpiles: " + loc + " = " + ans);
				vars.put("arg1", ans);
			}
			// SCAN ROBOTS
			...
			// SCAN OBJECTS
				...
				Vector v = Universe.getNFObjectsByLocation(loc);
				for (int a = 0; a < v.size(); a++) {
					if (v.elementAt(a) instanceof NFObject
							&& v.elementAt(a) != this) {
						if (v.elementAt(a) instanceof Active) {
							Active A = (Active) v.elementAt(a);
							if (A.hasTech("T94"))
			...
			// SCAN TARGETS
				...
				Vector v = Universe.getNFObjectsByLocation(loc);
				for (int a = 0; a < v.size(); a++) {
					if ((v.elementAt(a) instanceof Facility || v.elementAt(a) instanceof Active)
							&& v.elementAt(a) != this
							&& !((NFObject) (v.elementAt(a))).getCorpTick()
									.equalsIgnoreCase(corpTick)) {
						if (v.elementAt(a) instanceof Active) {
							Active A = (Active) v.elementAt(a);
							if (A.hasTech("T94"))
								continue;
				...
				if (ans.length() == 0)
					ans = "-";
				if (dolog)
					log("Scan Targets: " + loc + " = " + ans);
				vars.put("arg1", ans);
			} else {
				log("In line: "
						+ workingCommand
						+ " WARNING: Bad Scan command "
						+ n[0]
						+ ". Use SYS,SEC,TEM,SUR,RES,STO,ROB,SHI,FAC,OBJ,TAR or PRE");
			}
		}
		// MOVE
			...
			if (l == null) {
				log("In line: " + workingCommand
						+ " WARNING: no such location " + eq + " Move canceled");
				return;
			}
			if (l.equals(getInsideLoc())) {
				log("In line: " + workingCommand
						+ " WARNING: cannot move inside self.  Move canceled");
				return;
			}
			...
			if (!l.valid()) {
				log("In line: " + workingCommand
						+ " WARNING: Target locaiton not valid " + eq
						+ " Move canceled");
				return;
			}
			int moveType = loc.getMoveType(l);
			if (moveType == Location.LOAD || moveType == Location.UNLOAD) {
				...
				if (hasTech("T8") && (loc.isOrbit() || l.isOrbit()))
					techlevel = 1;
				if (hasTech("T1") && !loc.isOrbit())
					techlevel = 1;
				if (hasTech("T2") && !loc.isOrbit())
					techlevel = 1;
				if (hasTech("T3") && !loc.isOrbit())
					techlevel = 1;
				if (hasTech("T4") && !loc.isOrbit())
					techlevel = 1;
				if (hasTech("T7") && !loc.isOrbit())
					techlevel = 1;
				if (techlevel < 1) {
					if (loc.isOrbit())
						log("In line: "
								+ workingCommand
								+ " WARNING: Cannot move no EVAJets technology available.  Move canceled");
					else
						log("In line: "
								+ workingCommand
								+ " WARNING: Cannot move no surface movement technology avaialble .  Move canceled");
					return;
				}
				if (loc.isInside()) {
					NFObject nfo = Universe
					...
					if (nfo == null) {
						log("SYSTEM ERROR:In line: "
								+ workingCommand
								+ " WARNING: Bad move cannot find source inside location");
						System.out
								.println("SYSTEM ERROR: line: "
										+ workingCommand
										+ " WARNING: Bad move cannot find source inside location");
						return;
					}
					if (nfo instanceof Active) {
						...
						if (act.hasTech("T57")
								&& !nfo.getCorpTick()
										.equalsIgnoreCase(corpTick)
								&& !nfo2.getLocation().equalsIgnoreCase(loc)) {
							log("In line: " + workingCommand
									+ " WARNING: Exit blocked by security.");
							return;
						}
				...
				if (l.isInside()) {
					...
					if (nfo == null) {
						log("In line: "
								+ workingCommand
								+ " WARNING: Bad move cannot find target inside location");
						System.out
								.println("MyError line: "
										+ workingCommand
										+ " WARNING: Bad move cannot find source inside location");
						return;
					}
					if (nfo instanceof Active) {
						...
						if (act.hasTech("T57")
								&& !nfo.getCorpTick()
										.equalsIgnoreCase(corpTick)) {
							log("In line: " + workingCommand
									+ " WARNING: Entry blocked by security.");
							return;
						}
						if (act == this) {
							log("In line: "
									+ workingCommand
									+ " WARNING: Bad move cannot move inside self");
							return;
						}
						if (act.getCargoCapacity() < getMass()
								&& !act.contains(this)) {
							log("In line: " + workingCommand
									+ " WARNING: No room to enter active " + eq
									+ " Move canceled");
							return;
						}
					} else if (nfo instanceof Facility) {
						...
						if (fac.getCapacity() < getMass()
								&& !fac.contains(this)) {
							if (Main.DEBUG)
								System.out
										.println("MYDEBUG: entrance blocked.  Facility full with "
												+ fac.getCapacity()
												+ " room and "
												+ getMass()
												+ " added");
							log("In line: " + workingCommand
									+ " WARNING: No room to enter facility "
									+ eq + " Move canceled");
				...
				if (b != null)
					atm = b.hasAtmosphere();
				if (hasTech("T1"))
					techlevel = Math.max(techlevel, 1);
				if (hasTech("T2"))
					techlevel = Math.max(techlevel, 2);
				if (hasTech("T5") && atm)
					techlevel = Math.max(techlevel, 3);
				if (hasTech("T3"))
					techlevel = Math.max(techlevel, 3);
				if (hasTech("T4"))
					techlevel = Math.max(techlevel, 4);
				if (hasTech("T6") && atm)
				...
				if (techlevel < 1) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move no Track technology available");
					return;
				}
				if (sur.equals("b") && techlevel < 2
						&& !Universe.locationContainsTechnology(l, "T58")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move no Track or HoverFit technology available");
					return;
				}
				if (sur.equals("B") && techlevel < 3
						&& !Universe.locationContainsTechnology(l, "T58")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move no Hexapod technology available");
					return;
				}
				if (sur.equals("m") && techlevel < 4
						&& !Universe.locationContainsTechnology(l, "T58")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move no Bipod technology available");
					return;
				}
				if (sur.equals("M") && techlevel < 5
						&& !Universe.locationContainsTechnology(l, "T58")) {
					if (atm)
						log("In line: "
								+ workingCommand
								+ " WARNING: Cannot move no PropulsionJets or Wings technology available");
					else
						log("In line: "
								+ workingCommand
								+ " WARNING: Cannot move no PropulsionJets technology available");
					return;
				}
				setLocation(l);
			} else if (moveType == Location.SUBTER) {
				if (!hasTech("T9")
						&& !Universe.locationContainsTechnology(l, "T59")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move subterranin no BoringDrill technolgy available");
					return;
				}
				setLocation(l);
				...
				if (techlevel < 1) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move from orbit no Reentry technolgy available");
					return;
				}
				if (techlevel < 2) {
					if (l.isFacility()) {
						...
						if (nfo instanceof Facility) {
							...
							if (!ff.hasTech("T77")) {
								// no log necessary robot will be destroyed
								// anyway
								ff.destroy();
							}
						}
				...
				if (l.isFacility() && Universe.locationHasTechnology(l, "T57")
						&& faci != null && faci instanceof Facility
						&& !faci.getCorpTick().equals(corpTick)) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot land.  Entry blocked by security.  Landing nearby.");
					l = l.getOutsideFacilityLocation();
				}
				if (l.isFacility() && !Universe.locationHasTechnology(l, "T77")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot land.  Facility doesn't have LandingZone technology. Landing nearby.");
					l = l.getOutsideFacilityLocation();
				}
				if (faci != null && faci instanceof Facility
						&& ((Facility) faci).getCapacity() < getMass()) {
					// System.out.println("MYDEBUG: reentry entrance blocked.
					// Facility full");
					log("In line: " + workingCommand
							+ " WARNING: No room to enter facility " + eq
							+ ".  Landing nearby.");
					l = l.getOutsideFacilityLocation();
				}
				setLocation(l);
				if (!l.isFacility()) {
					...
					if (su.equalsIgnoreCase("W") && Math.random() > .95)
						landingDamage();
					if (su.equals("P") && Math.random() > .90)
						landingDamage();
					if (su.equals("p") && Math.random() > .85)
						landingDamage();
					if (su.equals("b") && Math.random() > .75)
						landingDamage();
					if (su.equals("B") && Math.random() > .5)
						landingDamage();
					if (su.equals("m") && Math.random() > .3)
						landingDamage();
					if (su.equals("M") && Math.random() > .1)
						landingDamage();
				}
				...
				if (!hasTech("T12") && !hasTech("T13") && !hasTech("T14")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot escape.  No Rocket technolgy available");
					return;
				}
				if (loc.isFacility()
						&& !Universe.locationHasTechnology(loc, "T77")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot launch.  Facility doesn't have LandingZone technology.");
					return;
				}
				NFObject faci = Universe.getNFObjectById(l.getInsideWhat());
				if (loc.isFacility()
						&& !Universe.locationHasTechnology(loc, "T57")
						&& faci != null
						&& !faci.getCorpTick().equalsIgnoreCase(corpTick)) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot launch.  Exit blocked by security.");
					return;
				}
				setLocation(l);
				...
				if (!hasTech("T12") && !hasTech("T13") && !hasTech("T14")
					...
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move intraplanetary no Warp technolgy available");
					return;
				}
				setLocation(l);
				...
				if (!hasTech("T87")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot move intrasolar no Warp technolgy available");
					return;
				}
				setLocation(l);
			}
		...
		// LOAD
			...
			if (n.length > 3 || n.length < 1) {
				log("In line: " + workingCommand
						+ " WARNING:  bad format for load command");
				return;
			}
			if (!hasTech("T35") && !hasTech("T36") && !hasTech("T88")) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot load no Arm technolgy available");
				return;
			}
			Location loadloc = loc;
			if (n.length > 1) {
				...
				if (loadloc != null) {
					...
					if (moveType == Location.INTERPLANET
						...
						log("In line: "
								+ workingCommand
								+ " WARNING: Cannot remote load between orbits or surface to orbit. "
								+ n[0]);
						return;
					}
					// SECURITY HERE
					if (loadloc.isInside()) {
						...
						if (nfoo instanceof Active) {
							...
							if (aaa.hasTech("T57")
								...
								log("In line: "
										+ workingCommand
										+ " WARNING: Cannot remote load.  Blocked by secuirty at "
										+ aaa.getId());
								return;
							}
						}
			...
			int cc = getCargoCapacity();
			if (cc <= 0) {
				log("In line: " + workingCommand + " WARNING: unable to load "
						+ n[0] + ".  Cargo bay full.");
				return;
			...
			if (nfo != null) {
				if (!nfo.getLocation().equals(loadloc)) {
					log("In line: " + workingCommand
							+ " WARNING: unable to load " + n[0]
							+ " not at location");
					return;
				}
				if (nfo == this) {
					log("In line: " + workingCommand
							+ " WARNING: cannot load self");
					return;
				}
				if (nfo instanceof Facility) {
					log("In line: " + workingCommand
							+ " WARNING: cannot load facilities");
					return;
				}
				// TODO if stockpile then load partial stockpile
				if (nfo.getMass() > cc) {
					log("In line: " + workingCommand
							+ " WARNING:  Insufficent cargospace to load "
							+ n[0]);
					return;
				}
			}
			if (nfo == null) {
				...
				try {
					...
					try {
						...
						if (num > cc) {
							log("In line: " + workingCommand
									+ " WARNING: unable to load " + num + " "
									+ mat + " not enough room will load " + cc);
							num = cc;
						}
					} catch (NumberFormatException nfe) {
						log("In line: " + workingCommand
								+ " WARNING: bad number in command" + n[1]
								+ ".");
						return;
					}
					...
					nfo = new StockPile(corpTick, mat, num, Location
							.parse("INSIDE." + getId()));
					Universe.takeMaterial(corpTick, loadloc, mat, num);
					Universe.add(nfo);
				} catch (NumberFormatException NFE) {
					log("In line: " + workingCommand
							+ " WARNING: bad number in load " + n[1]);
				}
			}
		...
		// UNLOAD
			...
			if (n.length != 2 && n.length != 1) {
				log("In line: " + workingCommand
						+ " WARNING:  bad format for Unload command");
				return;
			}
			if (!hasTech("T35") && !hasTech("T36") && !hasTech("T88")) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot unload no Arm technolgy available");
				return;
			}
			if (nfo != null) {
				if (!nfo.getLocation().equals(
						Location.parse("INSIDE." + getId()))) {
					log("In line: " + workingCommand
							+ " WARNING: unable to unload " + n[0]
							+ " not carried");
					return;
				}
				nfo.setLocation(loc);
			...
			if (nfo == null) {
				...
				try {
					...
					int ava = Universe.availableMaterial(Location
							.parse("INSIDE." + getId()), mat);
					if (num == -1)
						num = ava;
					if (ava == 0) {
						log("In line: " + workingCommand
								+ " WARNING: cargo not carried to unload");
						return;
					}
					if (ava < num) {
						log("In line: " + workingCommand
								+ " WARNING: unable to unload " + num + " "
								+ mat + " will unload " + ava);
						num = ava;
					}
					nfo = new StockPile(corpTick, mat, num, loc);
					Universe.takeMaterial(corpTick, Location.parse("INSIDE."
							+ getId()), mat, num);
					Universe.add(nfo);
				} catch (NumberFormatException NFE) {
					log("In line: " + workingCommand
							+ " WARNING: bad number in unload " + n[0]);
				}
			}
		}
		// COLLECT <eq> <eq>
			...
			if (n.length != 2) {
				log("In line: " + workingCommand
						+ " WARNING:  bad format for Collect command");
				return;
			}
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot collect no HeavyWorkArm technolgy available");
				return;
			}
			...
			if (matden < 6 && !Universe.locationHasTechnology(loc, "T44")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot collect no Collector technolgy available");
				return;
			}
			if (matden == 6 && !Universe.locationHasTechnology(loc, "T52")) {
				...
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot collect no Condenser technolgy available");
				return;
			}
			if (matden > 6 && !Universe.locationHasTechnology(loc, "T51")) {
				...
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot collect no Distilation technolgy available");
				return;
			}
			if (matden > 0) {
				try {
					...
					log("In line: " + workingCommand
							+ " WARNING: bad number in Collect command " + n[1]);
				}
			} else {
				log("In line: " + workingCommand
						+ " WARNING: Unable to collect " + n[0]
						+ " at location " + loc + " no resources");
			}
		}
		// REFINE <eq> <eq>
			...
			if (n.length != 2) {
				log("In line: " + workingCommand
						+ " WARNING: bad num args in refine command");
				return;
			}
			int amt = 1;
			try {
			...
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot refine no HeavyWorkArm technolgy available");
				return;
			}
			if (!Universe.locationHasTechnology(loc, "T45")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot refine no Refinery technolgy available");
				return;
			}
			...
			if (mi == null) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot find material " + ma);
				return;
			}
			ma = mi.getName();
			String reqmat = mi.getRefineMaterial();
			if (reqmat == null) {
				log("In line: " + workingCommand + " WARNING: Cannot refine "
						+ ma + " is native or must be produced");
				return;
			}
			// check for material availability
			int ava = Universe.availableMaterial(Location.parse("INSIDE."
			...
			if (ava == 0) {
				log("In line: " + workingCommand
						+ " WARNING: unable to refine " + ma + " no " + reqmat
						+ " available");
				return;
			}
			if (ava < amt) {
				log("In line: " + workingCommand
						+ " WARNING: unable to refine " + amt + " of " + ma
						+ " not enought " + reqmat + ".  Will use " + ava
						+ " instead");
				amt = ava;
			}
			// consume required materals
			int collected = Universe.takeMaterial(corpTick, Location
					.parse("INSIDE." + getId()), reqmat, amt);
		...
		// SALVAGE <eq>
			...
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot salvage no HeavyWorkArm technolgy available");
				return;
			}
			if (!Universe.locationHasTechnology(loc, "T48") && !hasTech("T48")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot salvage no Salvage technolgy available");
				return;
			}
			int amt = ava;
			if (ava == 0) {
			...
			if (n.length == 2) {
				try {
					...
					if (reqamt > ava) {
						log("  WARNING: Not enough Junk available will Salvage "
								+ amt + " instead");
					}
					amt = Math.max(0, Math.min(amt, reqamt));
				} catch (NumberFormatException nfe) {
					log("In line: " + workingCommand
							+ " WARNING: Bad number in salvage " + n[1]);
					return;
				}
			}
			if (Main.DEBUG)
				System.out.println("MYDEBUG: Salvage requested for " + amt);

			int collected = Universe.takeMaterial(corpTick, Location
					.parse("INSIDE." + getId()), "JUNK", amt);
			...
			NFObject nfo2 = new StockPile(corpTick, "IndustrialWaste", iwamt,
					loc);
			Universe.add(nfo1);
			Universe.add(nfo2);
		}
		// RECOVEr <eq>
			...
			if (nfo == null || !(nfo instanceof Facility)) {
				log("In line: " + workingCommand
						+ " WARNING: cannot find Facility " + n[0]);
				return;
			}
			...
			if (!nfo.getLocation().equals(loc)) {
				log("In line: "
						+ workingCommand
						+ " WARNING: cannot recover facility.  Not at location.");
				return;
			}
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot recover no HeavyWorkArm technolgy available");
				return;
			}
			Facility f = (Facility) nfo;
			if (!f.canRecover()) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot recover facility too damaged.");
				return;
			}
		...
		// CONSTRUCT <eq>
			...
			if (nfo == null || !(nfo instanceof Prefab)) {
				log("In line: " + workingCommand
						+ " WARNING: cannot find prefab " + n[0]);
				return;
			}
			// check if prefab at loc
			if (!nfo.getLocation().equals(loc)) {
				log("In line: "
						+ workingCommand
						+ " WARNING: cannot construct facility.  No prefab available.");
				return;
			}
			// check location is inside
			if (loc.isInside() || loc.isFacility()) {
				log("In line: " + workingCommand
						+ " WARNING: cannot construct facility inside");
				return;
			}
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot construct no HeavyWorkArm technolgy available");
				return;
			}
			Prefab pf = (Prefab) nfo;
			if (!pf.canConstructAt(loc)) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot construct facility.  Technology cannot operate at locaiton.");
				return;
			}
			int fcount = Universe.facilityCount(loc);
			if (fcount > 3) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot construct facility.  Locaiton at maximum capacity.");
				return;
			}
			...
			if (!hasTech("T36")) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Cannot produce no HeavyWorkArm technolgy available");
				return;
			}
			...
			if (itt != null) {
				if (n.length > 1) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Bad use of design production.  Cannot produce more than one.");
					return;
				}
				...
				if (!Universe.locationHasTechnology(loc, "T46")) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Cannot produce no Production technolgy available");
					return;
				}
				if (itt instanceof ActiveDesign) {
					ActiveDesign att = (ActiveDesign) itt;
					if (availableMaterialList(Location.parse("INSIDE."
							+ getId()), loc, att.getAllProductionCosts()) < 1) {
						log("In line: " + workingCommand
								+ " WARNING: Cannot produce " + n[0]
								+ " cannot find needed materials");
						return;
					}
					...
					if (!itt.isPartner(corpTick) && !itt.isPublic()) {
						...
						if (c != null) {
							if (c.canAfford(leasePrice)) {
								...
								c.report += "  " + getId()
										+ " Paying lease on " + n[0] + " of "
										+ Stuff.money(leasePrice, 2) + "\n";
								itt.payLease();
							} else {
					...
					consumeMaterialList(Location.parse("INSIDE." + getId()),
							loc, att.getAllProductionCosts(), 1);
					Active AA = new Active(corpTick, att.getId(), loc, -1, "",
							"", new Hashtable());
					Universe.add(AA);
				} else if (itt instanceof FacilityDesign) {
					FacilityDesign att = (FacilityDesign) itt;
				...
				if (Main.DEBUG)
					System.out.println("MYDEBUG: produce material found "
							+ mi.getName());
				if (n.length != 2) {
					log("In line: "
							+ workingCommand
							+ " WARNING: Bad use of material production.  Must specify number to produce.");
					return;
				}
				int reqamt = 0;
				try {
				...
				if (Main.DEBUG)
					System.out.println("MYDEBUG: amount requested =" + reqamt);
				if (mi.getId().equals("M1")) {
					if (Main.DEBUG)
						System.out
								.println("MYDEBUG: recycle or incenerate requested");
					if (!Universe.locationHasTechnology(loc, "T49")
							&& !Universe.locationHasTechnology(loc, "T50")) {
					...
					int amt2 = 0;
					int industwast = availableMaterialList(Location
							.parse("INSIDE." + getId()), loc, "M70");
					int toxwast = availableMaterialList(Location
							.parse("INSIDE." + getId()), loc, "M68");
					int humanwast = availableMaterialList(Location
							.parse("INSIDE." + getId()), loc, "M69");
					int otherwast = toxwast + humanwast;
					if (Main.DEBUG)
						System.out
								.println("MYDEBUG: recycle max =" + industwast
										+ "  incenerate max =" + otherwast);
					// indwastamt
					if (!Universe.locationHasTechnology(loc, "T49")) {
						amt2 += industwast;
					}
					...
					if (amt2 == 0) {
						log("In line: "
								+ workingCommand
								+ " WARNING: No material available for recycle or incenerate.");
						return;
					}
					if (reqamt > amt2) {
						log("In line: "
								+ workingCommand
								+ " WARNING: Insufficent material.  Will produce "
								+ amt2 + " instead");
						reqamt = amt2;
					}
					// consumeMaterialList
					...
					if (!Universe.locationHasTechnology(loc, "T49")
						...
						consumeMaterialList(
								Location.parse("INSIDE." + getId()), loc,
								"M70", get);
						collect += get;
						if (Main.DEBUG)
							System.out.println("MYDEBUG: recycling  =" + get);
					}
					if (!Universe.locationHasTechnology(loc, "T50")
							&& otherwast > 0) {
						...
						if (Main.DEBUG)
							System.out
									.println("MYDEBUG: incenerate 1  =" + get);
						...
						if (Main.DEBUG)
							System.out
									.println("MYDEBUG: incenerate 2  =" + get);
					}
					// make stokepiles
					NFObject nfo = new StockPile(corpTick, mi.getName(),
							reqamt, loc);
					Universe.add(nfo);
				} else if (mi.isProduced()) {
					if (!Universe.locationHasTechnology(loc, "T46")) {
					...
					int unitsize = mi.getUnitSize();
					reqamt = reqamt
							+ (reqamt % unitsize != 0 ? unitsize - reqamt
									% unitsize : 0);
					int maxamount = availableMaterialList(Location
							.parse("INSIDE." + getId()), loc, mi
							.getProducedMaterial());
					if (maxamount == 0) {
						log("In line: " + workingCommand
								+ " WARNING: No materials available.");
						return;
					}
					if (reqamt > maxamount * unitsize) {
						log("In line: "
								+ workingCommand
								+ " WARNING: Insufficent material.  Will produce "
								+ maxamount * unitsize + " instead");
						reqamt = maxamount * unitsize;
					}
					consumeMaterialList(Location.parse("INSIDE." + getId()),
							loc, mi.getProducedMaterial(), reqamt / unitsize);
					NFObject nfo = new StockPile(corpTick, mi.getName(),
							reqamt, loc);
					Universe.add(nfo);
				} else {
					log("WARNING: Materila cannot be produced.  Must be Refined or Collected. "
							+ n[0] + ".");
					return;
				}
			} else {
		...
		// REPAIR <eq> [<eq>]
			...
			if (n.length < 1) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair bad number of args");
				return;
			}
			NFObject nfo = Universe.getNFObjectById(n[0]);
			// find NFOID
			if (nfo == null) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair cannot find " + n[0]);
				return;
			}
			if (nfo == this) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair self");
				return;
			}
			if (!nfo.getLocation().equalsIgnoreCase(loc)) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair not at same location.");
				return;
			}
			if (nfo instanceof StockPile || nfo instanceof Prefab) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair " + n[0]
						+ " is stockpile not robot, ship or facility");
				return;
			}
			if (nfo instanceof Active) {
				Active aa = (Active) nfo;
			...
			if (n.length == 2) {
				try {
					amt = Math.max(1, Math.min(amt, (int) Stuff
							.parseDouble(n[1])));
				} catch (NumberFormatException nfe) {
					log("In line: " + workingCommand
							+ " WARNING: Bad number in repair " + n[1]);
					return;
				}
			}
			if (!Universe.locationHasTechnology(loc, "T47") && !hasTech("T47")) {
				log("In line: " + workingCommand
						+ " WARNING: No Repair technology.");
				return;
			}
			int ava = Universe.availableMaterial(Location.parse("INSIDE."
					+ getId()), "SpareParts");
			...
			if (ava == 0) {
				log("In line: " + workingCommand
						+ " WARNING: unable to repair no SpareParts available");
				return;
			}
			if (ava < amt) {
				if (n.length == 2) {
					log("In line: " + workingCommand
							+ " WARNING: unable to repair " + amt
							+ " not enough SpareParts.  Will use " + ava
							+ " instead");
				}
				amt = ava;
			}
			// consume required materals
			int collected = Universe.takeMaterial(corpTick, Location
					.parse("INSIDE." + getId()), "SpareParts", amt);
			Universe.takeMaterial(corpTick, loc, "SpareParts", amt - collected);
			if (Main.DEBUG)
				System.out.println("DEBUG: do repair test" + amt);
			if (nfo instanceof Active) {
				Active aa = (Active) nfo;
				aa.doRepair(amt);
				if (Main.DEBUG)
					System.out.println("DEBUG: repair done test"
							+ aa.getRepairMass());
			}
			if (nfo instanceof Facility) {
				Facility aa = (Facility) nfo;
		...
		// REFUEL <id> <amt> : fill other amt
			...
			if (targ.isRobot()) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot refuel robot, must Recharge");
				return;
			}

			amt = Math.min(amt, (int) (activeDesign.getDesignEndurance() - targ
					.getEndurance()) * 2);
			int ava = Universe.availableMaterial(Location.parse("INSIDE."
					+ getId()), "RocketFuel");
			...
			if (ava == 0) {
				log("In line: " + workingCommand
						+ " WARNING: unable to refuel no RocketFuel found.");
			} else if (amt > ava) {
				log("In line: " + workingCommand
						+ " WARNING: unable to refuel " + amt
						+ " not enough RocketFuel.  Will use " + ava
						+ " instead.");
			}
			amt = Math.min(ava, amt);
			// collect fuel needed
			int collected = Universe.takeMaterial(corpTick, Location
					.parse("INSIDE." + getId()), "RocketFuel", amt);
		...
		// RECHARGE [<eq>]
			...
			if (n.length == 0) {
				if (isShip()) {
					log("In line: " + workingCommand
							+ " WARNING: Cannot recharge ship, must Refuel");
					return;
				}
				if (loc.isInside()) {
					NFObject nfo = Universe
					...
					if (nfo == null) {
						log("In line: " + workingCommand
								+ " WARNING: Cannot find INSIDE for recharge");
						return;
					}
					if (nfo == this) {
						log("In line: " + workingCommand
								+ " WARNING: Cannot recharge self");
						return;
					}
					if (nfo instanceof Active) {
						Active a = (Active) nfo;
						if (!a.isPowered() && a.isShip()) {
							log("In line: "
									+ workingCommand
									+ " WARNING: Cannot recharge.  Ship out of power.");
							return;
						} else if (a.isRobot()) {
							log("In line: "
									+ workingCommand
									+ " WARNING: Cannot recharge inside robot only ships.");
							return;
						}
						recharge();
						return;
					...
					if (nfo instanceof Facility) {
						Facility f = (Facility) nfo;
						if (f.hasTech("T64") || f.hasTech("T65")
								|| f.hasTech("T66") || f.hasTech("T67")
								|| f.hasTech("T68") || f.hasTech("T85")) {
							recharge();
							return;
						} else {
							log("In line: "
									+ workingCommand
									+ " WARNING: Cannot recharge.  NO power inside facility.");
							return;
						}
					}
					log("In line: " + workingCommand
							+ " WARNING: Unknown error in recharge.");
					return;
				} else {
					log("In line: "
							+ workingCommand
							+ " WARNING: Not inside ship or facilty.  Cannot recharge");
					return;
				}
			}
			if (n.length == 1) {
				NFObject nfo = Universe.getNFObjectById(n[0]);
				if (nfo == null) {
					log("In line: " + workingCommand + " WARNING: Cannot find "
							+ n[0] + " for recharge");
					return;
				}
				if (nfo instanceof Active) {
		...
		// ATTACK <eq>
			...
			if (n.length != 1) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Bad use of attack.  Must specify target robot, ship or facility.");
				return;
			}
			NFObject nfo = Universe.getNFObjectById(n[0]);
			if (nfo == null) {
				log("In line: " + workingCommand
						+ " WARNING: Bad use of attack.  Cannot find target "
						+ n[0]);
				return;
			}
			if (nfo == this) {
				log("In line: " + workingCommand
						+ " WARNING: Bad use of attack.  Cannot attack self");
				return;
			}
			if (nfo instanceof StockPile || nfo instanceof Prefab) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Bad use of attack.  target not robot, ship of facility. "
						+ n[0]);
				return;
			}
			if (!nfo.getLocation().equals(getLocation())) {
				log("In line: " + workingCommand
						+ " WARNING: Cannot attack not at same location. "
						+ n[0]);
				return;
			}
			if (nfo instanceof Active) {
		...
		// SEND <eq> [<corp>] message
			...
			if (n.length < 2) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Bad use of send.  Must specify robot or ship to receive and message.");
				return;
			}
			NFObject nfo = Universe.getNFObjectById(n[0]);
			if (nfo == null) {
				log("In line: " + workingCommand
						+ " WARNING: Bad use of send.  Cannot find " + n[0]
						+ " to send message to.");
				return;
			}
			if (!(nfo instanceof Active)) {
				log("In line: "
						+ workingCommand
						+ " WARNING: Bad use of send.  Message can only be sent to robot or ship.");
				return;
			}
			Active act = (Active) nfo;
			if (n[1].length() == 0)
	...
	public void advanceClockTo(long t) {
		...
		if (finishTime == 0) {
			...
			loadNextCommand();
			if (Main.DEBUG)
				System.out.println("init program found");
		}
	...
	public static Active parse(GmlPair g) {
		...
		for (int a = 0; a < n.length; a++) {
			...
			if (temp.indexOf(":") > 0) {
				...
				System.out
						.println("MyError: bad var read " + temp + " in " + g);
			}
		}
	...
	public void destroy() {
		...
		for (int a = 0; a < cc.length; a++) {
			...
			if (nfo != null)
				...
				System.out.println("MyError: bad nfo in cargo during destory "
						+ cc[a]);
			}
		}
		// create junk stockpile
		NFObject nfo = new StockPile(corpTick, "Junk", activeDesign
				.getDesignMass(), loc);
		Universe.add(nfo);
		// remvoe self
	...
	public void consumeMaterialList(Location l1, Location l2, String prodList,
			int mult) {
	...
	public void consumeMaterialList(Location l1, Location l2, Vector prodList,
			int mult) {
		for (int a = 0; a < prodList.size(); a++) {
			...
			if (prodReq.indexOf("x") > 0) {
				try {
					unitcost = Integer.parseInt(prodReq.substring(0, prodReq
							.indexOf("x")));
				} catch (NumberFormatException nfe) {
				}
			...
			int collected = 0;
			if (l1 != null)
				collected = Universe.takeMaterial(corpTick, l1, prodReq,
						unitcost * mult);
			if (l2 != null)
				Universe.takeMaterial(corpTick, l2, prodReq, unitcost * mult
						- collected);
		}
	}
	...
	public int availableMaterialList(Location l1, Location l2, Vector prodList) {
		...
		for (int a = 0; a < prodList.size(); a++) {
			...
			int ava = 0;
			if (l1 != null)
				ava = Universe.availableMaterial(l1, prodReq);
			if (l2 != null)
				ava += Universe.availableMaterial(l2, prodReq);
