...
public class Main {
	...
	public Main() {
		if (DEBUG)
			System.out.println("Init Main object");
		try {
			System.out.println("parse " + Main.DIRDATA
					+ Main.universeDataFileName);
			GmlPair g = GmlPair.parse(new File(Main.DIRDATA
					+ Main.universeDataFileName));
			// GmlPair g = GmlPair.parse(new
			// File(Main.DIRDATA+emailConfigFileName));
	...
	public void executeTurn() {
		if (DEBUG)
			System.out.println("executing turn " + new Date());
		Universe.load();
	...
	public void processOrders() {
		...
		if (tt.length > 0)
			type = tt[0].toUpperCase();// type
		if (tt.length > 1)
			cname = tt[1];// corp name
		if (tt.length > 2)
			cpass = tt[2];// pass
		System.out.println("type =" + type + " cname=" + cname + " pass="
				+ cpass);
	...
	public String blindMail(Corp c, String all[]) {
		...
		for (int a = 0; a < all.length; a++) {
			if (!all[a].toUpperCase().trim().startsWith("TO:")
					&& !all[a].toUpperCase().trim().startsWith("SUBJECT:")) {
				mail.addElement(all[a]);
			}
		}
		sendBlind(mail, tar.email, tar.getName() + " (" + tar.getTick() + ")",
				c.getName() + " (" + c.getTick() + ")", subject);
			...
			while (true) {
				...
				if (!started && s.trim().toUpperCase().startsWith("FROM:")) {
					...
					if (replyTo.indexOf("<") >= 0
							&& replyTo.indexOf(">") > replyTo.indexOf("<")) {
						replyTo = replyTo.substring(replyTo.indexOf("<") + 1,
								replyTo.indexOf(">"));
					}
					if (replyTo != null
							&& (replyTo.indexOf("@") < 0 || replyTo
									.lastIndexOf(".") <= replyTo.indexOf("@"))) {
						System.out.println("MyError: Bad from email found:"
								+ replyTo);
						replyTo = null;
					}
					if (replyTo != null
							&& replyTo.toUpperCase().indexOf(
									SYSTEMEMAIL.toUpperCase()) >= 0) {
				...
				if (started && !stopped) {
					v.addElement(s);
						...
						&& s.toUpperCase().trim().startsWith("@LOGIN")
						&& !stopped) {
					login = s.substring(s.toUpperCase().indexOf("@LOGIN") + 6)
							.trim();
					started = true;
				}
			}
	...
	public String validOrder(Corp c, String s) {
		...
		if (s.startsWith("#") || s.startsWith(";") || s.length() == 0) {
			...
			return dis;
		} else if (s.toUpperCase().equals("NOOP")
				|| s.toUpperCase().equals("QUIT")
				|| s.toUpperCase().startsWith("RESET ")
				|| s.toUpperCase().startsWith("INIT ")
				|| s.toUpperCase().startsWith("SETLOG ")
				|| s.toUpperCase().startsWith("SETTECH ")
				|| s.toUpperCase().startsWith("UPLOAD ")
				|| s.toUpperCase().startsWith("BUY ")
				|| s.toUpperCase().startsWith("SELL ")
				|| s.toUpperCase().startsWith("CONSIGN ")
				|| s.toUpperCase().startsWith("BID ")
				|| s.toUpperCase().startsWith("DEFINE ")
				|| s.toUpperCase().startsWith("LEASE ")
				|| s.toUpperCase().startsWith("PARTNER ")
				|| s.toUpperCase().startsWith("GIVE ")
				|| s.toUpperCase().startsWith("PRODUCE ")
				|| s.toUpperCase().startsWith("SETMAIL ")
				|| s.toUpperCase().startsWith("PURCHASE ")
				|| s.toUpperCase().startsWith("RENAME ")
				|| s.toUpperCase().startsWith("SETPASSWORD ")
				|| s.toUpperCase().startsWith("RESEARCH ")
				|| s.toUpperCase().startsWith("PAY ")
				|| s.toUpperCase().startsWith("NICK ")
				|| s.toUpperCase().startsWith("DESIGN ")
				|| s.toUpperCase().startsWith("RELEASE")) {
			return dis;
		} else {
	...
	public static void sendBlind(Vector v, String toEmail, String toCorp,
			String fromCorp, String subject) {
		if (toEmail == null) {
			System.out
					.println("MyError: Unable to send message no reply found");
		...
		if (DEBUG)
			System.out.println("Sending via blind mailer to " + toEmail);
	...
	public static void send(Vector v, String replyTo, String subject) {
		...
		if (DEBUG)
			System.out.println("Sending orders mailer to " + replyTo);
		StringBuffer all = new StringBuffer();
		all.append("Sender: " + NONGAMEEMAIL + "\n");
		all.append("Reply-To: " + SYSTEMEMAIL + "\n");
		all.append("From: \"New Frontiers Mail\"<" + SYSTEMEMAIL + ">\n");
	...
	public static void sendFormatedMail(String all) {
		...
		if (DEBUG)
			System.out.println("piping mail through " + tar);
		...
		try {
			...
			PrintWriter o = new PrintWriter(new OutputStreamWriter(p
					.getOutputStream()));
			if (DEBUG)
	...
	public static void logTiming(String s) {
		if (ttime == -1) {
			...
			if (Main.TIMING)
				System.out.println("TIMING: begin "
						+ (System.currentTimeMillis() - tttime) + " at "
						+ new Date());
			return;
		}
		if (Main.TIMING)
			System.out.println("TIMING: "
					+ (System.currentTimeMillis() - ttime) + " " + s);
		if (Main.TIMING && s == null)
