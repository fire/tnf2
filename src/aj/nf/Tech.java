...
public class Tech implements ITThing {
	...
	public Tech(String n, Vector p, Vector pc, double lp, int sid, String ty,
			Vector pr) {
		if (sid == -1) {
			sid = count;
			count++;
	...
	public String display(String tic) {
		...
		if (!res && !pub && !part) {
			...
			for (int a = 0; a < prerequisits.size(); a++) {
				ITThing t = Universe.getITThingByName((String) prerequisits
						.elementAt(a));
				if (a != 0)
					s += ", ";
				if (t == null) {
	...
	public GmlPair toGmlPair() {
		...
		for (int a = 0; a < prerequisits.size(); a++) {
			...
			if (itt != null)
				...
				System.out.println("MyError: bad prerequisite in save in "
						+ name + " of " + t);
			}
			pres += t;
		}
		...
		for (int a = 0; a < productionCosts.size(); a++) {
			...
			if (Universe.getMarketValue(t) == 0) {
				System.out.println("MyError: bad productioncost in " + name
						+ " of " + t);
			}
		...
		for (int a = 0; a < n.length; a++) {
			...
			for (int b = 0; b < tt.length; b++) {
				...
				if (Universe.getMarketValue(lookup) == 0) {
					System.out.println("MyError: in " + na + " " + lookup
							+ " unknown market item");
				}
				pc.addElement(tt[b]);
			}
