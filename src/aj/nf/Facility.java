...
public class Facility implements NFObject {
	...
	public String getResearchReport() {
		return "" + getId() + ":" + facilityDesign.getName()
				+ " Contract available for " + Stuff.money(newContractPrice, 2);
	}
			...
			for (int a = 0; a < give.size(); a++) {
				...
				if (what.indexOf("x") > 0) {
					try {
						...
						System.out
								.println("MyError: Bad number in set active, material produce");
					}
				}
				...
				if (what.startsWith("M")) {
					...
					if (na != null) {
						// create stockpiple
						NFObject nfo = new StockPile(getCorpTick(), na, amt,
								getInsideLoc());
						Universe.add(nfo);
	...
	public boolean hasAbility(String s) {
		if (facilityDesign != null)
			return facilityDesign.hasTech(s);
		else
			return false;
	}
	...
	public int getCapacity() {
		...
		for (int a = 0; a < v.size(); a++) {
			NFObject nfo = (NFObject) v.elementAt(a);
			// System.out.println("Facility "+getId()+" contains "+nfo.getId()+"
			// of "+nfo.getMass()+" mass");
			total += nfo.getMass();
		}
		int max = 200;
	...
	public void setCorpTick(String s) {
		if (hasAbility("T84"))
			return;// cannot auction or give corporate HQ
		corpTick = s;
	}
	...
	public String display() {
		String s = (nick.length() > 0 ? nick + ":" : "") + getId() + ": "
				+ facilityDesign.getName() + " at " + loc.toString() + " "
				+ damage + "% damaged";
		if (isDisabled()) {
			s += " DISABLED";
		} else if (!isActive()) {
	...
	public static Facility parse(GmlPair g) {
		...
		if (n.length > 0)
			f.damage = Math.max(0, (int) (n[0].getDouble()));
		GmlPair NN = g.getOneByName("Nick");
		if (NN != null)
			f.nick = NN.getString();
		NN = g.getOneByName("ContractPrice");
		if (NN != null)
			f.newContractPrice = f.contractPrice = NN.getDouble();
		NN = g.getOneByName("Disabled");
		if (NN != null)
			f.setDisabled(NN.getDouble() == 1);
		NN = g.getOneByName("Active");
		if (NN != null)
			f.active = (NN.getDouble() == 1);
		// power on time
		return f;
	}
	...
	public int getProvidesAmt(String s) {
		...
		if (what.indexOf("x") > 0) {
			try {
				...
				System.out.println("MyError: bad number in provides list"
						+ what);
			}
		}
		String what2 = null;
