...
public class Location {
	...
	public static int SURFACE = 0, SUBTER = 1, ESCAPE = 2, INTERPLANET = 3,
			INTERSOLAR = 4, LOAD = 5, REENTRY = 6, UNLOAD = 7;

	public Location(String all, String t, int so, int p, String m, int se,
			int r, int c, int l, String f) {
		if (t != null) {
	...
	public static Location getInsideActive(String id) {
		...
		return new Location("INSIDE." + id, "INSIDE", -1, -1, "", -1, -1, -1,
				-1, id);
	}
	...
	public Location reference(String s) {
		...
		if (isSurface() && (s.equals("N") || s.equals("NORTH"))) {
			if (row > 0)
				return new Location(null, type, solar, planet, moon, sector,
						row - 1, col, level, null);
		}
		...
		if (isSurface() && (s.equals("W") || s.equals("WEST"))) {
			...
			if (rc < 0)
				rc = rc + size * 2 * 4;
			return new Location(null, type, solar, planet, moon, rc
					/ (size * 2), row, rc % (size * 2), level, null);
		}
		...
		if (isFacility()
				&& level == 0
				&& (s.equals("U") || s.equals("UP") || s.equals("LAUNCH") || s
						.equals("ESCAPE"))) {
			return new Location(null, type, solar, planet, moon, -1, -1, -1,
					-1, null);
		}
		if (isSurface()
				&& (s.equals("U") || s.equals("UP") || s.equals("LAUNCH") || s
		...
		if (isLevel() && (s.equals("U") || s.equals("UP"))) {
			return new Location(null, type, solar, planet, moon, sector, row,
					col, level - 1, null);
		}
		...
		if (isOrbit()
				&& (s.equals("D") || s.equals("DOWN") || s.equals("LAND") || s
						.equals("REENTRY"))) {
			return new Location(null, type, solar, planet, moon, (int) (Math
					.random() * 4), (int) (Math.random() * size * 2),
					(int) (Math.random() * size * 2), 0, null);
		}
	...
	public boolean isPlanet() {
		return planet != -1 && moon.length() == 0 && sector == -1
				&& fac.length() == 0;
	}
	...
	public boolean isOrbit() {
		return planet != -1 && sector == -1 && fac.length() == 0;
	}

	public boolean isSurface() {
		return sector != -1 && row != -1 && col != -1 && level == 0
				&& fac.length() == 0;
	}

	public boolean isLevel() {
		return level != 0 && fac.length() == 0;
	}

	public boolean isFacility() {
		return fac != null && fac.length() > 0
				&& fac.substring(0, 1).equalsIgnoreCase("F");
	}
	...
	public boolean valid() {
		...
		if (isFacility()) {
			...
			if (facNFO instanceof Facility) {
				...
				if (!loc
						.equalsIgnoreCase(fff.getLocation() + "." + fff.getId()))
					return false;
				return true;
			}
			return false;
		...
		if (row >= 0 && b.getMaxRow() < row)
			return false;
		if (col >= 0 && b.getMaxCol() < col)
			return false;
		// check depth boundary
		if (level > 0 && b.size < level)
			return false;
		return true;
	}
	...
	public void rebuild() {
		...
		if (type != null && type.equalsIgnoreCase("INSIDE")) {
			loc = "INSIDE." + fac;
			return;
		}
		if (solar >= 0) {
	...
	public static boolean adjacentTo(Location one, Location two) {
		...
		if (two.isInside() && !two.isFacility()) {
			...
			if (nfo == null)
				return false;
			if (nfo.getLocation().equals(one))
				return true;
		}

		// any Orbit to any Orbit in same Solar
		if (one.isOrbit() && two.isOrbit() && two.getSolar() == one.getSolar())
			return true;

		// any Surface on same Planet & moon to Orbit()
		// from Orbit to any Surface on same Planet & Moon
		if (one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				&& one.isOrbit() && two.isSurface())
			return true;
		if (one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				&& two.isOrbit() && one.isSurface())
			return true;

		// from sufrace to level or level to level if same solar, planet, moon,
		// sector
		// ADD FOR LEVEL MAX CHECK && one.getLevel()<onep.getMaxLevel() &&
		// two.getLevel()<twop.getMaxLevel()
		if (one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				&& one.getSector() == two.getSector()
				&& one.getRow() == two.getRow()
				&& one.getCol() == two.getCol()
				&& ((one.isLevel() || one.isSurface()) && (two.isLevel() || two
						.isSurface()))
				&& (one.getLevel() == two.getLevel() + 1 || one.getLevel() == two
						.getLevel() - 1) && one.getLevel() >= 0
				&& two.getLevel() >= 0)
			return true;

		// from Level() or Surface to fac if fac in same location && not in
		// facility
		if (one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				...
				&& one.getRow() == two.getRow()
				&& one.getCol() == two.getCol()
				&& one.getLevel() == two.getLevel()
				&& ((one.isFacility() && !two.isFacility()) || (!one
						.isFacility() && two.isFacility())))
			return true;

		// from one Surface() to next if (Row()+/-1==Row() || Col()+/-1 &&
		// Row()+/-1 =! Col()+/-1) && same Sector()
		if (onep != null
				&& twop != null
				&& one.getFac() == null
				&& two.getFac() == null
				&& one.getCol() < onep.getMaxCol()
				&& two.getCol() < twop.getMaxCol()
				&& one.getRow() < onep.getMaxRow()
				&& one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				&& one.getSector() == two.getSector()
				&& one.getRow() == two.getRow()
				&& one.getRow() >= 0
				&& (one.getCol() == two.getCol() + 1 || one.getCol() == two
						.getCol() - 1) && one.getCol() >= 0
				&& two.getCol() >= 0)
			return true;
		if (onep != null
				&& twop != null
				...
				&& one.getCol() == two.getCol()
				&& one.getCol() >= 0
				&& (one.getRow() == two.getRow() + 1 || one.getRow() == two
						.getRow() - 1) && one.getRow() >= 0
				&& two.getRow() >= 0)
			return true;

		// from fac to Orbit() if fac on Surface() of same Planet() ||from
		// Orbit() to fac if fac on same Planet() && on Surface()
		if (one.getSolar() == two.getSolar()
				&& one.getPlanet() == two.getPlanet()
				&& one.getMoon().equalsIgnoreCase(two.getMoon())
				&& ((one.isOrbit() && two.isFacility() && two.getLevel() == 0) || (two
						.isOrbit()
						&& one.isFacility() && one.getLevel() == 0)))
			return true;
		if (one.loc.equalsIgnoreCase(two.loc))
			return true;

		// from one Surface() to next if Sector() +/-1 && Col() =0 and Col() two
		// =max
		if (onep != null
				&& twop != null
				&& one.getFac() == null
				...
				&& one.getRow() >= 0
				&& one.getLevel() == two.getLevel()
				&& ((one.getCol() == 0 && one.getSector() == 0
						&& two.getCol() == twop.getMaxCol() - 1 && two
						.getSector() == 3)
						|| (one.getCol() == 0 && one.getSector() == 1
								&& two.getCol() == twop.getMaxCol() - 1 && two
								.getSector() == 0)
						|| (one.getCol() == 0 && one.getSector() == 2
								&& two.getCol() == twop.getMaxCol() - 1 && two
								.getSector() == 1)
						|| (one.getCol() == 0 && one.getSector() == 3
								&& two.getCol() == twop.getMaxCol() - 1 && two
								.getSector() == 2)
						||

						(one.getCol() == onep.getMaxCol() - 1
								&& one.getSector() == 0 && two.getCol() == 0 && two
								.getSector() == 1)
						|| (one.getCol() == onep.getMaxCol() - 1
								&& one.getSector() == 1 && two.getCol() == 0 && two
								.getSector() == 2)
						|| (one.getCol() == onep.getMaxCol() - 1
								&& one.getSector() == 2 && two.getCol() == 0 && two
								.getSector() == 3) || (one.getCol() == onep
						.getMaxCol() - 1
						&& one.getSector() == 3 && two.getCol() == 0 && two
						.getSector() == 0)))
			return true;
	...
	public int getMoveType(Location dest) {
		if (!adjacentTo(this, dest))
			return -1;

		if (isOrbit() && dest.isOrbit()) {
			return INTERPLANET;
		}

		if (isSurface() && dest.isOrbit()) {
			return ESCAPE;
		}
		if (isOrbit() && dest.isSurface()) {
			return REENTRY;
		}

		if (isFacility() && nested().isSurface() && dest.isOrbit()) {
			return ESCAPE;
		}
		if (isOrbit() && dest.isFacility() && dest.nested().isSurface()) {
			return REENTRY;
		}

		if (isSurface() && dest.isSurface()) {
			return SURFACE;
		}

		if (isLevel() || dest.isLevel()) {
			return SUBTER;
		}

		if (isInside() && !dest.isInside()) {
			return UNLOAD;
		}
		if (!isInside() && dest.isInside()) {
			return LOAD;
	...
	public static Location parse(String s) {
		...
		if (s.startsWith("INSIDE.")) {
			...
			if (nnnn != null && !nnnn.getId().equals(fac))
				fac = nnnn.getId();
			return new Location(loc, type, solar, planet, moon, sector, row,
					col, level, fac);
		}
		...
		try {
			if (s.toUpperCase().indexOf(".L") > 0) {
				level = Integer.parseInt(s.substring(
						s.toUpperCase().indexOf(".L") + 2).trim());
	...
	public static void main(String s[]) {
		...
		System.out.println(L1 + " is "
				+ (adjacentTo(L1, L2) ? "adjacentTo" : "NOT adjacentTo")
				+ " to " + L2 + " type=" + L1.getMoveTypeS(L2));
		System.out.println(L2 + " is "
				+ (adjacentTo(L2, L1) ? "adjacentTo" : "NOT adjacentTo")
				+ " to " + L1 + " type=" + L2.getMoveTypeS(L1));
		...
		System.out.println(L1 + " is "
				+ (adjacentTo(L1, L2) ? "adjacentTo" : "NOT adjacentTo")
				+ " to " + L2 + " type=" + L2.getMoveTypeS(L1));
		System.out.println(L2 + " is "
				+ (adjacentTo(L2, L1) ? "adjacentTo" : "NOT adjacentTo")
				+ " to " + L1 + " type=" + L1.getMoveTypeS(L2));
		...
		System.out.println("reference NORTH " + L1 + " is "
				+ L1.reference("NORTH"));
		System.out.println("reference SOUTH " + L1 + " is "
				+ L1.reference("SOUTH"));
		System.out.println("reference WEST " + L1 + " is "
				+ L1.reference("WEST"));
		System.out.println("reference EAST " + L1 + " is "
				+ L1.reference("EAST"));
		System.out.println("reference UP " + L1 + " is " + L1.reference("UP"));
		System.out.println("reference DOWN " + L1 + " is "
				+ L1.reference("DOWN"));
		System.out.println("reference EXIT " + L1 + " is "
				+ L1.reference("EXIT"));
		L1 = Location.parse("01.03");
		System.out.println("reference DOWN " + L1 + " is "
				+ L1.reference("DOWN"));
		...
		for (int mm = 0; mm < s.length; mm++) {
			...
			for (int mmm = mm; mmm < s.length; mmm++) {
				...
				System.out
						.println(L1
								+ " is "
								+ (adjacentTo(L1, L2) ? "adjacentTo"
										: "NOT adjacentTo") + " to " + L2
								+ " type=" + L2.getMoveTypeS(L1));
				System.out
						.println(L2
								+ " is "
								+ (adjacentTo(L2, L1) ? "adjacentTo"
										: "NOT adjacentTo") + " to " + L1
								+ " type=" + L1.getMoveTypeS(L2));
			}
		}
	}
