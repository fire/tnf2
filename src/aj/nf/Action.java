...
public class Action {
	...
	public String getReport() {
		return "Action: " + Active.myTimeFormat(completetime) + " " + line
	...
	public void post() {
		if (line.toUpperCase().startsWith("PRODUCE")
				|| line.toUpperCase().startsWith("PURCHASE")) {
			String n[] = Stuff.getTokens(line, " \t");
			String designId = n[1];
			if (Main.DEBUG)
				System.out.println("DEBUG: POSTING ACTION");
			...
			if (it instanceof ActiveDesign) {
				if (Main.DEBUG)
					System.out.println("CREATING ACTIVE");
				Active AA = new Active(corptick, designId, location, -1, "",
						"", new Hashtable());
				Universe.add(AA);
				...
				c.addReport("" + (AA.isShip() ? "Ship" : "")
						+ (AA.isRobot() ? "Robot" : "") + " " + AA.getName()
						+ " complete at " + location + " ID=" + AA.getId()
						+ " on " + Active.myTimeFormat(completetime) + "\n");
			} else if (it instanceof FacilityDesign) {
				if (Main.DEBUG)
					System.out.println("CREATING PREFAB");
				...
				c.addReport("" + p.getName() + " Prefab complete at "
						+ location + " ID=" + p.getId() + " on "
						+ Active.myTimeFormat(completetime) + "\n");
			} else {
				System.out.println("MyError: UNKNOWN PRODUCTION ACTION. "
						+ line + "\n" + this);
			}
		}
	}
