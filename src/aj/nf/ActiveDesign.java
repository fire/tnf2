...
public class ActiveDesign implements ITThing {
	...
	public ActiveDesign(String t, String n, Vector p, Vector f, Vector pc, double lp, double me, double cc) {
		maxendurance = me;
		cargocapacity = cc;
		type = t;
	...
	public ActiveDesign(String t, String n, Vector p, Vector f, Vector pc, double lp, double me, double cc, int sid) {
		maxendurance = me;
		cargocapacity = cc;
		type = t;
		activeDesignId = sid;
	...
	public boolean hasTech(String s) {
		for(int a = 0; a < functions.size(); a++) {
			...
			if(sss.equalsIgnoreCase(s))return true;
		}
		return false;
	}
	...
	public void growRandom() {
		...
		for(int a = 0; a < v.size(); a++) {
			...
			try {
				...
				if(cnt > 0)additional.addElement(cnt + "x" + s);
			}
	...
	public Vector getAllProductionCosts() {
		...
		for(int a = 0; a < functions.size(); a++) {
			...
			for(b = 0; b < v2.size(); b++) {
				...
				if(isShip())cnt *= t.getMultiplyerForType("s");
				else if(isRobot())cnt *= t.getMultiplyerForType("r");
	...
	public String display(String tic) {
		...
		String s = (isShip()?"Ship":"Robot") + "Design:" + getId() + " " + name;
		if(pub)s += " (PUBLIC)";
		else if(part) {
		...
		if( pub || part || leasePrice>0) {
			...
			s += "\n  Specs ( Cargo Capacity " + cargocapacity + ", endurance " + Stuff.trunc(maxendurance, 2);
		}
		s += " Mass " + mass + ")";
		s += "\n  Functions (";
		for(int a = 0; a < functions.size(); a++) {
...
//or give to abandonded
	...
	public GmlPair toGmlPair() {
		...
		for(int a = 0; a < functions.size(); a++) {
			String tt = (String)functions.elementAt(a);
			ITThing itt = Universe.getITThingByName(tt);
			if(itt != null)ss += itt.getId();
			else ss += tt;
			if(a + 1 < functions.size())ss += ",";
		}
