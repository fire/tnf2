...
public class Market {
	...
	public void restart() {
		for(int a = 0; a < allItems.size(); a++) {
			...
			if (mi.amt>0) mi.amt=(int)(500+Math.random()*200);
			if (mi.goalSupply>0) mi.goalSupply=500;
			mi.autoPrice();
			mi.adjust();
			mi.adjust();
			mi.adjust();
	...
	public String getOwnReport(Vector v) {
		...
		s += "\nSummary    listed=" + num + "  Average price=" + Stuff.trunc(vt / num, 2) + "\n";
		return s;
	}
	...
	public String getFullReport() {
		...
		s += "\nSummary    listed=" + allItems.size() + "  Average price=" + Stuff.trunc(vt / allItems.size(), 2) + "\n";
		return s;
	}
	...
	public Vector getRandomRecources() {
		...
		for(int a = 0; a < allItems.size(); a++) {
			MarketItem mi = (MarketItem)allItems.elementAt(a);
			if(mi.nativeElement && mi.occuranceRate != 0 && mi.occuranceRate > Math.random()) {
	...
	public MarketItem getItemByName(String n) {
		...
		while(n.indexOf("\"") >= 0)n = n.substring(0, n.indexOf("\"")) + n.substring(n.indexOf("\"") + 1);
		for(int a = 0; a < allItems.size(); a++) {
			MarketItem mi = (MarketItem)allItems.elementAt(a);
			if(mi.getName().equalsIgnoreCase(n) || mi.getId().equalsIgnoreCase(n)) {
				return mi;
			}
		}
		return null;
	...
	public void load() {
		try {
			GmlPair g = GmlPair.parse(new File(Main.DIRDATA+Main.marketFileName));
	...
	public void save() {
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(Main.DIRDATA+Main.marketFileName));
