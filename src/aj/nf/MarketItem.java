...
public class MarketItem {
	...
	public MarketItem(int i, String t, String n, double bp, int gs, int a, double p, boolean ne, String rf, double occ, Vector pf) {
		id = i;
	...
	public void autoPrice() {
		if(isRefined()) {
			MarketItem mi=Universe.getMarket().getItemByName(getRefineMaterial());
			double np = mi.basePrice;
			basePrice = np * REFINEVALUE;
			if (baseMult>0) basePrice*=baseMult;
		}
		else if(isProduced()) {
			int num = 0;
			double tot = 0;
	...
	public void adjust() {
		...
		double demand=Math.max(1/MAXDEMAND,Math.min(MAXDEMAND,1.0*goalSupply/amt));
	...
	public String toDisplay() {
		...
		fixedprice = fixedprice.substring(fixedprice.length() - 12, fixedprice.length());
		String fixedchange = "            " + (change >= 0?"+":"") + Stuff.trunc(change, 2);
		fixedchange = fixedchange.substring(fixedchange.length() - 12, fixedchange.length());
		String fixedamt = "            " + amt;
		fixedamt = fixedamt.substring(fixedamt.length() - 12, fixedamt.length());
		String make="";
		if (isProduced()) {
			make=" Produced ";
			Vector v=getProducedMaterial();
		...
		return"" + fixedname + fixedamt + fixedprice + fixedchange+" "+make;
	}
	...
	public GmlPair toGmlPair() {
		...
		if(isRefined()) {
			if(Universe.getMarketValue(refinedFrom) == 0) {
				System.out.println("MyError: Bad material in refined " + refinedFrom + " in " + name);
			}
		...
		if (isProduced()) {
			...
			for(int a = 0; a < producedFrom.size(); a++) {
				if(Universe.getMarketValue((String)producedFrom.elementAt(a)) == 0) {
					System.out.println("MyError: Bad material in produced " + producedFrom.elementAt(a) + " of " + name);
				}
				slist += (String)producedFrom.elementAt(a);
				if(producedFrom.size() > a + 1)slist += ",";
			}
	...
	public static MarketItem parse(GmlPair g) {
		...
		MarketItem mi=new MarketItem(id, re, na, tp, tq, q, p, ne, rf, occ, pf);
		...
		if(n != null)mi.liquidTemp = n.getDouble();

		n = g.getOneByName("gasTemp");
		if(n != null)mi.gasTemp = n.getDouble();
