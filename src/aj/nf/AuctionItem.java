...
if (n==null) {
	...
	return "System Error.";
		...
		if(bidamount == 0) {
			...
			s += "\n  High bidder is " + highBidder + " at " + Stuff.money(bidamount, 2) + ".";
		}
		s += "\n  Closing ";
		if(close > 2) {
			s += "in " + (close - 1) + " turns\n";
	...
	public boolean newHighBidder(Corp c, double amt) {
		...
		if (hb.equalsIgnoreCase(origOwner)) {
			c.report+="  AUCTION NOTICE:  You cannot bid on your own auction " + nfoid + "\n";
			return false;
		}
		...
		if (alreadyHighBidder) {
			if (amt>maxbidamount) {
				maxbidamount=amt;
				c.report+="  AUCTION NOTICE: Proxy bid on "+nfoid+" set to "+amt+"\n";
				return true;
			}
			else if (amt>bidamount && amt<maxbidamount) {
				c.report+="  AUCTION NOTICE: Cannot lower proxy bid on "+nfoid+".  Prxoy bid remains at "+maxbidamount+"\n";
				return false;
			}
			else if (amt<bidamount) {
				c.report+="  AUCTION NOTICE: Cannot lower winning bid on "+nfoid+".  Bid remains at "+bidamount+"\n";
				return false;
			}
		}
		else {
			if (reserve>amt) {
				c.report+="  AUCTION NOTICE: Insufficient bid on "+nfoid+".  Reserve not met.\n";
				return false;
			}
  			if (maxbidamount*1.1>amt) {
				...
				if(l != null) {
				l.addReport("  AUCTION NOTICE: Proxy bid on " + nfoid + " made with bid of "+Stuff.money(amt,2)+" to match bid by "+c.getTick()+"\n");
						l.makePayment(amt-bidamount);
				}
				bidamount=amt;
				c.report+="  AUCTION NOTICE: Insufficient bid on "+nfoid+".  Out bid by "+highBidder+" proxy bid of "+amt+".\n";
				return false;
			}
  			if (maxbidamount*1.1<=amt) {
				Corp l = Universe.getCorpByTick(highBidder);
				if(l != null) {
					l.addReport("  AUCTION NOTICE: Outbid on " + nfoid + " by "+c.getTick()+" with bid of "+Stuff.money(amt,2)+"\n");
					l.cash += bidamount;

					c.report+="  AUCTION NOTICE:  You are current highbidder on " + nfoid + " exceeding "+highBidder+" bid of "+Stuff.money(bidamount,2)+"\n";
					bidamount = maxbidamount;
					maxbidamount = amt;
					c.makePayment(bidamount);
				}
				else {
					c.report+="  AUCTION NOTICE:  You are current highbidder on " + nfoid + " exceeding the reserve price.\n";
					bidamount = reserve;
					maxbidamount = amt;
	...
	public void transferToBidder() {
		...
		c.addReport("AUCTION NOTICE you have won your bid on " + nfoid + " paying "+Stuff.money(bidamount,2)+"\n");
		Corp l = Universe.getCorpByTick(origOwner);
		l.addReport("AUCTION CLOSED on " + nfoid + " EARNED " + Stuff.money(bidamount, 2) + "\n");
		l.cash += bidamount;
		n.setCorpTick(highBidder);
		if(n.isMoveable())n.setLocation(c.getHome());
